...
TEST_CASE("Check ReplicatedMatrix", "[replicated_matrix]")
{
    ...
    for(int i=0;i<n;i++)diag[i]=static_cast<double>(i);
    ...
    for(int i=0;i<n;i++)CHECK(evals[i] == Approx(diag[i]).epsilon(1.e-8));

    double tr = mat.trace();
    CHECK(tr == Approx(static_cast<double>(n*(n-1)/2)).epsilon(1.e-8));
    }
    ...
    for(int i=0;i<n;i++)
    for(int j=0;j<n;j++)slm.setVal(i,j,value);
    ...
    for(int i=0;i<n*n;i++)CHECK(values[i] == Approx(value*static_cast<double>(npes)).epsilon(1.e-8));
