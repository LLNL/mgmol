...
public:
    ...
    void assign(const Solution& y)
    {
        u_=y.u_;
    }

    void setInvS(const double invs)
    {
        invs_=invs;
    }
    ...
    void setVal(const int i, const double val)
    {
        u_[i]=val;
    }
    void initRand(const int imax)
    {
        ...
        std::unique_ptr<boost::variate_generator<rng_type,distribution_type> > gen
            (new gen_type(rng,nd));
