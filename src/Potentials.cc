...
double Potentials::update(const vector<vector<RHODTYPE> >& rho)
{
    ...
    for(int idx = 0;idx < size_;idx++) {
        vtot_[idx] = (POTDTYPE)(ha2ry*((double)v_nuc_[idx]+(double)v_ext_[idx]
        										+(double)vh_rho_[idx]+(double)vxc_rho_[idx]));
    }    
    double two=ha2ry;
    if(diel_)
        MPaxpy(size_, two, &vepsilon_[0], &vtot_[0]);
...
#ifdef USE_MPI
    double  sum=0.;
    int rc=MPI_Allreduce(&dvdot, &sum, 1, MPI_DOUBLE, MPI_SUM, myPEenv.comm());
    if(rc!=MPI_SUCCESS){
...
#ifdef DEBUG
    if(onpe0)
        (*MPIdata::sout)<<"Potentials::update(mix)"<<endl;
#endif
//    int ione=1;
    double potmix=mix;
...
double Potentials::delta_v(const vector<vector<RHODTYPE> >& rho)
{
    ...
    // update vtot_ (factor 2. to get Rydbergs)
    for(int idx = 0;idx < size_;idx++) {
        dv_[idx] = (POTDTYPE)(ha2ry*((double)v_nuc_[idx]+(double)v_ext_[idx]
        									+(double)vh_rho_[idx]+(double)vxc_rho_[idx]));
    }    
    double two=ha2ry;
    if(diel_)
        MPaxpy(size_, two, &vepsilon_[0], &dv_[0]);
...
// 2->text
// 3->binary
void Potentials::readExternalPot(const string filename,
                                 const short type)
{
    ...
    if( type==3 ){
        ...
        if( mmpi.instancePE0() ){
            ...
            if( length<=0 ){
                (*MPIdata::serr)<<"ERROR Potential: file length <=0!!!!"<<endl;
                ct.global_exit(2);
            }
        }
    }
    ...
    double ll[3]={end[0]-origin[0],end[1]-origin[1],end[2]-origin[2]};
    ...
    for(short d=0;d<3;d++){
        ...
        {
            (*MPIdata::serr)<<"ERROR Potential: Incompatible cell origin in direction "
                <<d<<endl;
            (*MPIdata::serr)<<"Potential origin="<<origin[d]<<endl;
            (*MPIdata::serr)<<"MGmol origin="<<mygrid.origin(d)<<endl;
            (*MPIdata::serr)<<"Difference="<<fabs(origin[d]-mygrid.origin(d))<<endl;
            ct.global_exit(2);
        }
        if( fabs(ll[d]-mygrid.ll(d))>1.e-3 )
        {
            (*MPIdata::serr)<<"ERROR Potential: Incompatible cell dimension in direction "
                <<d<<endl;
            (*MPIdata::serr)<<"Potential cell end="<<end[d]<<endl;
            (*MPIdata::serr)<<"Potential cell dimension="<<ll[d]<<endl;
            (*MPIdata::serr)<<"MGmol cell dimension="<<mygrid.ll(d)<<endl;
            ct.global_exit(2);
        }
    }
    ...
    for(short i=0;i<3;i++)
    if( nxyz[i]!=gdim_[i] ){
        (*MPIdata::serr)<<"Potentials::read_ExternalPot(): dimension "<<i
            <<" incompatible with Grid!!!"<<endl;
        (*MPIdata::serr)<<"n="<<nxyz[i]<<", gdim_="<<gdim_[i]<<endl;
        ct.global_exit(2);
    }
    ...
    int start=mygrid.istart(0)*incx
             +mygrid.istart(1)*incy;
        ...
        for(int i=0;i<dim_[0];i++){
            ...
            for(int j=0;j<dim_[1];j++){
                ...
                for(int k=0;k<dim_[2];k++){
                    ...
                    //(*MPIdata::sout)<<", v_ext_["<<index<<"]="<<v_ext_[index]<<endl;
                    index++;
                }
    ...
    if( type==3 ){
        ...
        for(int i=0;i<dim_[0];i++){
            ...
            for(int j=0;j<dim_[1];j++){
                ...
                // advance endz-startz-dim_[2] positions
                from->seekg( (endz-startz-dim_[2])*sizeof(float), ios::cur );
            }
            file_index+=gdim_[2]*dim_[1];
...
bool Potentials::withVext()const
{
    return( vext_tricubic_!=NULL );
}
...
#else
            (*MPIdata::sout)<<"ERROR: cannot read external potential "
...
void Potentials::setVh(const pb::GridFunc<POTDTYPE>& vh, const int iterativeIndex)
{
    assert( iterativeIndex>=0 );
...
template void Potentials::setVxc<double>(const double* const vxc, const int iterativeIndex);
template void Potentials::setVxc<float>(const float* const vxc, const int iterativeIndex);
