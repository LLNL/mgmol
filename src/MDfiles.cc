...
void MDfiles::createSnapshotDir(const int mdstep, string& md_print_dir)
{
    ...
    if( onpe0 )
    {
        cout<<"Print coordinates and forces in directory "<<md_print_dir<<endl;
        struct stat buf;
...
void MDfiles::gatherStrings(vector<string>& ions_names, 
                            vector<char>& recvbufs)
{
    int size_s=0;
    ...
    int mpi_err=MPI_Gather(&size_s, 1, MPI_INT, 
                           &recvcounts[0], 1, MPI_INT, 0, sub_comm_);
    if( mpi_err!=MPI_SUCCESS )
    {
        (*MPIdata::serr)<<"MDfiles::gatherStrings(), ERROR in MPI_gather!!!"<<endl;
    }
    ...
    mpi_err=MPI_Gatherv(&buffer[0],   size_s,          MPI_CHAR, 
                        &recvbufs[0], &recvcounts[0], &displs[0], MPI_CHAR, 
                        0, sub_comm_);

    if( mpi_err!=MPI_SUCCESS )
    {
        (*MPIdata::serr)<<"MDfiles::gatherStrings(), ERROR in MPI_Gatherv !!!"<<endl;
    }
}

void MDfiles::gatherVector3D(vector<Vector3D>& vectors,
...
void MDfiles::printDataInFiles(vector<string>& ions_names,
                               vector<double>& tau,
                               ...
                               const double dt)
{
    ...
    //if(key_==0)cout<<"key="<<key_<<", color="<<color_<<", sum of tau sizes: "<<sumtausize<<endl;
    ...
    if( key_==0 )
    {
        // atomic coordinates
        {
            ...
            if ( !tfile.is_open() )
            {
                cerr << " Unable to open file " << coord_dir_name.data() << endl;
                ct.global_exit(0);
            }
