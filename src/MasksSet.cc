...
{ 
    ...
    if( r<r1 )return 1.;
    else      return (lmasktype)((1.-r)*inv_one_minus_r1);
}
...
{ 
    ...
    float val=1.;
    if( r>rc_exp2 )val = (exp(-fac_exp2*(r-rc_exp2)*(r-rc_exp2))-exp2_rc)*exp2_coeff;
    assert( val>=0. );
    assert( val<=1. );
...
void MasksSet::setup(const LocalizationRegions& lrs,
                     const double override_radius)
{
    Control& ct = *(Control::instance());
    if( onpe0 && ct.verbose>0 ) (*MPIdata::sout)<<"MasksSet::setup()"<<endl;
    ...
    if( onpe0 && ct.verbose>0 )
        (*MPIdata::sout)<<"MasksSet: Total: "<<pgrid_masks_.size()<<" masks"<<endl;
}
...
#if USE_MASKMAX
            grid_mask
                =new GridMaskMax(mg_levels_, subdivx, mygrid);
#else
            grid_mask
                =new GridMaskMult(mg_levels_, subdivx, mygrid);
#endif
        }
        else
        {
...
int MasksSet::initialize(const LocalizationRegions& lrs, 
                         const unsigned short ln,
                         const double override_radius)
{
    Control& ct = *(Control::instance());
    if( onpe0 && ct.verbose>0 )
        (*MPIdata::sout)<<" MasksSet::initialize(), ln="<<ln<<", n="<<pgrid_masks_.size()<<endl;
    
#if USE_MASKMAX
    lmasktype (*maskfunc)(const double) = type_corr_mask_ ? &one : &gaussianDecay;
#else
...
#pragma omp parallel private(it)
            ...
            it++)
        {
            ...
            {
                int gid=it->first;
                (it->second)->init(lrs.getCenter(gid), override_radius, ln, maskfunc);
            }
        }
    }
