...
void MultipoleExpansion::get_dipole(RHODTYPE* rho)
{
    ...
    (*MPIdata::sout)<<"MultipoleExpansion::get_dipole(), Total Dipole=("
        <<dipole_moment_[0]<<","<<dipole_moment_[1]<<","<<dipole_moment_[2]<<")"<<endl;
}

//template <typename T>
void MultipoleExpansion::get_dipole(const pb::GridFunc<RHODTYPE>& rho)
{
    const bool mympi0 = ( grid_.mype_env().my_mpi(0)==0
                       && bc_[0]!=1 );
    const bool mympi1 = ( grid_.mype_env().my_mpi(1)==0
                       && bc_[1]!=1 );
    const bool mympi2 = ( grid_.mype_env().my_mpi(2)==0
                       && bc_[2]!=1 );
    ...
    const int istart[3] = {mympi0 ? nghosts+1 : nghosts,
                           mympi1 ? nghosts+1 : nghosts,
                           mympi2 ? nghosts+1 : nghosts};
...
void MultipoleExpansion::computeQuadrupole(const pb::GridFunc<RHODTYPE>& rho)
{
	...
	const int iix = ix*incx;
	    ...
	    const int iiy=iy*incy+iix;
            ...
            for(int iz = istart[2];iz < nghosts+dim[2];iz++) {	    
                gpoint[2]=start[2]+hspacing[2]*iz-origin_[2];
                const double ri2=(gpoint[0]*gpoint[0]
                                 +gpoint[1]*gpoint[1]
                                 +gpoint[2]*gpoint[2]);
    ...
    if(onpe0_)
    (*MPIdata::sout)<<"MultipoleExpansion::computeQuadrupole(), quadrupole=("
        <<setprecision(5)<<scientific
        <<quadrupole_moment_[0]<<","
        <<quadrupole_moment_[1]<<","
        <<quadrupole_moment_[2]<<","
        <<quadrupole_moment_[3]<<","
        <<quadrupole_moment_[4]<<","
        <<quadrupole_moment_[5]<<")"<<endl;
}
//template <typename T>
void MultipoleExpansion::resetOriginToChargeCenter(const pb::GridFunc<RHODTYPE>& rho)
{
    const bool mympi0 = ( grid_.mype_env().my_mpi(0)==0
                       && bc_[0]!=1 );
    const bool mympi1 = ( grid_.mype_env().my_mpi(1)==0
    ...
    if(onpe0_)
    (*MPIdata::sout)<<"MultipoleExpansion::resetOriginToChargeCenter(), charge_center=("
...
#endif
    if(onpe0_)
    (*MPIdata::sout)<<"MultipoleExpansion::resetOriginToChargeCenter(), new origin_=("
        <<origin_[0]<<","<<origin_[1]<<","<<origin_[2]<<")"<<endl;
}
//template <typename T>
...
void MultipoleExpansion::expand2d(pb::GridFunc<T>& func)
{
    ...
    if(onpe0_){
        (*MPIdata::sout)<<"MultipoleExpansion::expand2d(), origin 0="<<origin_[index0]
          <<", origin 1="<<origin_[index1]
          <<", order="<<order_
          <<endl;
    }
    ...
    const double invlp=2./grid.ll(indexp); // factor 2 for Hartree units in 2D
    ...
    if( order_>=1 ){ // dipole
        ...
        for(int i0=0;i0<dim0g;i0++){
            ...
            for(int i1=0;i1<dim1g;i1++){
                ...
                for(int ip=0;ip<dimpg;ip++)
                {
                    double addval = (double)pfunc[is1+ip*incp] + rdotp*alpha*invlp;
    ...
    if( order_>=2 ){ // quadrupole
        ...
        switch( indexp ){
            ...
            case 2:
                ...
                break;
            default:
                (*MPIdata::serr)<<"Invalid case for quadrupole moment"<<endl;
                exit(1);
        }
        ...
        for(int ix=0;ix<dim0g;ix++){
            ...
            for(int iy=0;iy<dim1g;iy++){
                ...
                for(int iz=0;iz<dim2g;iz++){
                    ...
                    const double r2=vr[0]*vr[0]+vr[1]*vr[1]+vr[2]*vr[2];
