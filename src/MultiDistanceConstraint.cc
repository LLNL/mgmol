...
////////////////////////////////////////////////////////////////////////////////
void MultiDistanceConstraint::setup(
    const vector<string>& names_known_locally,
    ...
    const vector<string>& local_names)
{
        ...
        for ( int ia = 0; ia < (int)names_known_locally.size(); ia++ )
        {
            if ( m_name1_[i].compare(names_known_locally[ia])==0 )
            {
                ...
                for ( int j = 0; j < (int)u_name_.size(); j++ )
                    if ( u_name_[j].compare( m_name1_[i] )==0 )
                        found = true;
                if ( !found )
                {  
            ...
            else if ( m_name2_[i].compare(names_known_locally[ia])==0 )
            {
                bool found = false;
                for ( int j = 0; j < (int)u_name_.size(); j++ )
                    if ( u_name_[j].compare( m_name2_[i] )==0 )
                        found = true;
                if ( !found )
                {
                    u_name_.push_back(names_known_locally[ia]);
    ...
    if( check!=1 )
    {
        ...
        cerr<<"Cannot find "<<m_name1_[0]<<" in any list of local atoms..."<<endl;
    ...
    for ( int p = 0; p < na; p++)
    {
        for ( int e = 0; e < 3; e++)
        {
            ...
            for ( int k = 0; k < nc; k++)
            {
                ...
                a_[p][e]+=m_alpha_[k]*(d1-d2)*(m_tau1_[k][e]-m_tau2_[k][e])/d;
            }
        }
    }
...
bool MultiDistanceConstraint::project_out_forces(void)
{
    ...
    for ( int p = 0; p < na; p++)
    {
        ...
        { 
            ...
            if( d1 || d2 ){
              ...
              dd /= sqrt(dr2);
              // projection
              proj += dd*(dx*u_fion_[p][0]+dy*u_fion_[p][1]+dz*u_fion_[p][2]);
              // gradient
              a_[p][0]+=dd*dx;
              a_[p][1]+=dd*dy;
              a_[p][2]+=dd*dz;
    ...
    // (may be 0. if atoms are not local and forces are not computed for these atoms)
    double md2=0.;
    for ( int p = 0; p < na; p++)
    {
    ...
    for ( int p = 0; p < na; p++)
    {
        ...
        { 
            ...
            if( d1 || d2 ){
                ...
                dd /= sqrt(dr2);
                // projection
                de -= dd*(dx*u_fion_[p][0]+dy*u_fion_[p][1]+dz*u_fion_[p][2]);
                // gradient
                a_[p][0]+=dd*dx;
                a_[p][1]+=dd*dy;
                a_[p][2]+=dd*dz;
...
ostream& MultiDistanceConstraint::printConstraint( ostream &os )const
{
    ...
    for (int ic = 0; ic < (int)m_alpha_.size(); ic++ )
        os << setw(10) << setprecision(6) << m_alpha_[ic]
           <<" "<< m_name1_[ic]<<" "<<m_name2_[ic]<<" ";
...
void MultiDistanceConstraint::printForce( ostream &os )const
{
    ...
    printConstraint(os);
    os << ", force = " << setw(10) << setprecision(6)
       << projected_force() << endl;
}
