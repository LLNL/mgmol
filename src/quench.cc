...
// 1 -> radius only
// 2 -> center and radius
void MGmol::adaptLR(const SpreadsAndCenters* spreadf,
                    const OrbitalsTransform* ot)
{        
    Control& ct = *(Control::instance());
    if( ct.verbose>0 )printWithTimeStamp(" Adapt localization regions...",os_);
...
updateCenters_tm.stop();
    ...
    if( ct.wannier_transform_type>0 && ct.lr_updates_type>0 )
    {
        ...
        if( ct.lr_volume_calc==1 )
        {
            ...
            if( onpe0 && ct.verbose>1 )
            {
                os_<<" vol. rotated localized orbitals = "<<vol_rotated<<endl;
        ...
        else if( ct.lr_volume_calc==2 )
        {
            assert( ot!=NULL );
            if( onpe0 )
                os_<<" Adapt LR using NOLMO spreads"<<endl;
        ...
        {
            if( onpe0 )
                os_<<" Adapt with constant LR volume"<<endl;
...
void MGmol::updateHmatrix(LocGridOrbitals& orbitals, 
                          Ions& ions)
{
...
void MGmol::resetProjectedMatricesAndDM(LocGridOrbitals& orbitals, 
                                        Ions& ions)
{
...
bool MGmol::rotateStatesPairsCommonCenter(LocGridOrbitals& orbitals,
                                          LocGridOrbitals& work_orbitals)
{
    ...
    if( onpe0 )os_<<"drmin="<<drmin<<", ct.getMinDistanceCenters()="<<ct.getMinDistanceCenters()<<endl;
    ...
    {
        if( onpe0 && ct.verbose>1 )
            os_<<"Min. distance between centers "
                <<st1<<" and "<<st2<<" = "<<drmin<<endl;
        ...
        // save centers
        save_centers.insert(pair<int,Vector3D>(st1,spreadf2st.computeCenter(0)));
        save_centers.insert(pair<int,Vector3D>(st2,spreadf2st.computeCenter(1)));
                                     ...
                                     const double threshold)
{
    ...
    {
        os_<<"-------------------------"<<endl;
        os_<<"rotateStatesPairsOverlap: compute smallest eigenvalue of S..."<<endl;
    }
    double eigmin=proj_matrices_->getLinDependent2states(st1, st2, ct.verbose>2);

    if( onpe0 )
    {
        os_<<setprecision(3)<<scientific
           <<"eigmin="<<eigmin
           <<", threshold="<<threshold
           <<endl;
    }
    ...
    while( eigmin<threshold )
    {
        if( onpe0 && ct.verbose>1 )
        {
            ...
            os_<<setprecision(5)<<scientific
               <<"Min. eigenvalue for states "<<st1<<", "<<st2<<" = "<<eigmin<<endl;
        }
        ...
        if( d>ct.getThresholdDistancePairMLWF() )
        {
            if( onpe0 )
            {
                cout<<"Smallest eigenvalue not related to close LR centers..."<<endl;
                cout<<"d="<<d<<endl;
        ...
        ProjectedMatricesSparse* projmatrices=dynamic_cast<ProjectedMatricesSparse*>(proj_matrices_);
        if( onpe0 )cout<<"Gram Matrix for pair of states after transformation:"<<endl;
        ...
        eigmin=proj_matrices_->getLinDependent2states(st1, st2, ct.verbose>2);
        ...
        {
            if( onpe0 )cout<<"Same pair found twice in a row... stop MLWF transforms..."<<endl;
            break;
        }
    };    
    if( onpe0 )cout<<endl;
...
void MGmol::disentangleOrbitals(LocGridOrbitals& orbitals, LocGridOrbitals& work_orbitals,
                         Ions& ions,
                         int& max_steps)
{    
    Control& ct = *(Control::instance());
    ...
    {
        wannier_pairs=rotateStatesPairsOverlap(orbitals, work_orbitals,ct.getThresholdEigenvalueGram());
    }
...
int MGmol::quench(LocGridOrbitals* orbitals,
           Ions& ions, 
           const int max_inner_steps,
           const int iprint,
           double& last_eks)
{
    ...
    quench_tm.start();
    int retval=1; // 0 -> converged, -1 -> problem, -2 -> ( de>tol_energy_stop )
    ...
    orbitals_precond_->setup(*orbitals,ct.getMGlevels(),ct.lap_type,
                             currentMasks_, lrs_);
    ...
    switch ( ct.it_algo_type )
    {
        case 0:
        {
            DFTsolver solver(hamiltonian_,
                        proj_matrices_, energy_, electrostat_,
                        this, ions, rho_, dm_strategy_,
                        os_);
        ...
        case 3:
        {
           PolakRibiereSolver solver(hamiltonian_,
                        proj_matrices_, energy_, electrostat_,
                        this, ions, rho_, dm_strategy_,
                        os_);
    ...
    if( ct.short_sighted )
    {
        ...
        if( onpe0 ){
            os_<<setprecision(8)<<fixed<<" Enl            [Ha] = "<<evnl<<endl;
        }
    }
    ...
    if( ct.verbose>1 ){
        if( onpe0 ){
            os_<<setprecision(8)<<fixed<<" TS             [Ha] = "<<ts<<endl;
        }
    }
    ...
    if( ct.computeCondGramMD() )
    {
        ...
        if( onpe0 )
            os_<<setprecision(2)<<scientific<<"Condition Number of S: "<<condS<<endl;
    }
