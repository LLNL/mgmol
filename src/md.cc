...
{ 
    ...
    {
        if( ct.dm_mix<1. )proj_matrices_->dressupDM();
        else              proj_matrices_->setDMto2InvS();
    }
}
...
void MGmol::extrapolate_centers(bool small_move)
{
    ...
    if( onpe0 && ct.verbose>0 )
        os_<<"Extrapolate LR centers..."<<endl;
...
void checkMaxForces(const vector<double>& fion,
...
int MGmol::dumprestartFile(LocGridOrbitals** orbitals, Ions& ions, Rho& rho, 
                           const bool write_extrapolated_wf,
                           const short count)
{
    ...
    if( ierr<0 ) {
        if(onpe0)(*MPIdata::serr)<<"dumprestartFile: cannot write ...previous_orbitals..."<<endl;
        return ierr;
    }
    ...
    if( write_extrapolated_wf && ct.out_restart_info>2 )
    {
        ...
        if( ierr<0 )
        {
            if(onpe0)(*MPIdata::serr)<<"dumprestartFile: cannot write ...ExtrapolatedFunction..."<<endl;
            return ierr;
        }
    }
    ...
    if( ierr<0 )
    {
        if(onpe0)(*MPIdata::serr)<<"dumprestartFile: cannot close file..."<<endl;
        return ierr;
    }
...
void MGmol::md(LocGridOrbitals** orbitals, 
               Ions& ions)
{
    Control& ct = *(Control::instance());
    ...
    vector<unsigned short>& rand_states(ions.getRandStates());    // initial random states 
    ...
    orbitals_extrapol=OrbitalsExtrapolationFactory::create(ct.wf_extrapolation, NULL);

    MD_IonicStepper* stepper
        = new MD_IonicStepper(ct.dt, atmove, tau0, taup, taum, fion, pmass, rand_states);
    ...
    {
        if( onpe0 )
            os_<<"Use restart file to initialize MD..."<<endl;
        stepper->init(*h5f_file_);
    }
    else
    {
        if( onpe0 )
            os_<<"Use input file to initialize MD..."<<endl;
    ...
    if(ct.restart_info > 1)
    {
        ...
        if( onpe0 ){
            flag_extrapolated_data=h5f_file_->dset_exists("ExtrapolatedFunction0000");
            if( flag_extrapolated_data==0 )
                flag_extrapolated_data=h5f_file_->dset_exists("ExtrapolatedFunction0");
        }
#ifdef USE_MPI
        ...
        if(ct.restart_info > 2)
        {
            ...
            {
                if( onpe0 )
                    os_<<"Create new orbitals_minus1..."<<endl;


                orbitals_extrapol->setupPreviousOrbitals(&current_orbitals_,
                   proj_matrices_,
                   lrs_,local_cluster_,currentMasks_,corrMasks_,*h5f_file_);
    ...
    for(int mdstep = 1;mdstep <= ct.num_MD_steps;mdstep++)
    {
        ...
        {
            if( onpe0 )
                os_<<"Signal caught: No more MD iterations"<<endl;
            break;
        }
        ...
        do
        {
            retval=quench(*orbitals, ions,  
                          ct.max_electronic_steps, 0, eks);
            ...
            if( ct.adaptiveLRs() )
            {
                ...
                if(ct.load_balancing_alpha > 0.0 && mdstep%ct.load_balancing_modulo == 0)
                {
            ...
            if( !lrs_->moveIsSmall() )
            {
                printWithTimeStamp("WARNING: large move->extra inner cycle...",cout);
                small_move=false;
        ...
        if( retval<0 )
        {
            ...
            if( ct.dt>0. )
            {
                if( onpe0 )
                {
                    os_<<"WARNING md(): quench returned value "<<retval<<endl;
                    os_<<"STOP md..."<<endl;
                }
                break;
            }
        ...
        if( onpe0 )
            os_<<setprecision(12)<<fixed
                <<"%%  "<<md_iteration_<<"  IONIC CONFIGURATION ENERGY = "
                <<eks<<endl;
        ...
        if( md_iteration_%ct.md_print_freq==0 )
        {
            ...
            if( zero.compare (ct.md_print_filename)!=0 )
            {
                ...
                md_files.printDataInFiles(ions_names,tau0,fion,taum,
                                          centers,spreads,lgids,md_iteration_,ct.dt);
            }
        ...
        if( ct.dt<=0. )
            checkMaxForces(fion, atmove, os_);
        ...
        if( ct.verbose>2 && ct.dt>0. )
            stepper->printVelocities(os_);
        ...
        {
            if( ct.lrs_extrapolation>0 )
                extrapolate_centers(small_move);
        ...
        if( (md_iteration_%ct.checkpoint)==0 && mdstep<ct.num_MD_steps )
        {
            int ierr=-1;
            short count=0;
            ...
            {
                dump_tm_.start();
                ierr=dumprestartFile(orbitals, ions, *rho_, extrapolated_flag, count);
                dump_tm_.stop();
