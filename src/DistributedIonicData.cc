...
DistributedIonicData::DistributedIonicData(const std::vector<std::string>& local_names, 
                     const std::vector<double>& local_data):
    ion_names_(local_names),
    data_(local_data)
{
...
// augment Data
void DistributedIonicData::augmentData(const int nsteps, const int dir, const int disp, 
                            ...
                            DistributedIonicData& data2send)
{
    ...
    while(step < nsteps)
    {
       ...
       // doubles data
       MPI_Irecv(dbuff_r, dbuff_size_r, MPI_DOUBLE, src, 0, cart_comm_, &request2[0]);
