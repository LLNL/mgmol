...
private:
    ...
    void augmentOverlapRegions(const short dir, const short disp, const int buffer_size, 
                               const std::vector<LRData>& aug_data, std::set<int>& overlap_gid_map);   
    ...
    int data_pos_[3]; //starting position of ints (gids), floats (radii) and doubles (centers)       

    void push_back_global(const Vector3D& center, 
                   const float radius, 
                   const std::vector<int>& gid);

    void push_back_local(const Vector3D& center, 
                         const float radius, 
                         const std::vector<int>& gid);
...
protected:
    ...
    std::vector<LRData> local_regions_; // center of LR located in local subdomain
...
public:
    ...
    LocalizationRegions(const Vector3D& cell,
                        const float tol_displ=1.e10 ):
       volume_(0.),
    ...
    virtual void setup()
    {
        ...
        if( ct.verbose>0 )
            printWithTimeStamp("LocalizationRegions::setup()...",(*MPIdata::sout));
        ...
        if( ct.restart_info<=2 )
        {
            ...
            bcastLRs();
            computeOverlapRegionsGlobal(); // compute overlaps before setting up local regions
    ...
    void printInfo(std::ostream& os)
    {
        ...
        {
            os<<"LocalizationRegions: Number of localization regions = "<<nglobal_<<std::endl;
        }
    }
    ...
    float moveTo(const std::vector<Vector3D>& target_centers, const std::vector<Vector3D>& shifts);

    bool moveIsSmall()
    {
        return ( max_displ_<tol_displ_ );
    }
    ...
    const Vector3D& getCenter(const int gid)const 
    {
        ...
        std::cout<<"mype:"<<mype<<", WARNING: didn't find a center for gid="<<gid<<std::endl;
    ...
    int getLRsColor(const int gid)const 
    {
        ...
        std::cout<<"mype:"<<mype<<", WARNING: didn't find a color for gid="<<gid<<std::endl;
        MPI_Abort(MPI_COMM_WORLD,0);
        return -1;
    }
    ...
    double getCenterAndRadius(const int gid, Vector3D& center)const 
    {
        ...
        std::cout<<"mype:"<<mype<<", WARNING: didn't find a center and radius for gid="<<gid<<std::endl;
        MPI_Abort(MPI_COMM_WORLD,0);
    ...
    void setCenters(const std::vector<double>& tau)
    {
        ...
        for(int j=0;j<nc;j++)
        {
            overlap_regions_[j].center.assign(tau[3*j  ],
                                      tau[3*j+1],
                                      tau[3*j+2]);
        }
    ...
    // compute distance square between two centers of global indexes gid0 and gid1
    ...
    const std::vector<int>& getOverlapGids()const
    {
        return overlap_gids_;
    }
    const std::vector< std::vector<int> >& getSubdivOverlapGids()const
    {
        return subdiv_overlap_gids_;
    }
    int getNumOverlapGids()const
    {
        return overlap_gids_.size();
    }
    //bool gidIncluded(const int gid)
    //{
    //    return( find(overlap_gids_.begin(),overlap_gids_.end(),gid)!=overlap_gids_.end() );
    //}
    ...
    void getMatrixDistances(SquareLocalMatrices<MATDTYPE>& mat,const std::vector<std::vector<int> >& gids);
    ...
    void printMinMaxRadius(std::ostream& os)
    {
        ...
        if( onpe0 )
        {
            os<<"Radius LRs: min = "<<minr<<", max = "<<maxr<<std::endl;
        }
    }
