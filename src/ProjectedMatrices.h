...
class ProjectedMatrices:public ProjectedMatricesInterface
{
    ...
    double computeChemicalPotentialAndOccupations(const std::vector<DISTMATDTYPE>& energies, 
                                                  const double width, 
                                                  const int nel,
                                                  const int max_numst);
...
public:
    ...
    void setup(const double kbt, const int nel, const std::vector<std::vector<int> >& global_indexes);
    ...
    SquareLocalMatrices<MATDTYPE>& getLocalX()const
    {
        return *localX_;
    }
    
    SquareLocalMatrices<MATDTYPE>& getLocalT()const
    {
        return *localT_;
    }
    ...
    void updateSubMatX(const dist_matrix::DistMatrix<DISTMATDTYPE>& dm)
    {
        ...
        for(short iloc=0;iloc<subdiv_;iloc++){
            ...
            for(int jcolor=0;jcolor<chromatic_number_;jcolor++)
                localX_iloc[icolor+chromatic_number_*jcolor]=submatWork_->val(icolor,jcolor,iloc);
        }
    ...
    void printHamiltonianMM(ofstream& tfile)
    {
        matH_->printMM(tfile);
    }
    ...
    bool occupationsUptodate()const
    {
        return dm_->occupationsUptodate();
    }
    ...
    const dist_matrix::DistMatrix<DISTMATDTYPE>& getH()
    {
        return *matH_;
    }
    ...
    virtual void updateHB()
    {
        setHB2H();
    }

    void setGram2Id(const int orbitals_index)
    {
        gm_->set2Id(orbitals_index);
    }
    
    void setGramMatrix(const dist_matrix::DistMatrix<DISTMATDTYPE>& mat, const int orbitals_index)
    {
        assert( gm_!=0 );
    ...
    void setDMto2InvS();
    void buildDM(const dist_matrix::DistMatrix<DISTMATDTYPE>& z, const int orbitals_index);
    void buildDM(const dist_matrix::DistMatrix<DISTMATDTYPE>& z, 
                 const std::vector<DISTMATDTYPE>&,
                 const int orbitals_index);
    ...
    void updateDMwithEigenstatesAndRotate(const int iterative_index,
                                          dist_matrix::DistMatrix<DISTMATDTYPE>&  zz);
    double computeChemicalPotentialAndOccupations(const double width, 
                                                  const int nel,
                                                  const int max_numst)
    {
    ...
    {
        if( onpe0 )
            cout<<"ProjectedMatrices::saveDM()"<<endl;
        if( !mat_X_old_ )
        {
            mat_X_old_=new dist_matrix::DistMatrix<DISTMATDTYPE>(dm_->getMatrix());
            mat_L_old_=new dist_matrix::DistMatrix<DISTMATDTYPE>(gm_->getCholeskyL());
        }
        else
        {
    ...
    {
        (void) ss;
        cerr<<"ERROR: ProjectedMatrices::initializeMatB() should never be called!!!"<<endl;
    }
#if EXTRAPOLATE_H
    void initExtrapolationH()
    {
        *new_h_=*matHB_;
    }
    ...
    double computeTraceInvSmultMat(const dist_matrix::DistMatrix<DISTMATDTYPE>& mat)
    {
    ...
    double computeTraceInvSmultMatMultTheta(const dist_matrix::DistMatrix<DISTMATDTYPE>& mat);   
    ...
    void computeMatMultTheta(const dist_matrix::DistMatrix<DISTMATDTYPE>& mat, dist_matrix::DistMatrix<DISTMATDTYPE>& pmat)
    {
        assert(theta_ != 0);
        pmat.gemm('n', 'n', 1.0, mat, *theta_, 0.); 
    }     
    const dist_matrix::DistMatrix<DISTMATDTYPE>& getMatHB()
    {
        return *matHB_;
    }
    void setDM(const dist_matrix::DistMatrix<DISTMATDTYPE>& mat, const int orbitals_index)
    {
