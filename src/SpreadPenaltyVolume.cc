...
void SpreadPenaltyVolume::addResidual(LocGridOrbitals& phi,
                                      LocGridOrbitals& res)
{
    ...
    //if( onpe0 && ct.verbose>1 )cout<<"Average Spread Volume="<<setprecision(4)<<sum_sigma3/phi.numst()<<endl;
    ...
    // setup factors for corrections proportional to gradient of penalty spread functional
    float max_spread2=0.;
    ...
    if( onpe0 && ct.verbose>1 )
        cout<<"Max. spread="<<setprecision(4)<<sqrt(max_spread2)
            ...
            <<", Average spread Volume="<<setprecision(4)<<sum_sigma3/phi.numst()
...
void SpreadPenaltyVolume::computeAndAddResidualSpreadPenalty(const vector<float>& sigma,
                                                       const float eta,
                                                       ...
                                                       LocGridOrbitals& res)
{
    ...
    for(vector<float>::const_iterator it =sigma.begin();
                                      it!=sigma.end();
                                    ++it)
    {
        float lm=(*it)*(*it)*(*it);
                ...
                for(int ix = loc_length*iloc;ix < loc_length*(iloc+1);ix++)
                {
                    ...
                    for(int iy = 0;iy < dim[1];iy++)
                    {
                        ...
                        for(int iz = 0;iz < dim[2];iz++)
                        {
                            ...
                            float val=eta*(2.*sigma*(x2+y2+z2) -lambda );
