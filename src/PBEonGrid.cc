...
//    int     ione=1;
    ...
    pb::GridFunc<RHODTYPE> gf_rho(&vrho[0][0],newGrid,
                    ct.bc[0],ct.bc[1],ct.bc[2],'d');
    ...
    for(int i=0;i<3;i++)delete myoper_del[i];
    
#ifdef USE_LIBXC
    xc_gga_exc_vxc(&xfunc_, np_, &rho_.rho_[0][0], sigma, &exc_[0], &vxc_[0], &vsigma_[0]);
    vector<double> vtmp(np_);
    vector<double> etmp(np_);
    vector<double> stmp(np_);
    xc_gga_exc_vxc(&cfunc_, np_, &rho_.rho_[0][0], sigma, &etmp[0], &vtmp[0], &stmp[0]);
    delete[] sigma;
    ...
    pb::GridFunc<POTDTYPE> gf_vsigma(&vstmp[0],newGrid, ct.bc[0],ct.bc[1],ct.bc[2],'d');
    gf_vsigma*=2.;
#else
    pbe_->computeXC();

    pb::GridFunc<POTDTYPE> gf_vsigma(pbe_->pvxc2_,newGrid, ct.bc[0],ct.bc[1],ct.bc[2],'d');
    gf_vsigma*=-1.;
#endif
...
#ifdef USE_LIBXC
    int ione=1;
    double exc= mygrid.vel()*ddot(&np_, &rho_.rho_[0][0], &ione, &exc_[0], &ione);
