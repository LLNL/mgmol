...
void PBEFunctional::computeXC(void) 
{
    if ( nspin_ == 1 )
    {
        ...
        for ( int i = 0; i < np_; i++ )
        {
            double gp = (double)grad0[i]*(double)grad0[i]+(double)grad1[i]*(double)grad1[i]+(double)grad2[i]*(double)grad2[i];
    ...
    else
    {
        assert( prho_up_ != 0 );
        assert( prho_dn_ != 0 );
        assert( pgrad_rho_up_[0] != 0 
             && pgrad_rho_up_[1] != 0 
             && pgrad_rho_up_[2] != 0 );
        assert( pgrad_rho_dn_[0] != 0 
             && pgrad_rho_dn_[1] != 0 
             && pgrad_rho_dn_[2] != 0 );
        assert( pexc_up_ != 0 );
        assert( pexc_dn_ != 0 );
        ...
        for ( int i = 0; i < np_; i++ )
        {
          ...
          double grxyz_up = (double)grx_up*(double)grx_up + (double)gry_up*(double)gry_up + (double)grz_up*(double)grz_up;
          double grad_up = sqrt(grxyz_up);
          double grxyz_dn = (double)grx_dn*(double)grx_dn + (double)gry_dn*(double)gry_dn + (double)grz_dn*(double)grz_dn;
          double grad_dn = sqrt(grxyz_dn);
          double grad    = sqrt(grx*grx + gry*gry + grz*grz);
          excpbe_sp(prho_up_[i],prho_dn_[i],grad_up,grad_dn,grad,
                    &pexc_up_[i],&pexc_dn_[i],
                    &pvxc1_up_[i],&pvxc1_dn_[i],&pvxc2_upup_[i],&pvxc2_dndn_[i],
                    &pvxc2_updn_[i], &pvxc2_dnup_[i]);
        }
    }
}
...
void PBEFunctional::excpbe_sp(RHODTYPE rho_up, RHODTYPE rho_dn, 
  RHODTYPE grad_up, RHODTYPE grad_dn, RHODTYPE grad, POTDTYPE *exc_up, POTDTYPE *exc_dn,
  POTDTYPE *vxc1_up, POTDTYPE *vxc1_dn, POTDTYPE *vxc2_upup, POTDTYPE *vxc2_dndn,
  POTDTYPE *vxc2_updn, POTDTYPE *vxc2_dnup)
{
  const double third  = 1.0 / 3.0;
