...
void PCGSolver_Diel<T,T2>::setupPrecon()
{
    ...
    pb::GridFunc<T2>*  gf_work=new pb::GridFunc<T2>(*grid_[0],bc_[0],bc_[1],bc_[2]);
    ...
    for(short ln=1;ln<=max_nlevels_;ln++){
       ...
       const bool flag_coarsen=
        ...
        pb::GridFunc<T2>* gf_rcoarse=new pb::GridFunc<T2>(*coarse_grid,bc_[0],bc_[1],bc_[2]);        
        gf_rcoarse_.push_back(gf_rcoarse);
        pb::GridFunc<T2>* gf_newv=new pb::GridFunc<T2>(*coarse_grid,bc_[0],bc_[1],bc_[2]);        
...
void PCGSolver_Diel<T,T2>::preconSolve(pb::GridFunc<T2>& gf_v, const pb::GridFunc<T2>&  gf_f, const short level)
{
...
bool PCGSolver_Diel<T,T2>::solve(pb::GridFunc<T2>& gf_phi, pb::GridFunc<T2>& gf_rhs)
{
...
template <class T, typename T2>
// Left Preconditioned CG
bool PCGSolver_Diel<T,T2>::solve(pb::GridFunc<T2>& gf_phi, pb::GridFunc<T2>& gf_rhs, pb::GridFunc<T2>& gf_rhod, pb::GridFunc<T2>& gf_vks)
{
    if( !oper_.grid().active() )return 0.;
