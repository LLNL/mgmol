...
double MPdot(const int len, const float * __restrict__ xptr, const float * __restrict__ yptr)
{
   mpdot_tm.start();
...
void MPaxpy(const int len, double scal, const double* __restrict__ xptr, double* __restrict__ yptr)
{
    const int one = 1;
    daxpy(&len, &scal, xptr, &one, yptr, &one);
}

void Ttrsm(const char side, const char uplo, const char transa, 
...
void Tsyrk(const char uplo, const char trans, const int n, const int k, const double alpha,
				const double* const a, const int lda, const double beta, double* c, const int ldc)
{
   dsyrk_tm.start();
   dsyrk(&uplo, &trans, &n, &k, &alpha, a, &lda, &beta, c, &ldc);
   dsyrk_tm.stop();
}
void Tsyrk(const char uplo, const char trans, const int n, const int k, const float alpha,
				const float* const a, const int lda, const float beta, float* c, const int ldc)
{
   ssyrk_tm.start();
...
void MPsyrk(const char uplo, const char trans, const int n, const int k, const double alpha,
				const double* const a, const int lda, const double beta, double* c, const int ldc)
{
   dsyrk_tm.start();
   dsyrk(&uplo, &trans, &n, &k, &alpha, a, &lda, &beta, c, &ldc);
...
void MPsyrk(const char uplo, const char trans, const int n, const int k, const double alpha,
				const float* const a, const int lda, const double beta, float* c, const int ldc)
{
    mpsyrk_tm.start();
       ...
       if( uplo == 'U' || uplo == 'u')
       {
          for( int j=0; j<n; j++)
          {
             ...
             for(int l=0; l<k; l++)
             {
                ...
                /* get multiplier */
                double mult = (double)(alpha*colL[j]); // same as alpha * a[lda*l + j];
      ...
      else /* uplo = 'L' or 'l' */
      {
          for( int j=0; j<n; j++)
          {
             ...
             for(int l=0; l<k; l++)
             {
                ...
                /* get multiplier */
                double mult = (double)(alpha * colL[0]);//same as alpha * a[lda*l + j];
                MPaxpy(len, mult, colL, buff);
             }
             /* Update col j of upper part of matrix C. */
             /* Get pointer to beginning of column j in C. */
...
void Tgemm(const char transa, const char transb, 
...
void Tgemm(const char transa, const char transb, 
             const int m, const int n, const int k, 
             const float alpha, const float* const a, const int lda, 
...
void MPgemm(const char transa, const char transb, 
             const int m, const int n, const int k, 
             const double alpha, const double* const a, const int lda, 
             const double* const b, const int ldb, 
             const double beta, double* const c, const int ldc)
...
void MPgemmNN(const int m, const int n, const int k, 
              ...
              const double beta,        float*  const c, const int ldc)
{
   ...
   for(int i=0;i<nbf;i++)bf[i]=(float)b[i];
...
void MPgemmTN(const int m, const int n, const int k, 
              const double alpha, const float*  const a, const int lda, 
                                  const float*  const b, const int ldb, 
              const double beta,        double* const c, const int ldc)
{
   ...
   jag_sgemm(&transa, &transb, &m, &n, &k, &falpha, a, &lda, b, &ldb, &fbeta, 
             cf, &ldc);
   for(int i=0;i<ncf;i++)c[i]=(double)cf[i];
   delete[] cf; 
...
double MPdot(const int len, const T1 * __restrict__ xptr, const T2 * __restrict__ yptr)
{
   ttdot_tm.start();
...
template <typename T1, typename T2>
void MPaxpy(const int len, double scal, const T1* __restrict__ xptr, T2 * __restrict__ yptr)
{
   for(int k=0; k<len; k++)
   {
...
template <typename T1, typename T2>
void MPsyrk(const char uplo, const char trans, const int n, const int k, const double alpha,
				const T1* const a, const int lda, const double beta, T2* c, const int ldc)
{
    tttsyrk_tm.start();
...
void MPcpy(double* __restrict__ dest, const float* __restrict__ src, const int n)
{
    for(int i=0;i<n;i++)dest[i]=src[i];
}
void MPcpy(float* __restrict__ dest, const double* __restrict__ src, const int n)
{
    for(int i=0;i<n;i++)dest[i]=src[i];
}

template double MPdot<double, float>(const int len, const double * __restrict__ xptr, const float * __restrict__ yptr);
template double MPdot<float, double>(const int len, const float * __restrict__ xptr, const double * __restrict__ yptr);

template void MPaxpy<float, double>(const int len, const double scal, const float * __restrict__ xptr, double* __restrict__ yptr);
template void MPaxpy<float, float>(const int len, const double scal, const float * __restrict__ xptr, float* __restrict__ yptr);

template void MPsyrk<double, float>(const char uplo, const char trans, const int n, const int k, const double alpha,
				const double* const a, const int lda, const double beta, float* c, const int ldc);
template void MPsyrk<float, double>(const char uplo, const char trans, const int n, const int k, const double alpha,
				const float* const a, const int lda, const double beta, double* c, const int ldc);

template void MPgemm<double, float, double>(const char transa, const char transb, 
             const int m, const int n, const int k, 
             const double alpha, const double* const a, const int lda, 
             const float* const b, const int ldb, 
             const double beta, double* const c, const int ldc);
template void MPgemm<float, double, float>(const char transa, const char transb, 
             ...
             const double* const b, const int ldb, 
             const double beta, float* const c, const int ldc);             
template void MPgemm<double, double, float>(const char transa, const char transb, 
             const int m, const int n, const int k, 
             const double alpha, const double* const a, const int lda, 
             const double* const b, const int ldb, 
             const double beta, float* const c, const int ldc);    
template void MPgemm<float, float, double>(const char transa, const char transb, 
             ...
             const float* const b, const int ldb, 
             const double beta, double* const c, const int ldc);                       				
template void MPgemmNN<double, double, double>( 
             const int m, const int n, const int k, 
             const double alpha, const double* const a, const int lda, 
                                 const double* const b, const int ldb, 
             const double beta,        double* const c, const int ldc);                       				
template void MPgemmTN<double, double, double>( 
             const int m, const int n, const int k, 
             const double alpha, const double* const a, const int lda, 
                                 const double* const b, const int ldb, 
             const double beta,        double* const c, const int ldc);                       				
