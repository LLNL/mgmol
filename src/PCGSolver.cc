...
void PCGSolver<T,T2>::setupPrecon()
{
    ...
    pb::Lap<POISSONPRECONDTYPE>* myoper = LapFactory<POISSONPRECONDTYPE>::createLap(*grid_[0],lap_type_);
    pc_oper_.push_back(myoper);

    pb::GridFunc<POISSONPRECONDTYPE>*  gf_work=new pb::GridFunc<POISSONPRECONDTYPE>(*grid_[0],bc_[0],bc_[1],bc_[2]);
    ...
    for(short ln=1;ln<=max_nlevels_;ln++){
       const bool flag_coarsen=
          ( (!( mygrid->dim(0)&1 )) // cannot coarsen if mesh not divisible by 2
          &&(!( mygrid->dim(1)&1 ))
          &&(!( mygrid->dim(2)&1 )) 
        ...
        pb::Lap<POISSONPRECONDTYPE>* myoper = LapFactory<POISSONPRECONDTYPE>::createLap(*coarse_grid,1);
        pc_oper_.push_back(myoper);

        gf_work=new pb::GridFunc<POISSONPRECONDTYPE>(*coarse_grid,bc_[0],bc_[1],bc_[2]);        
        gf_work_.push_back(gf_work);
        
        pb::GridFunc<POISSONPRECONDTYPE>* gf_rcoarse=new pb::GridFunc<POISSONPRECONDTYPE>(*coarse_grid,bc_[0],bc_[1],bc_[2]);        
        gf_rcoarse_.push_back(gf_rcoarse);
        pb::GridFunc<POISSONPRECONDTYPE>* gf_newv=new pb::GridFunc<POISSONPRECONDTYPE>(*coarse_grid,bc_[0],bc_[1],bc_[2]);        
...
template <class T, typename T2>
void PCGSolver<T,T2>::preconSolve(pb::GridFunc<POISSONPRECONDTYPE>& gf_v, const pb::GridFunc<POISSONPRECONDTYPE>&  gf_f, const short level)
{
    ...
    if( bc_[0]!=1 || bc_[2]!=1 || bc_[2]!=1 )
        gf_v.trade_boundaries();
}
...
//   printf("Iters = %d. Residual NORM Info := %2.8e, %2.8e, %2.8e \n", k, init_rnorm, final_residual_,residual_reduction_ );   
...
bool PCGSolver<T, T2>::solve(T2* phi, T2* rhs, const char dis)
{
    ...
    pb::GridFunc<T2>  gf_phi(phi,  oper_.grid(), 
                     bc_[0], bc_[1], bc_[2], dis);
    pb::GridFunc<T2>  gf_work(rhs, oper_.grid(), 
                      bc_[0], bc_[1], bc_[2], dis);
