...
public:
    ...
    hid_t  open_dset(const char* const  datasetname)const
    {
        ...
        if( active_ )
        {
            ...
            if( dset_id<0 ){
                if( onpe0 )
                    (*MPIdata::sout)<<"HDFrestart::open_dset() --- Cannot open Dataset "<<datasetname<<std::endl;
            }
        }
        return dset_id;
    }
    ...
    herr_t dset_exists(const char* const  datasetname)const
    {
        ...
        if( active_ ){
            ...
            if( err_id<0 ){
                if( onpe0 )
                    (*MPIdata::sout)<<"HDFrestart::dset_exists() failed for dataset "<<datasetname<<std::endl;
            }
        }
#endif
        return err_id;
    ...
    void setVerbosity(const short verbosity)
    {
    ...
    hid_t createFilespace()const
    {
        ...
        if( filespace<0 && onpe0 )
            (*MPIdata::serr)<<"ERROR: createFilespace() failed!!!"<<std::endl;
        return filespace;
    }
    hid_t createMemspace()const
    {
        ...
        if( memspace<0 && onpe0 )
            (*MPIdata::serr)<<"ERROR: createMemspace() failed!!!"<<std::endl;
        return memspace;
    }
    
    int getLRCenters(std::multimap<std::string,Vector3D>&, const int,
                     const std::string& name);
    int getLRs(LocalizationRegions& lrs, 
               const int max_nb,const std::string& name,
               const bool add=false);
    int read_1func_hdf5(double*, std::string);
    int read_1func_hdf5(float*, std::string);    
    int write_1func_hdf5(double*, std::string, double* ll=NULL, double* origin=NULL);
    int write_1func_hdf5(float*, std::string, double* ll=NULL, double* origin=NULL);    
    int read_att(const hid_t dset_id, std::string attname, std::vector<double>& attr_data);

    int writeData(double *vv, hid_t filespace, hid_t memspace,
                  hid_t dset_id,
                  const short precision);
    int writeData(float *vv, hid_t filespace, hid_t memspace,
                  hid_t dset_id,
                  const short precision);
    int readData(double *vv, hid_t memspace,
                 hid_t dset_id,
                 const short precision);
    int readData(float *vv, hid_t memspace,
                 hid_t dset_id,
                 const short precision);                 
