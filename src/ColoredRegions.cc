...
void ColoredRegions::setup(LocalizationRegions& lrs,
                           const bool global )
{
    std::vector<int> gids;
    if( global )
...
int ColoredRegions::getLocCentersAndRadii4color(const short color,
...
int ColoredRegions::getLocGids4color(
    const short color, vector<int>& data)const
{
    data.clear();
    multimap<short,Sphere>::const_iterator p  =loc_regions_.find(color);
    if( p==loc_regions_.end() )return 0;
