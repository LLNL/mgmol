...
Timer  KBPsiMatrixSparse::computeHvnlMatrix_tm_("KBPsiMatrixSparse::computeHvnlMatrix");
...
KBPsiMatrixSparse::KBPsiMatrixSparse(pb::Lap<ORBDTYPE>* lapop, const bool need2radius):
    lapop_(lapop),
    need2radius_(need2radius)
{
    kbpsimat_ = 0;
    kbBpsimat_ = 0;
    distributor_= 0;
...
void KBPsiMatrixSparse::clearData()
{
    if(isDataSetup_)
    {
       delete kbpsimat_;  kbpsimat_=0;
       if( lapop_  ) 
       {
           delete kbBpsimat_;
       }
       kbBpsimat_=0;
       delete distributor_;distributor_=0;   
                              ...
                              const LocGridOrbitals& orbitals)
{
    ...
    assert(kbpsimat_ == 0);
    assert(kbBpsimat_ == 0);
    kbpsimat_ = new VariableSizeMatrix<sparserow>("KBpsi",count_proj_subdomain_);
    if( lapop_ )
        kbBpsimat_ = new VariableSizeMatrix<sparserow>("KBBpsi",count_proj_subdomain_);
    else
...
//    if( onpe0 )
//        (*MPIdata::sout)<<"KBPsiMatrixSparse using radius "<<spread_radius_<<endl;
...
void KBPsiMatrixSparse::computeKBpsi(Ions& ions, 
...
#pragma omp parallel for
    ...
    for(int color=0;color<nb_colors;color++)
    {
        ...
        if( gid!=-1 )
        {
            ...
            while( ion != iend )
            {
                computeLocalElement(**ion, gid, iloc, ppsi+color*ldsize, flag);
                ion++;
            }
        }
...
//    int ione=1;
    ...
    while( ion != iend )
    {
        ...
        for(short i=0;i<nprojs;i++)
        {
            ...
            // loop over states to multiply kbpsi_[st][gid] and kbBpsi_[st][gid] by coeff
                                          ...
                                          dist_matrix::SparseDistMatrix<DISTMATDTYPE>& hnlij)const
{
    ...
    for(short i=0;i<nprojs;i++)
    {
        ...
        // double loop over states to fill hnlij[st1][st2] (in general not symmetric... )
        ...
        for(int p1 = 0; p1 < nnzrow1; p1++)
        {
             ...
             if(fabs(kbpsi1) > tolKBpsi)
             {
                 ...
                 {
                      const int st2 =  (*kbpsi2->kbpsimat_).getColumnIndex(lrindex, p2);
                      const double alpha = kbpsi1*(*kbpsi2->kbpsimat_).getRowEntry(lrindex, p2);
                      /* set hnlij */
...
void KBPsiMatrixSparse::computeHvnlMatrix(const Ions& ions,
...
void KBPsiMatrixSparse::computeHvnlMatrix(const KBPsiMatrixInterface* const kbpsi2,
                                          const Ions& ions,
...
void KBPsiMatrixSparse::getPsiKBPsiSym(const Ions& ions, VariableSizeMatrix<sparserow>& sm)
{
    ...
    while(ion!=iend)
    {
        ...
        for(short i=0;i<nprojs;i++)
        {
            ...
            for(int p1 = 0; p1 < nnzrow1; p1++)
            {
                if(fabs((*kbpsimat_).getRowEntry(lrindex, p1)) <= tolKBpsi) continue;
                const int st1 =  (*kbpsimat_).getColumnIndex(lrindex, p1);
                for(int p2 = 0; p2 < nnzrow1; p2++)
                {
                    if(fabs((*kbpsimat_).getRowEntry(lrindex, p2)) <= tolKBpsi) continue;
                    const int st2 =  (*kbpsimat_).getColumnIndex(lrindex, p2);
                    const double alpha = coeff*(*kbpsimat_).getRowEntry(lrindex, p1)*(*kbpsimat_).getRowEntry(lrindex, p2);
                    /* set hnlij */
                    if(fabs(alpha) > tolKBpsi)
...
void KBPsiMatrixSparse::computeAll(Ions& ions, 
                                   LocGridOrbitals& orbitals)
{
...
#ifdef PRINT_OPERATIONS
        if( onpe0 )
            (*MPIdata::sout)<<"KBPsi coeff. up to date, KBPsiMatrixSparse::computeAll() skipped"<<endl;
#endif
        return;
    }
...
#ifdef PRINT_OPERATIONS
    if( onpe0 )
        (*MPIdata::sout)<<"KBPsiMatrixSparse::computeAll()"<<endl;
#endif
    reset();
    ...
    const int gid, const DISTMATDTYPE* const mat_X, const int numst)const
{
    ...
    {
        const int st1 =  (*kbpsimat_).getColumnIndex(lrindex, p1);
        const double t1 = (*kbpsimat_).getRowEntry(lrindex, p1); //getValIonState(gid,st1) ;
