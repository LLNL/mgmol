...
AOMMprojector::AOMMprojector(LocGridOrbitals& phi, LocalizationRegions& lrs)
{
    ...
    if( onpe0 )
        cout << "AOMM: setup with radius "<< radius << " and threshold "<<threshold<<endl;
...
{
    if( onpe0 )
        cout << "AOMM: reset projectors..."<<endl;
...
AOMMprojector::~AOMMprojector()
{
    ...
    delete kernelprojector_; kernelprojector_=0;
    delete kernel_phi_; kernel_phi_=0;
    delete kernel_proj_matrices_; kernel_proj_matrices_=0;
    delete kernelMasks_; kernelMasks_=0;
    delete matrix_mask_; matrix_mask_=0;
}
