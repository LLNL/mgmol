...
void PBh8<T>::pb_8th(GridFunc<T>& A, GridFunc<T>& B)
{
	...
	del1_8th(PB<T>::epsilon_, PB<T>::work2_, i);
	del1_8th(A, PB<T>::work1_, i);
	B.substract_prod(PB<T>::work1_, PB<T>::work2_);
    }

    del2_8th(A,PB<T>::work1_);
    B.add_prod(PB<T>::work1_, PB<T>::epsilon_);
}
template <class T>
void PBh8<T>::jacobi(GridFunc<T>& A, 
	    const GridFunc<T>& B, 
	    GridFunc<T>& W) 
{
    apply(A,W);
    ...
    const double  c0 = 1.5*(1435./504.)*(PB<T>::inv_h2(0)+PB<T>::inv_h2(1)+PB<T>::inv_h2(2));
...
template <class T>
void PBh8<T>::get_vepsilon(GridFunc<T>& vh, GridFunc<T> &rho, GridFunc<T>& vepsilon)
{
    PB<T>::work2_=0.;
    for(int i=0;i<3;i++){
	del1_8th(vh, PB<T>::work1_, i);
