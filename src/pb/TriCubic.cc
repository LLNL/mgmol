...
template <class T>
TriCubic<T>::TriCubic(const Grid& grid, const short bc[3]):
    grid_(grid)
{
    for(short i=0;i<3;i++){
        h_[i]=grid_.hgrid(i);
...
void TriCubic<T>::computeSplineCoeffs(const T* const f)
{
    ...
    computeSplineCoeffs(gf_f,*gf_df[0],  *gf_df[1],  *gf_df[2],
                             *gf_d2f[0], *gf_d2f[2], *gf_d2f[1],
                             gf_d3f);
                                   ...
                                   const GridFunc<T>& fxyz)
{
    ...
    for(int k=0;k<dim[2];k++)
    {
        ...
        tricubic_get_coeff(&spline_coeff_[index][0],
                           fval,
                           dfdxval,   dfdyval,   dfdzval,
                           d2fdxdyval,d2fdxdzval,d2fdydzval,
                           d3fdxdydzval);
    }
}
template <class T>
void TriCubic<T>::getValues(const vector<double>& vr,
                         vector<double>& val, MPI_Comm comm)
{
    ...
    for(int ir=0;ir<n;ir++)
    {
        ...
        const double origin[3]={grid_.origin(0),grid_.origin(1),grid_.origin(2)};
        ...
        const double rlocal[3]={rincell[0]-grid_.start(0),
                                rincell[1]-grid_.start(1),
                                rincell[2]-grid_.start(2)};
            ...
            (int)sz<dim[2] )
        {
            ...
            val[ir]=0.;
            cout<<"WARNING: TriCubic::getValues(), position outside domain..."<<endl;
        }
    }
//#ifdef USE_MPI
//    vector<double> tmp(val);
//    //cout<<"TriCubic::getValues(), MPI_Allreduce, tmp.size()="<<tmp.size()<<endl;
//    MPI_Allreduce(&tmp[0], &val[0], n, MPI_DOUBLE, MPI_SUM, comm);
//#endif
    return;
}
template <class T>
void TriCubic<T>::getGradient(const double r[3], double dfdr[3], MPI_Comm comm)
{
    ...
    {
        const int index=(int)sx*incx+(int)sy*incy+(int)sz;
        dfdr[0]=tricubic_eval(&spline_coeff_[index][0], x, y, z, 1,0,0)
                /h_[0];
        dfdr[1]=tricubic_eval(&spline_coeff_[index][0], x, y, z, 0,1,0)
                /h_[1];
        dfdr[2]=tricubic_eval(&spline_coeff_[index][0], x, y, z, 0,0,1)
                /h_[2];
    }
    else
    {
        for(short i=0;i<3;i++)
            dfdr[i]=0.;
        cout<<"WARNING: TriCubic::getGradient(), position outside domain..."<<endl;
    }
//#ifdef USE_MPI
//    double tmp[3]={0.,0.,0.};
