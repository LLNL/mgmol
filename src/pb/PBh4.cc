...
template <class T>
void PBh4<T>::jacobi(GridFunc<T>& A, 
	    const GridFunc<T>& B, 
	    GridFunc<T>& W) 
{
    apply(A,W);
    ...
    const double  c0 = 1.5*(30./12.)*(PB<T>::inv_h2(0)+PB<T>::inv_h2(1)+PB<T>::inv_h2(2));

    if(!PB<T>::epsilon_.updated_boundaries()) PB<T>::epsilon_.trade_boundaries();
...
template <class T>
void PBh4<T>::get_vepsilon(GridFunc<T>& vh, GridFunc<T> &rho, GridFunc<T>& vepsilon)
{
    PB<T>::work2_=0.;
    for(int i=0;i<3;i++){
