...
public:    
    ...
    explicit GridFunc(const T* const, const Grid&, 
    ...
    int inc(const short dir)const
    {
       return grid_.inc(dir);
    }
    ...
    void set_updated_boundaries(const bool i){
        updated_boundaries_=i;
    }
    ...
    void jacobi(const GridFunc<T>& v, const GridFunc<T>& epsilon, const double c0);
    ...
    virtual void trade_boundaries(){
        defaultTrade_boundaries();
    }
    void set_bc_func(GridFunc<T>* bc_func)
    {
        bc_func_=bc_func;
    }
    ...
    /* gdot is split into two for consistency between float-double and double-float
     * calls to gdot. See function definition for more details.
    */
    ...
    void getCellCornersValues(const int i, const int j, const int k,
                              double val[8])const;
