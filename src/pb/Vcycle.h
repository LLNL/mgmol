...
int
Vcycle(T1 &A, T2 &x, const GridFunc<T3> &rhs, const short cogr,
       const short nu1, const short nu2,
       const bool gather_coarse_level)
{
    ...
    norm_tmp=norm(x);
    if(level_grid.mype_env().onpe0())
        cout<<"Vcycle: x="<<norm_tmp<<endl;
#endif

    const bool flag_coarsen=
          ( (!( level_grid.dim(0)&1 )) // cannot coarsen if mesh not divisible by 2
          &&(!( level_grid.dim(1)&1 ))
          &&(!( level_grid.dim(2)&1 )) 
    ...
    const bool flag_gather = (
        ( !flag_coarsen || (level_grid.level()<=(-cogr)) )
        && gather_coarse_level );
    ...
    if( flag_gather && level_grid.mype_env().n_mpi_tasks()>1 ){
        ...
        GridFunc<T3>  replicated_rhs(replicated_func,replicated_grid,
                                 x.bc(0),x.bc(1),x.bc(2));
        
        GridFunc<T3>  replicated_x(replicated_grid,
                               x.bc(0),x.bc(1),x.bc(2));
    ...
    for(short i=0;i<nu1;i++){
        ...
        norm_tmp=norm(res);
        if(level_grid.mype_env().onpe0())
	    cout<<"pre-smoothing level "<<level_grid.level()<<", residual="<<norm_tmp<<endl;
#endif	
    }
    ...
    if( level_grid.level()>(-cogr) ){
        if( flag_coarsen ){
            ...
            norm_tmp=norm(res);
            if(level_grid.mype_env().onpe0())
	        cout<<"level "<<level_grid.level()<<", residual="<<norm_tmp<<endl;
#endif	    
...
#if USE_LOWER_ORDER
            Vcycle(B.getLowerOrderOp(),ucoarse,rcoarse,cogr,nu1,nu2, gather_coarse_level);
#else
	    ...
	    level_grid.mype_env().barrier();
            if(level_grid.mype_env().onpe0())
	        cout<<" Coarsest level: mesh "<<x.dim(0)<<"x"<<x.dim(1)<<"x"<<x.dim(2)
                    <<" reached\n";
#endif
        }
    }   
    ...
    for(short i=0;i<nu2;i++){
        ...
        norm_tmp=norm(res);
        if(level_grid.mype_env().onpe0())
	    cout<<"post-smoothing level "<<level_grid.level()<<", residual="<<norm_tmp<<endl;
#endif	
