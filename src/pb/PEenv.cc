...
PEenv::PEenv(MPI_Comm comm, ostream* os): // default constructor 
    comm_(comm),
    os_(os)
{
    ...
    os_(os)
{
    ...
    int periods[3]={1,1,1};
    int reorder = 0;
    MPI_Cart_create (comm_active_, 3, n_mpi_tasks_dir_, periods, reorder, &cart_comm_);
...
void PEenv::set_other_tasks_dir()
{
    ...
    MPI_Allgather(&mytask_dir_[0], 3, MPI_INT,
                          buffer, 3, MPI_INT, comm());
...
PEenv::~PEenv()
{
    ...
    if( comm_active_!=comm_ && comm_active_!=MPI_COMM_SELF )
    {
        ...
        int mpirc=MPI_Comm_free(&comm_active_);comm_active_=0;
...
void PEenv::task2xyz()
{
    if(color_==0){
        ...
        if(mytask_dir_[0] >= n_mpi_tasks_dir_[0]) mytask_dir_[0] -= n_mpi_tasks_dir_[0];
        if(mytask_dir_[0] >= n_mpi_tasks_dir_[0]) mytask_dir_[0] -= n_mpi_tasks_dir_[0];
#endif
        assert(my_mpi(0)>=0);
        assert(my_mpi(1)>=0);
        ...
        //cout<<" task "<<mytask_<<" -> ("<<my_mpi(0)<<","<<my_mpi(1)<<","<<my_mpi(2)<<")\n";
    }
} 
...
void PEenv::setup_my_neighbors()
{
    ...
    if(color_==0){
#if 1
        MPI_Cart_shift(cart_comm_, 0, 1, &mpi_neighbors_[WEST],  &mpi_neighbors_[EAST]);
        MPI_Cart_shift(cart_comm_, 1, 1, &mpi_neighbors_[SOUTH], &mpi_neighbors_[NORTH]);
        MPI_Cart_shift(cart_comm_, 2, 1, &mpi_neighbors_[DOWN],  &mpi_neighbors_[UP]);
#else
        ...
        for(int i=0;i<6;i++){
            if(mpi_neighbors(i)>=n_mpi_tasks()){
                cout<<"mpi_neighbors("<<i<<")="<<mpi_neighbors(i)<<endl;
...
int PEenv::geom(const int nx, const int ny, const int nz, const int bias)
{
    ...
    if( onpe0_ && os_!=0 )
        (*os_)<<"Factorization of mesh dimensions and number of cpus:"<<endl;
    for(int i=0;i<4;i++){
        const int ni=n[i];
    ...
    for(int i=0;i<3;i++){
        if(fac[i][0]>1){
            ...
            if( onpe0_ && os_!=0 ){
                (*os_)<<"Direction "<<i<<": ";
                (*os_)<<"Poisson Solver Requires grid size to be divisible by 4!!!"
                    <<endl;
            }
            return 0;
        }
    ...
    // reset n[]
    n[0]=nx; n[1]=ny; n[2]=nz, n[3]=n_mpi_tasks_;
#ifndef NDEBUG
    if( onpe0_ && os_!=0 )
        (*os_)<<"n="<<n[0]<<", "<<n[1]<<", "<<n[2]<<", "<<n[3]<<endl;
#endif
...
#ifndef NDEBUG
    if( onpe0_ && os_!=0 ){
        (*os_)<<" Subdomain: "<<n[0]<<" * "<<n[1]<<" * "<<n[2]<<endl;
        (*os_)<<" MPI: "<<n_mpi_tasks_dir_[0]<<" * "
                      <<n_mpi_tasks_dir_[1]<<" * "
                      <<n_mpi_tasks_dir_[2]<<endl;
    }
#endif
...
void PEenv::split_comm(const int nx, const int ny, const int nz, const int bias)
{
    ...
    while( flag )
    {
        ...
        if(color_==0)
        {
            ...
            if(nmpi<n_mpi_tasks_ && nmpi>0)// reduces communicator size by 1
            {
