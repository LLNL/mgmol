...
public:
    ...
    ShiftedLaph4M(const Grid& mygrid, const double shift):Lap<T>(mygrid)
    {
        ...
        if(lambda2_<0.)
        {
            std::cout<<"Warning! ShiftedLaph4M constructor: shift should be >0"<<std::endl;
        }
        diagEl_=(4./3.)*(Lap<T>::inv_h2(0)+Lap<T>::inv_h2(1)+Lap<T>::inv_h2(2))+lambda2_;
    ...
    {
        setFDLowerOrderGrid(ShiftedLaph4M::minNumberGhosts());
    }

    ShiftedLaph4M& getLowerOrderOp()
    {
        return *this;
    }
