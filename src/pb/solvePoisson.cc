...
extern "C"
{
void solvepoisson(const int* pbtype,
                  const int* const nx, 
                  const int* const ny, 
                  const int* const nz,
                  const double* const lx, 
                  const double* const ly, 
                  const double* const lz,
                  const int* const bcx,
                  const int* const bcy,
                  const int* const bcz,
                  const double* rhs,
                  const double* rhoe,
                  const int* const niter,
                  double* phi,
                  double* phi_eps)
{
    ...
    if( *pbtype==1 && *bcx==1 && *bcy==1 && *bcz==1 ){
        cout<<"solvePoisson: periodic BC not implemented for dielectric model"<<endl;
        return;
    }
...
//    MPI_Comm_rank(MPI_COMM_WORLD, &mype);
    ...
    GridFunc<GFDTYPE> *gf_veps; // Additional potential in KS for solvation model
    ...
    if (myPEenv.color()==0){
        if( *pbtype ){
            ...
            gf_rhoe    =new GridFunc<GFDTYPE>(rhoe,*myGrid,1,1,1,'g',ldz);
        }else{
        ...
        GridFunc<GFDTYPE> gf_phi(phi,*myGrid, *bcx, *bcy, *bcz,'g',ldz);   // Coulomb potential
        GridFunc<GFDTYPE> gf_rhs(rhs,*myGrid,1,1,1,'g',ldz);   // r.h.s

        double nv=norm(gf_phi);
        if(myPEenv.onpe0())
            cout<<" Norm of gf_phi="<<nv<<endl;
        nv=norm(gf_rhs);
        if(myPEenv.onpe0())
            cout<<" Norm of gf_rhs="<<nv<<endl;
        ...
        if( *pbtype ){
            if (myPEenv.onpe0())
                cout<<"PB Solver"<<endl;
            (*gf_veps)=0.;
            res=solverPB->solve(gf_phi, gf_rhs, *gf_rhoe, *gf_veps, *niter);
            gf_veps->init_vect(phi_eps,'g',ldz);
        }else{
            if (myPEenv.onpe0())
                cout<<"Laplacian Solver"<<endl;
    ...
    if (npes>myPEenv.n_mpi_tasks())
    {
        ...
        if (myPEenv.mytask()<nn && myPEenv.color()==0){
            ...
            MPI_Send(phi,size,MPI_DOUBLE,
                     npes-1-myPEenv.mytask(), 0,
                     MPI_COMM_WORLD);
            MPI_Send(phi_eps,size,MPI_DOUBLE,
                     npes-1-myPEenv.mytask(), 1,
                     MPI_COMM_WORLD);
        }else if(myPEenv.color()!=0){
            MPI_Status status;
            //cout<<"Receive data from "<<npes-1-myPEenv.mytask()
            //    <<" to "<<myPEenv.mytask()<<endl;
            MPI_Recv(phi,size,MPI_DOUBLE,myPEenv.mytask(),
                0,  MPI_COMM_WORLD, &status);
            MPI_Recv(phi_eps,size,MPI_DOUBLE,myPEenv.mytask(),
