...
public:
    ...
    DielFunc(Grid& my_grid, const T emax, 
             const T r0, const T dr0):GridFunc<T>(my_grid,1,1,1)
    {
        epsilon_max_=emax;
	rho0_       =r0;
	drho0_      =dr0;
    ...
    // Constructor
    DielFunc(const T* const vv, const Grid& new_grid, 
             const short tag=0):GridFunc<T>(vv,new_grid,1,1,1,tag)
    {
	epsilon_max_=e0def_;
	rho0_       =rho0def_;
	drho0_      =drho0def_;
    ...
    double epsilon_rho(const T, const T, 
                       const T, const T);
    double epsilon_rho(const T rho);
    double depsilon_rho(const T, const T, 
                        const T, const T);
