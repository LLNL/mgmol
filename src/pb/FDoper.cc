...
template <class T>
void FDoper<T>::del1_2nd(GridFunc<T>& A, GridFunc<T>& B, 
...
template <class T>
void FDoper<T>::del1_4th(GridFunc<T>& A, GridFunc<T>& B, 
                      const short direction)const
{
...
template <class T>
void FDoper<T>::del1_6th(GridFunc<T>& A, GridFunc<T>& B, 
                      const short direction)const
{
    assert(direction<3);
    assert(direction>=0);
...
template <class T>
void FDoper<T>::del1_8th(GridFunc<T>& A, GridFunc<T>& B, 
                      const short direction)const
{
    if( !grid_.active() )return;
...
template <class T>
void FDoper<T>::del2_4th_withPot(GridFunc<T>& A, const double* const pot, T* B)const
{
    if( !grid_.active() )return;
...
void FDoper<T>::del2_8th(GridFunc<T>& A, GridFunc<T>& B)const
{
    ...
    const double c0 = -2.*(c1x+c2x+c3x+c4x
                          +c1y+c2y+c3y+c4y
                          +c1z+c2z+c3z+c4z);
    ...
    for(int ix = 0;ix < dim0;ix++) {
        ...
        for(int iy = 0;iy < dim1;iy++) {
            ...
            for(int iz = 0;iz < dim2;iz++) {
                ...
                u0[iz] 
                    =  (T)(c0mehr4_*(double)v0[iz] 
    
                     + czmehr4_ * (double)(v0[iz-1]+v0[iz+1])
                     + cymehr4_ * (double)(vmy[iz]+vpy[iz])
                     + cxmehr4_ * (double)(vmx[iz]+vpx[iz])

                     + cxzmehr4_ * (double)(vmx[iz-1] +
                                         vmx[iz+1] +
                                         vpx[iz-1] +
                                         vpx[iz+1])
                     + cyzmehr4_ * (double)(vmy[iz-1] +
                                         vmy[iz+1] +
                                         vpy[iz-1] +
                                         vpy[iz+1])
                     + cxymehr4_ * (double)(vmxmy[iz] +
                                         vpxmy[iz] +
                                         vmxpy[iz] +
                                         vpxpy[iz]));
            ...
            for(int iz = 0;iz < dim2;iz++) {
                ...
                u[iiz] 
                    =  (T)(c0*(double)v[iiz] 
                     ...
                     + c1 * (double)(v[iiz-incx_]+v[iiz+incx_]
