...
Timer  GridFuncVectorInterface::trade_bc_colors_tm_("GridFuncVector::trade_bc_colors");
Timer  GridFuncVectorInterface::prod_tm_("GridFuncVector::prod");
Timer  GridFuncVectorInterface::finishExchangeNorthSouth_tm_("GridFuncVector::finishExNorthSouth");
Timer  GridFuncVectorInterface::finishExchangeUpDown_tm_("GridFuncVector::finishExUpDown");
Timer  GridFuncVectorInterface::finishExchangeEastWest_tm_("GridFuncVector::finishExEastWest");
...
template <typename T>
void GridFuncVector<T>::initiateNorthSouthComm(const int begin_color, const int end_color)
{    
    ...
    if( north_ )
        grid_.mype_env().Irecv(&comm_buf4[0], sizebuffer, NORTH, &req_north_south_[3]);
    if( south_ )
        grid_.mype_env().Irecv(&comm_buf3[0], sizebuffer, SOUTH, &req_north_south_[1]);
    ...
    if(south_)
    {
        ...
        *buf2_ptr=(T)ncolors;
        ...
        for(int color=begin_color;color<end_color;color++)
        {
            for(short iloc=0;iloc<nsubdivx_;iloc++)
            {
                ...
                *buf2_ptr=(T)gid_[iloc][color];
                ...
                for (int i=imin+iloc*iinc; i<imin+(iloc+1)*iinc; i+=incx_ )
                {
        ...
        grid_.mype_env().Isend(&comm_buf2[0], 1+ncolors*south_north_size_, SOUTH, &req_north_south_[2]);
    }
    ...
    if(north_){
        ...
        *buf1_ptr=(T)ncolors;
        ...
        grid_.mype_env().Isend(&comm_buf1[0], 1+ncolors*south_north_size_, NORTH, &req_north_south_[0]);
    }
}
template <typename T>
void GridFuncVector<T>::finishNorthSouthComm()
{
            ...
            for(short iloc=0;iloc<nsubdivx_;iloc++)
            {
                ...
                if( gid>=0 )
                {
                    ...
                    if( ilid!=gid2lid_.end() )
                    {
                        ...
                        {
                            T* uus=functions_[lid]->uu(nghosts_*(incy_+1)+ymax);
                            for (int j=0;j<jmax;j+=incy_)
                            for (int i=imin+iloc*iinc; i<imin+(iloc+1)*iinc; i+=incx_ )
                            {
    ...
    { // grid_.mype_env().n_mpi_task(1)==1
        ...
        for(int k=0;k<nfunc_;k++)
        {
            ...
            for (int i=nghosts_*incx_; i<(dimx_+nghosts_)*incx_; i+=incx_ )
            {
                memcpy(&pu[i+nghosts_+j],               &pu[i+nghosts_+j+ymax],     sdimz);
                memcpy(&pu[i+nghosts_*(incy_+1)+j+ymax], &pu[i+nghosts_*(incy_+1)+j], sdimz);
...
template <typename T>
void GridFuncVector<T>::initiateUpDownComm(const int begin_color, const int end_color)
{
    ...
    if(down_)
    {
        grid_.mype_env().Irecv(&comm_buf3[0], sizebuffer, DOWN, &req_up_down_[1]);
    }
    if(up_)
    {
    ...
    if(up_)
    {
        ...
        *buf1_ptr=(T)ncolors;
        ...
        for(int color=begin_color;color<end_color;color++)
        {
            for(short iloc=0;iloc<nsubdivx_;iloc++)
            {
                const T* const uus=functions_[color]->uu(nghosts_+incy_*iinit);
                *buf1_ptr=(T)gid_[iloc][color];
                buf1_ptr++;
                for(int j=0;j<nghosts_;j++)
                {
                    Tcopy(&incxy_, &uus[zmax-1-j+iloc*incxy_*incy_], &incy_, buf1_ptr, &ione);
                    buf1_ptr+=incxy_;
                }
            }
        }
        grid_.mype_env().Isend(&comm_buf1[0], 1+ncolors*up_down_size_, UP, &req_up_down_[0]);
    }
    if(down_)
    {
        T* buf2_ptr=&comm_buf2[0];
        ...
        for(int color=begin_color;color<end_color;color++)
        {
            for(short iloc=0;iloc<nsubdivx_;iloc++)
            {
                ...
                *buf2_ptr=(T)gid_[iloc][color];
                ...
                for(int j=0;j<nghosts_;j++)
                {
                    Tcopy(&incxy_, &uus[j+iloc*incxy_*incy_], &incy_, buf2_ptr, &ione);
                    buf2_ptr+=incxy_;
                }
            }
        }
        grid_.mype_env().Isend(&comm_buf2[0], 1+ncolors*up_down_size_, DOWN, &req_up_down_[2]);
    }
}
template <typename T>
void GridFuncVector<T>::finishUpDownComm()
{
                ...
                if( gid>=0 )
                {
                    ...
                    if( ilid!=gid2lid_.end() )
                    {
                        ...
                        if( gid==gid_[iloc][lid] )
                        {
                            for(int j=0;j<nghosts_;j++)
                            {
                                T* const uus=functions_[lid]->uu(nghosts_-1-j);
                                Tcopy(&incxy_, buf3_ptr, &ione, &uus[incy_*iinit+iloc*incxy_*incy_], &incy_);
                                buf3_ptr+=incxy_;
                            }
                        }
                        else
                ...
                if( gid>=0 )
                {
                    ...
                    if( ilid!=gid2lid_.end() )
                    {
                        ...
                        if( gid==gid_[iloc][lid] )
                        {
                            for(int j=0;j<nghosts_;j++)
                            {
                                T* const uus=functions_[lid]->uu(nghosts_+zmax+j);
                                Tcopy(&incxy_, buf4_ptr, &ione, &uus[incy_*iinit+iloc*incxy_*incy_], &incy_);
                                buf4_ptr+=incxy_;
                            }
                        }
                        else
    ...
    { 
        ...
        for(int k=0;k<nfunc_;k++)
        {
            ...
            for(int j=0;j<nghosts_;j++)
            {
                Tcopy(&dimxy_, &pu[nghosts_-j+iinit*incy_+zmax-1], &incy_, 
                              &pu[nghosts_-j+iinit*incy_-1],      &incy_);
            }
            for(int j=0;j<nghosts_;j++)
            {
...
template <typename T>
void GridFuncVector<T>::initiateEastWestComm(const int begin_color, const int end_color)
{
    ...
    /* Non-blocking MPI */
    if( east_ )
        grid_.mype_env().Irecv(&comm_buf3[0], sizebuffer, EAST, &req_east_west_[3]);
    if( west_ )
        grid_.mype_env().Irecv(&comm_buf4[0], sizebuffer, WEST, &req_east_west_[2]);
    if( west_ )
    {
        T* buf1_ptr=&comm_buf1[0];
        //first element will tell how many functions (data) are in buffer
        *buf1_ptr=(T)ncolors;
        ...
        grid_.mype_env().Isend(&comm_buf1[0], sizebuffer, WEST, &req_east_west_[1]);
    }
    if( east_ )
    {
        T* buf2_ptr=&comm_buf2[0];
        ...
        *buf2_ptr=(T)ncolors;
        ...
        grid_.mype_env().Isend(&comm_buf2[0], sizebuffer, EAST, &req_east_west_[0]);
    }
}        
template <typename T>
void GridFuncVector<T>::finishEastWestComm()
{
    ...
    { 
        ...
        for(int k=0;k<nfunc_;k++)
        {
            ...
            memcpy(&pu[east_west_size_+xmax],&pu[east_west_size_],east_west_size_data);
        }
    }
...
void GridFuncVector<T>::trade_boundaries()
{
    ...
    for(int k=0;k<nfunc_;k++)
    {
        assert( functions_[k]->updated_boundaries() == functions_[0]->updated_boundaries() );
...
template <typename T>
void GridFuncVector<T>::communicateRemoteGids(const int begin_color, const int end_color)
{
    const int ncolors=end_color-begin_color;
    ...
    for(short i=0;i<6;i++)remote_gids_[i].resize(ndata);
    
    if( north_ )
        grid_.mype_env().Irecv(&remote_gids_[NORTH][0], ndata, NORTH, &gid_req[NORTH]);
    if( south_ )
        grid_.mype_env().Irecv(&remote_gids_[SOUTH][0], ndata, SOUTH, &gid_req[SOUTH]);
    if( east_ )
        grid_.mype_env().Irecv(&remote_gids_[EAST][0],  ndata, EAST,  &gid_req[EAST]);
    if( west_ )
        grid_.mype_env().Irecv(&remote_gids_[WEST][0],  ndata, WEST,  &gid_req[WEST]);
    if( up_ )
        grid_.mype_env().Irecv(&remote_gids_[UP][0],    ndata, UP,    &gid_req[UP]);
    if( down_ )
        grid_.mype_env().Irecv(&remote_gids_[DOWN][0],  ndata, DOWN,  &gid_req[DOWN]);
    ...
    if( up_ )
        grid_.mype_env().Isend(&local_gids[0], ndata, UP,    &gid_req[UP]);
    if( down_ )
        grid_.mype_env().Isend(&local_gids[0], ndata, DOWN,  &gid_req[DOWN]);

    if(south_)
        MPI_Wait(gid_req+SOUTH, MPI_STATUS_IGNORE);
    if(north_)
        MPI_Wait(gid_req+NORTH, MPI_STATUS_IGNORE);
    if(west_)
        MPI_Wait(gid_req+WEST, MPI_STATUS_IGNORE);
    if(east_)
        MPI_Wait(gid_req+EAST, MPI_STATUS_IGNORE);
    if(down_)
        MPI_Wait(gid_req+DOWN, MPI_STATUS_IGNORE);
    if(up_)
        MPI_Wait(gid_req+UP, MPI_STATUS_IGNORE);
}

// assumes all processors call with same arguments
template <typename T>
void GridFuncVector<T>::trade_boundaries_colors(const short first_color, const short last_color)
{
    ...
    grid_.mype_env().barrier(); 
    if(onpe0)cout<<"Color "<<first_color<<" to "<<last_color-1<<endl;
    ...
    for(short color=first_color+1;color<end_color;color++)
        assert( functions_[color]->updated_boundaries() == functions_[first_color]->updated_boundaries() );
    ...
    if(grid_.mype_env().n_mpi_task(1)>1){
        ...
        if(south_){
            ...
            *buf2_ptr=(T)ncolors;
            ...
            for(short color=first_color;color<last_color;color++){
                for(short iloc=0;iloc<nsubdivx_;iloc++){
                    const short rgid=remote_gids_[SOUTH][nsubdivx_*color+iloc];
        ...
        if(north_){
            ...
            *buf1_ptr=(T)ncolors;
            ...
            for(short color=first_color;color<last_color;color++){
                for(short iloc=0;iloc<nsubdivx_;iloc++){
                    const short rgid=remote_gids_[NORTH][nsubdivx_*color+iloc];
                    const short k=gid2lid_[rgid];
...
#endif
    if(bc_[1]==1){ // grid_.mype_env().n_mpi_task(1)==1 
    ...
    if(grid_.mype_env().n_mpi_task(2)>1){
        ...
        if(up_){
            ...
            *buf1_ptr=(T)ncolors;
            ...
            for(short color=first_color;color<last_color;color++){
                for(short iloc=0;iloc<nsubdivx_;iloc++){
                    const short rgid=remote_gids_[UP][nsubdivx_*color+iloc];
                    const short k=gid2lid_[rgid];
                    const T* const uus=functions_[k]->uu(nghosts_+incy_*iinit);
                    *buf1_ptr=(T)gid_[iloc][k];
                    buf1_ptr++;
                    for(int j=0;j<nghosts_;j++){
                        Tcopy(&incxy_, &uus[zmax-1-j+iloc*incxy_*incy_], &incy_, buf1_ptr, &ione);
        ...
        if(down_){
            ...
            *buf2_ptr=(T)ncolors;
            ...
            for(short color=first_color;color<last_color;color++){
                for(short iloc=0;iloc<nsubdivx_;iloc++){
                    const short rgid=remote_gids_[DOWN][nsubdivx_*color+iloc];
                    const short k=gid2lid_[rgid];
                    const T* const uus=functions_[k]->uu(nghosts_+incy_*iinit);
                    *buf2_ptr=(T)gid_[iloc][k];
                    buf2_ptr++;
    ...
    if(grid_.mype_env().n_mpi_task(0)>1)
    {
        ...
        if( west_ )
        {
            ...
            *buf1_ptr=(T)ncolors;
            ...
            for(short color=first_color;color<last_color;color++)
            {
                const short rgid=remote_gids_[WEST][nsubdivx_*color+nsubdivx_-1];
                if( rgid>=0 )
                {
                    const short k=gid2lid_[rgid];
                    *buf1_ptr=(T)gid_[0][k];
...
#endif
    if(bc_[0]==1){ /* grid_.mype_env().n_mpi_task(0)==1 */
