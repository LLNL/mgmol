...
public:
    ...
    PBh4M(const Grid& mygrid, const double e0, const double rho0, const double drho0):
        PB<T>(mygrid, e0, rho0, drho0),
        pp_(PB<T>::grid_,1,1,1), sqrt_a_(PB<T>::grid_,1,1,1), inv_sqrt_a_(PB<T>::grid_,1,1,1)
    {
    ...
    PBh4M(const Grid& mygrid, DielFunc<T>& myepsilon, GridFunc<T>& pp):
          PB<T>(mygrid, myepsilon),pp_(pp,PB<T>::grid_),
          sqrt_a_(mygrid,1,1,1),inv_sqrt_a_(mygrid,1,1,1)
    { 
    ...
    void transform(GridFunc<T> &A)const
    {
        A*=inv_sqrt_a_;
    }

    void inv_transform(GridFunc<T> &A)const
    {
        A/=inv_sqrt_a_;
    }
