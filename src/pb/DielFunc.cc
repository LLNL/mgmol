...
template <class T>
double DielFunc<T>::epsilon_rho(const T rho, const T  e0, 
...
double DielFunc<T>::epsilon_rho(const T rho)
{
    ...
    return  (1.+0.5*(epsilon_max_-1.)*((1.-ratio)/(1.+ratio)+1.));
}		    
template <class T>
double DielFunc<T>::depsilon_rho(const T rho, const T  e0, 
                    const T rho0, const T drho0)
{
    ...
    return  (2.*inv_rho0)*(1.-e0)
            *drho0*pow(ratio,2.*drho0-1.)
	    /((1.+pratio)*(1.+pratio));
}		    
template <class T>
double DielFunc<T>::depsilon_rho(const T rho)
{
    ...
    return  (2.*inv_rho0)*(1.-epsilon_max_)
            *drho0_*pow(ratio,2.*drho0_-1.)
	    /((1.+pratio)*(1.+pratio));
}		    
template <class T>
...
//    GridFunc<T>::tag_ = rho.tag();
	...
	cout<<" Need a density to build dielectric function..."
	    <<endl;
	exit(0);
    }
...
//    GridFunc<T>::tag_ = rho.tag();
	...
	for(int ix = 0;ix < dim0;ix++) {
		...
		for(int iz =0;iz < dim2;iz++) {
		    ...
		    GridFunc<T>::uu_[iz1]=epsilon_rho(rho_data[iz2],epsilon_max_, rho0, drho0);
...
//    GridFunc<T>::tag_ = rho.tag();
	...
	for(int ix = 0;ix < dim0;ix++) {
		...
		for(int iz =0;iz < dim2;iz++) {
		    ...
		    de[iz1]=depsilon_rho(rho_data[iz2],epsilon_max_,rho0,drho0);
