...
PBh4M<T>::PBh4M(const Grid& mygrid, DielFunc<T>& myepsilon):
      PB<T>(mygrid, myepsilon),pp_(PB<T>::grid_,1,1,1),
...
template <class T>
PBh4M<T>::PBh4M(const PBh4M& oper):PB<T>(oper), pp_(oper.pp_,PB<T>::grid_),
                  ...
                  inv_sqrt_a_(oper.inv_sqrt_a_,PB<T>::grid_)
{
    ...
    //cout<<"Copy constructor for PBh4M: pp_ has "<<pp_.grid().sizeg()<<" points"<<flush<<endl;
...
template <class T>
void PBh4M<T>::jacobi(GridFunc<T>& A, 
	    const GridFunc<T>& B, 
	    GridFunc<T>& W) 
{
    ...
    const double  c0 = (30./12.)*(PB<T>::inv_h2(0)+PB<T>::inv_h2(1)+PB<T>::inv_h2(2));
...
template <class T>
void PBh4M<T>::get_vepsilon(GridFunc<T>& vh, GridFunc<T> &rhod, GridFunc<T>& vepsilon)
{
