...
double gaussian(const double r, const double rc)
{
    ...
    return exp(-(r*r)/(2.*rc*rc))/(rc*rc*rc*2.*M_PI*sqrt(2.*M_PI));
}
...
double comp_potential(const double r, const double rc)
{
    ...
    if(r>0.00001)return (erf(2.*r/rc)-erf(r/rc))/(M_PI*4.*r);
    else return 1./(2.*rc*sqrt(M_PI)*M_PI);
}
...
double potential_of_gaussian(const double r, const double rc)
{
    ...
    if(r>0.00001)return erf(r/rc)/r;
    else return 2./(rc*sqrt(M_PI));
}
...
double rho2(const double r, const double a)
{
   ...
   ((-1 + e0)*((2*(a - r))/exp(ar2) + 
        (2*(1 + ar2 + 0.*pow(a - r,4))*
           (-a + r))/exp(ar2))*
      ((2*(-2 + exp(3*r2))*r)/
         exp(4*r2) + sqrt(M_PI)*(erf(2.*r) - erf(r))))/
    r2;
