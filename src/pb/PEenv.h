...
public:
    ...
    explicit PEenv(MPI_Comm comm, const int nx, const int ny, const int nz, const int bias=4, std::ostream* os=0); 
    //explicit PEenv(MPI_Comm comm, const int mytask,
    //               const int taskx, const int tasky, const int taskz, ostream* os=0);
    ...
    int xyz2task(const int x, const int y, const int z) const
    {
        ...
        return rank;
#else
        return  ((x+n_mpi_task(0))%n_mpi_task(0))*n_mpi_task(1)*n_mpi_task(2)
    ...
    {
        MPI_Isend(buf, sizeb, MPI_DOUBLE, mpi_neighbors(dst), 0, cart_comm_, req);
    }
    void Isend(float* buf, int sizeb, const short dst, MPI_Request* req)const
    {
        MPI_Isend(buf, sizeb, MPI_FLOAT, mpi_neighbors(dst), 0, cart_comm_, req);
    }    
    ...
    {
        MPI_Irecv(buf, sizeb, MPI_DOUBLE, mpi_neighbors(src), 0, cart_comm_, req);
    }
    void Irecv(float* buf, int sizeb, const short src, MPI_Request* req)const
    {
        MPI_Irecv(buf, sizeb, MPI_FLOAT, mpi_neighbors(src), 0, cart_comm_, req);
    }
    ...
    void maxXdir(int* values, const int count)const
    {
        ...
        for(int i=0;i<count;i++)sendbuf[i]=values[i];
        int mpierr=MPI_Allreduce(sendbuf, &values[0], count, MPI_INT, MPI_MAX, comm_x_);
    ...
    void maxYdir(int* values, const int count)const
    {
        int* sendbuf=new int[count];
        for(int i=0;i<count;i++)sendbuf[i]=values[i];
        int mpierr=MPI_Allreduce(sendbuf, &values[0], count, MPI_INT, MPI_MAX, comm_y_);
        if( mpierr!=MPI_SUCCESS )
        {
    ...
    void maxZdir(int* values, const int count)const
    {
        int* sendbuf=new int[count];
        for(int i=0;i<count;i++)sendbuf[i]=values[i];
        int mpierr=MPI_Allreduce(sendbuf, &values[0], count, MPI_INT, MPI_MAX, comm_z_);
        if( mpierr!=MPI_SUCCESS )
        {
