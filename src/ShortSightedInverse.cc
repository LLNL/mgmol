...
Timer   ShortSightedInverse::Gram_Matrix_data_distribution_tm_("ShortSightedInverse::GramMat_data_distribution");
...
Timer   ShortSightedInverse::compute_invS_tm_("ShortSightedInverse::Compute_Inverse");
Timer   ShortSightedInverse::gather_invS_tm_("ShortSightedInverse::gather_inverse");
...
Timer   ShortSightedInverse::linear_solver_matvec_tm_("ShortSightedInverse::linear_solver_matvec");
Timer   ShortSightedInverse::linear_solver_matrix_init_tm_("ShortSightedInverse::linear_solver_matrix_init");

//const double mat_tol = 1.0e-14;
ShortSightedInverse::ShortSightedInverse(LocalizationRegions& lrs, 
                                         const vector<int>& locvars,
                                         ClusterOrbitals *local_cluster): locvars_(locvars)
{
    ...
    //estimate of size parameter (4096) needed for efficient table element access
    ...
    resnorm_ = 0.0;       
    precon_ = 0;
    recompute_pc_ = true;  /* recompute the preconditioner or not - initially true */
}
...
#pragma omp for reduction(+:conv)   
    ...
    for(int i = 0; i<locfcns_.size(); i++)    
    {
        ...
        conv = solver.solve((*matLS_), (*precon_), lrindex, solptr, fgmres_tol_, im_, maxits_);
    ...
    if(conv==0)
    {
       ...
       if((onpe0) && (ct.verbose> 1))
           printf("GMRES converged in %d iterations. Max .Final res. norm = %2.8e \n", maxits, maxrnrm);
       return 0;           
    }
    ...
    {   
       delete precon_;precon_=0;
       recompute_pc_=true;
       if(onpe0) printf("WARNING: Recompute Preconditioner - GMRES did not converge after max. its =  %d iterations. Final res. norm = %2.8e \n", 
                        maxits_, maxrnrm);
       return 1;
    }
    else
    {
       if(onpe0) printf("WARNING: GMRES did not converge after max. its =  %d iterations. Final res. norm = %2.8e \n", 
                        maxits_, maxrnrm);
       return 0;
    }
}
...
void ShortSightedInverse::computeInvS(DataDistribution& distributor_S, DataDistribution& distributor_invS)
{
...
for(int i=0; i<LSMat.n(); i++)
{
   ...
   for(int k=LSMat.nzptrval(i); k<LSMat.nzptrval(i+1); k++)
   {
      if(i == LSMat.getColumnIndex(k))
          flag = true;
      fprintf(ft," %d %d %2.8e \n", i, LSMat.getColumnIndex(k), LSMat.getRowEntry(k));
   }
   if(flag == false)
      fprintf(ft," %d %d %2.8e \n", i, i, MAT_TOL);
}    
...
*/    
    ...
    while(solve_flag)
    {
        if(recompute_pc_ == true)
        {
          pc_setup_tm_.start();
          assert( precon_==0 );
//          precon_ = new PreconILU<pcdatatype>(LSMat,droptol_,MaxFil_,lof_);
          precon_ = new PreconILU<pcdatatype>((*matLS_),droptol_,MaxFil_,lof_);
          /* setup the preconditioner */
//          (*precon_).setup(LSMat, ilutype_);
          (*precon_).setup((*matLS_), ilutype_);
          if(onpe0 && (ct.verbose > 1))
              (*MPIdata::sout)<<scientific<<"ILU preconditioner fill_factor = "
                              << ((double)(*precon_).nnz_ilu())/((double)matLS_->nnz())<<endl;
          new_pc_=true;
          pc_setup_tm_.stop();
        }   
...
void ShortSightedInverse::initGramMatrix(const LocalMatrices<MATDTYPE>& ss, const vector<vector<int> >& global_indexes, const int new_orbitals_index)
{
    ...
    (*gramMat_).initializeMatrixElements(ss, global_indexes, ct.numst);// should lsize_ be an argument here??
...
/* perform the trace of the product of some diagonal matrix represented by the vector ddiag
 * with the diagonal of invS. It is assumed that ddiag contains the diagonal entries of the 
 * functions centered on the local processor.(ie. functions in locfcns_)
*/ 
...
void ShortSightedInverse::printGramMat(ostream& os, int nrows)const
{
  ...
  {
     if( onpe0 )
        os<<" Local GramMatrix ... n = "<<lsize_<<" augmented_size = "<<aug_size_<<endl;
  ...
  else
  {
     ...
     if( onpe0 )
        os<<" Local GramMatrix size ... n = "<<n<<endl;
...
double ShortSightedInverse::getTraceDotProductWithInvS(VariableSizeMatrix<sparserow> *mat)
{
   /* compute trace */
   double trace = 0.0;
   for(std::vector<int>::iterator itr = locfcns_.begin(); itr != locfcns_.end(); ++itr)
   {
      trace += (*invS_).AmultSymB_ij(mat, *itr, *itr, *((*gramMat_).getTable()));
   }
...
void ShortSightedInverse::augmentGramMatrix(DataDistribution& distributor_S, const bool append)
{
...
// This latter stage should only do row/col assignments (no inserts of individual entries)
...
void ShortSightedInverse::printGramMM(ofstream& tfile)
{
   ...
   int nzmat = 0;
   for(std::vector<int>::const_iterator it = locfcns_.begin(); it != locfcns_.end(); ++it)
   {
...
int ShortSightedInverse::GramMatLSSolve(const double *rhs, double *sol)
{
    ...
    const int m=matLS_->n();
     ...
     conv = solver.solve((*matLS_), (*precon_), rhs, sol, fgmres_tol_, im_, maxits_);
     its  = solver.iters();        
    ...
    {
       if((onpe0) && (ct.verbose > 3)) printf("WARNING: GMRES did not converge after max. its =  %d iterations. Final res. norm = %2.8e \n", 
                        maxits_, rnrm);
       return 0;
    }
}
