                        ...
                        const bool first_application)
{
    ...
    {
        GridMask::cutWithMask(u, level, iloc);
    }
}

template<typename T>
void GridMaskMax::applyPrivate(pb::GridFunc<T>& gu, const unsigned short level,
                        const unsigned short iloc)
{    
        ...
        for(int ix = 0;ix < subdimx;ix++)
        {
            ...
            for(int iy = 0;iy < dim1;iy++)
            {
                ...
                for(int iz = 0;iz < dim2;iz++)
                {
                    ...
                    ppu[iz] = (T)GridMask::limitAbsValue( ppu[iz], pmask[iz]);
                }
                iy1+=incy1;
                iy2+=incy2;
            }
...
template
void GridMaskMax::applyPrivate(pb::GridFunc<double>& gu, const unsigned short level,
                               const unsigned short iloc);
template
void GridMaskMax::applyPrivate(pb::GridFunc<float>& gu, const unsigned short level,
                               const unsigned short iloc);
template
void GridMaskMax::applyPrivate(float *u, const unsigned short level,
                        const unsigned short iloc,
                        const bool first_application);
template
void GridMaskMax::applyPrivate(double *u, const unsigned short level,
                        const unsigned short iloc,
                        const bool first_application);
