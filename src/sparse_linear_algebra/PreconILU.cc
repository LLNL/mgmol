...
template <class T>Timer  PreconILU<T>::pcilu_setup_tm_("PreconILU::setup");
template <class T>Timer  PreconILU<T>::pcilu_solve_tm_("PreconILU::solve");

template <class T>
PreconILU<T>::PreconILU (LinearSolverMatrix<lsdatatype>& csmat, const float droptol, const int maxfill, const int lof)
{
    n_ = csmat.n();
    if(n_ > 0)
    {
...
template <class T>
void PreconILU<T>::setup (LinearSolverMatrix<lsdatatype>& csmat_, const PCILUTYPE type_)
{
...
template <class T>
void PreconILU<T>::qsplitC (std::vector<T>& a, std::vector<int>& ind, const int n, const int Ncut)
{
/*----------------------------------------------------------------------
|     does a quick-sort split of a complex array.
|     on input a[0 : (n-1)] is a complex array
...
template <class T>
void PreconILU<T>::setParams (const float droptol, const int maxfill, const int lof)
{
    droptol_ = droptol;
    if(maxfill < 0)
       MaxFil_ = 10000;
...
/*-------------------- symbolic k,i,j Gaussian elimination  */ 
        ...
        for(jpiv = ustart, uptr = upstart; jpiv != uend; ++jpiv, ++uptr)
        {
           ...
           */
           ...
           {
              int pos = row - (*L_).getColumnIterator(0); /* get the position of row */
        ...
        if(lenl > 0)
           llvl.insert(llvl.end(), levL, (levL+lenl));
    }
    return 0;
}
template <class T>
...
/*-------------------- eliminate column */
       ...
       for(jpiv = ustart, udptr = udstart; jpiv != uend; ++jpiv, ++udptr)
       {
          ...
          *  smallest row index among iU[k], k = j, j+1, ..., lenu-1
          *-----------------------------------------------------------------------*/   
          std::vector<int>::iterator jmin = min_element(jpiv, uend);              
          /* now swap current column with smallest column index */
          if(jmin != jpiv)
...
{
/*
 * This is a modified ILUT routine based on the new column ILUT version ILUT3
 * 
...
*/
/*-------------------- The new MILUT in C -------------------------------------
 * COLUMN-BASED MILUT (MILUTC) preconditioner
 * Modified Incomplete LU factorization with dual truncation mechanism
...
/* ------ select diagonal modification using optimal spreading ----*/    
       ...
       if(tnorm)
       {
          ...
          T bf = 1.0;        
          T mu = (-1 + sqrt((tnorm)/(gamma*gamma - alpha*alpha)))/pow(fabs(dd + alpha),2.0);  
...
/*---- done with diagonal modification. Now update for lower part of column ----*/
...
{
/*
 * This is a standard ILUT routine based on the new column ILUT version ILUT3
 * 
 * 
*/
/*-------------------- The new MILUT in C -------------------------------------
