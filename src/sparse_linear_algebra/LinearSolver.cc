...
int LinearSolver::fgmres(const LinearSolverMatrix<lsdatatype>& LSMat, 
                         const PreconILU<pcdatatype>& precon, 
                         const int lrindex, double *sol, const double tol, const int im, const int maxits)
{
...
int LinearSolver::fgmres(const LinearSolverMatrix<lsdatatype>& LSMat, 
                         const PreconILU<pcdatatype>& precon, 
                         const double *rhs, double *sol, const double tol, const int im, const int maxits)
{
    int i, i1, ii, j, k, k1, its, im1, pti, pti1, ptih, one = 1;
    double *hh, *c, *s, *rs, t;
    double negt, beta, eps1, gam, *vv, *z;     
...
// Compute minimum eigenvalue of LSMat - Assumes SPD matrix (requires Rayleigh quotient to retrieve sign of eigenvalue)
double LinearSolver::computeEigMin(LinearSolverMatrix<lsdatatype>& LSMat, std::vector<double>& evec)
{
...
// Compute maximum eigenvalue of LSMat - Assumes SPD matrix (requires Rayleigh quotient to retrieve sign of eigenvalue)
double LinearSolver::computeEigMax(LinearSolverMatrix<lsdatatype>& LSMat, std::vector<double>& evec)
{
   int one = 1, n = LSMat.n(), maxits = 10;
...
/* vector norms and dot products are returned in double precision (used in the MG-S/ Arnoldi process).
 * vectors and matrices used to perform transformations on the Hessenberg matrix are stored in single precision.
 * Updates to the solution are stored in single precision, but the updates themselves are done in double precision.
 * Matrix and vector operations (linear algebra ops) on single precision data are 'computed' in double precision and 
 * the results are stored in single precision
*/
int LinearSolver::fgmres_mp(LinearSolverMatrix<lsdatatype>& LSMat, PreconILU<pcdatatype>& precon, const int lrindex, double *sol, const double tol, const int im, const int maxits)
{
...
+-------------------------------------------------------*/
          for (k=1; k<=i; k++) 
          {
	           ...
	           t = (double)hh[ptih+k1];
	           t = (double)hh[ptih+k1];
	           hh[ptih+k1] = (float)((double)c[k1]*t + (double)s[k1]*(double)hh[ptih+k]);
	           hh[ptih+k] = (float)((double)-s[k1]*t + (double)c[k1]*(double)hh[ptih+k]);
          }
              gam = sqrt(pow((double)hh[ptih+i],2) + pow((double)hh[ptih+i1],2) );
/*-------------------- check if gamma is zero */
              if (gam == 0.0) gam = epsmac;
/*-------------------- get  next plane rotation    */
...
/*-------------------- get residual norm + test convergence*/
              hh[ptih+i] = (float)((double)c[i]*(double)hh[ptih+i] + (double)s[i]*(double)hh[ptih+i1]);
