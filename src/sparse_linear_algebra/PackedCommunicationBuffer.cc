...
PackedCommunicationBuffer::PackedCommunicationBuffer(const int bsize)
{
   ...
   {
      if(onpe0 && ct.verbose>0)cout<<"PackedCommunicationBuffer: resize buffers to 2 x "<<bsize<<endl;
...
void PackedCommunicationBuffer::setupPackedDataPointers(const char *data)
{
   ...
   if(packed_num_rows_ != 0)
   {
      ...
      packed_data_size_ = packed_double_pos + packed_nnzrow_ptr_[packed_num_rows_]*sizeof(double);
   }
   else
   {
...
void PackedCommunicationBuffer::packLocalData(VariableSizeMatrix<sparserow>& lmat, const int *pos)
{
...
/* This code had to be modified to split existing row updates from new row inserts for the sake of threading. 
 * Inserting a new row requires using a critical section, which cannot be called within a branch. This modification lets only
 * master perform tasks related to inserting new rows. Current implementation avoids the need for a critical section, but requires 
 * master to loop over all rows and only insert what is needed.
 */
template <class T>
void PackedCommunicationBuffer::mergeRecvDataToMatrix(VariableSizeMatrix<T>& amat, const bool append)
{   
...
//Threading here leads to results slightly dependent on number of threads (jlf, 07/15/2016)
#ifdef _OPENMP
#pragma omp parallel for
     ...
     { 
        ...
        if(rindex != NULL)
        {
              ...
              /* insert columns */
              amat.updateLocalRowAdd(k, lrindex, &packed_pj_ptr_[packed_nnzrow_ptr_[i]], &packed_pa_ptr_[packed_nnzrow_ptr_[i]]); 
              row_size[i] = -1;
        } 
     }
     ...
     for(int i = 0; i < packed_num_rows_; i++)
     {
        if(row_size[i] != -1)
        {
           amat.insertNewRow(row_size[i], packed_lvars_ptr_[i], &packed_pj_ptr_[packed_nnzrow_ptr_[i]], &packed_pa_ptr_[packed_nnzrow_ptr_[i]], append);     
        }
     }   
   }
...
template <class T>
void PackedCommunicationBuffer::updateMatrixEntriesWithRecvBuf(VariableSizeMatrix<T>& amat)
{
...
template <class T>
void PackedCommunicationBuffer::insertRowsFromRecvBuf(VariableSizeMatrix<T>& amat, const bool append)
{
   ...
   for(int i = 0; i < packed_num_rows_; i++)
   {
      ...
      {
           /* Insert new row */
           amat.insertNewRow(ncols, packed_lvars_ptr_[i], &packed_pj_ptr_[packed_nnzrow_ptr_[i]], &packed_pa_ptr_[packed_nnzrow_ptr_[i]], append);              
      }            
   } 
   copy_and_insert_new_rows_tm_.stop();
}

template void PackedCommunicationBuffer::mergeRecvDataToMatrix(VariableSizeMatrix<SparseRow>& amat, const char* data, const bool append);
template void PackedCommunicationBuffer::mergeRecvDataToMatrix(VariableSizeMatrix<SparseRowAndTable>& amat, const char* data, const bool append);
template void PackedCommunicationBuffer::updateMatrixEntriesWithRecvBuf(VariableSizeMatrix<SparseRow>& amat, const char* data);
template void PackedCommunicationBuffer::updateMatrixEntriesWithRecvBuf(VariableSizeMatrix<SparseRowAndTable>& amat, const char* data);
template void PackedCommunicationBuffer::insertRowsFromRecvBuf(VariableSizeMatrix<SparseRow>& amat, const char* data, const bool append);
template void PackedCommunicationBuffer::insertRowsFromRecvBuf(VariableSizeMatrix<SparseRowAndTable>& amat, const char* data, const bool append);

template void PackedCommunicationBuffer::mergeRecvDataToMatrix(VariableSizeMatrix<SparseRow>& amat, const bool append);
template void PackedCommunicationBuffer::mergeRecvDataToMatrix(VariableSizeMatrix<SparseRowAndTable>& amat,  const bool append);
template void PackedCommunicationBuffer::updateMatrixEntriesWithRecvBuf(VariableSizeMatrix<SparseRow>& amat);
template void PackedCommunicationBuffer::updateMatrixEntriesWithRecvBuf(VariableSizeMatrix<SparseRowAndTable>& amat);
template void PackedCommunicationBuffer::insertRowsFromRecvBuf(VariableSizeMatrix<SparseRow>& amat,  const bool append);
template void PackedCommunicationBuffer::insertRowsFromRecvBuf(VariableSizeMatrix<SparseRowAndTable>& amat, const bool append);
