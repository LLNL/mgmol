...
Timer DataDistribution::distribute_local_row_tm_("DataDistribution::distributeLocalRows");
Timer DataDistribution::distribute_local_data_tm_("DataDistribution::distributeLocalData");
Timer DataDistribution::send_recv_tm_("DataDistribution::MPISendRecv");
Timer DataDistribution::send_recv_rows_tm_("DataDistribution::MPISendRecv_rows");
//Timer DataDistribution::update_row_tm_("DataDistribution::consolidateLocalRows");
Timer DataDistribution::augment_local_data_tm_("DataDistribution::augmentLocalData");
Timer DataDistribution::update_local_rows_tm_("DataDistribution::updateLocalRows");
Timer DataDistribution::initLocalRow_tm_("DataDistribution::initLocalRows");      
Timer DataDistribution::send_recv_ovlp_tm_("DataDistribution::MPISendRecv_ovlp");
Timer DataDistribution::send_recv_ovlp_wait_tm_("DataDistribution::MPISendRecv_ovlp_wait");
Timer DataDistribution::send_recv_rows_ovlp_tm_("DataDistribution::MPISendRecv_Rows_ovlp");
Timer DataDistribution::send_recv_rows_ovlp_wait_tm_("DataDistribution::MPISendRecv_Rows_ovlp_wait");
...
template <class T>
void DataDistribution::mergeDataFromNeighborToLocalData(VariableSizeMatrix<T>& amat, const char *rbuf, const bool append)
{
...
//   #pragma omp parallel for
  ...
  { 
     ...
     if(rindex != NULL)
     {
           ...
           /* insert columns */
           amat.updateLocalRowAdd(k, lrindex, &rbuf_pj_ptr_[rbuf_nnzrow_ptr_[i]], &rbuf_pa_ptr_[rbuf_nnzrow_ptr_[i]]);     
     }
     else
     {
          /* Insert new row */
          amat.insertNewRow(k, rbuf_lvars_ptr_[i], &rbuf_pj_ptr_[rbuf_nnzrow_ptr_[i]], &rbuf_pa_ptr_[rbuf_nnzrow_ptr_[i]], append);              
     }           
  }
}
...
//   double* const pa = rbuf_pa_ptr_;
   ...
   for(int i = 0; i < *rbuf_nrows_ptr_; i++)
   {
      ...
      {
         /* insert columns */
         amat.initializeLocalRow(ncols, *rindex, &rbuf_pj_ptr_[start], &rbuf_pa_ptr_[start]);
      }
      else
      {
           /* Insert new row */
           amat.insertNewRow(ncols, rbuf_lvars_ptr_[i], &rbuf_pj_ptr_[rbuf_nnzrow_ptr_[i]], &rbuf_pa_ptr_[rbuf_nnzrow_ptr_[i]], append);              
      }            
   } 
...
//template <class T>
void DataDistribution::packLocalData(VariableSizeMatrix<sparserow>& lmat, const int *pos, char *buf)
{
...
//#if 1
   ...
   if( remote_size>bsiz )
   {
       cout<<"ERROR: "<<name_<<", dir="<<dir<<", remote_size="<<remote_size<<", bsiz="<<bsiz<<endl;
   ...
   send_recv_tm_.start();
   int mpircv=MPI_Irecv(packed_buffer.recvBuffer(), bsiz, MPI_CHAR, source, 0, cart_comm_, &request[0]);
   if( mpircv!=MPI_SUCCESS )
   {
       cout<<"ERROR in MPI_Irecv, code="<<mpircv<<endl;
       MPI_Abort(cart_comm_,0);
   }
   int mpisnd=MPI_Irsend(packed_buffer.sendBuffer(), siz,  MPI_CHAR, dest,   0, cart_comm_, &request[1]);   
   ...
   // complete remaining data transfer steps -- overlap communication and work
   for(int step=1; step<nsteps; step++)
   {
      ...
      MPI_Irecv(packed_buffer.recvBuffer(), bsiz, MPI_CHAR, source, 0, cart_comm_, &request[0]);
      MPI_Irsend(packed_buffer.sendBuffer(), siz,  MPI_CHAR, dest,   0, cart_comm_, &request[1]);  
      send_recv_ovlp_tm_.stop();
      
      // merge previously recv'd data to local data -- now stored in send_buffer
      // uses previously set internal pointers to data to accomplish this
      // See PackedCommunicationBuffer class for details
...
/* check that buffer size is large enough */ 
 ...
 while(step < nsteps)
 {
   ...
   if((step == nsteps-1)&&(bcflag == true)) // this is the last step - apply boundary condition
   {
      packed_buffer.updateMatrixEntriesWithRecvBuf(amat, packed_buffer.recvBuffer()); 
   }
   else
   {
      packed_buffer.mergeRecvDataToMatrix(amat, packed_buffer.recvBuffer(), append); 
   }
  ...
  for(short dir = 0; dir < 3; dir++)
  {
     ...
     {    
        ...
        */ 
        ...
        loc_data_sz_[dir] = (&data_pos_[pos])[1] + mat.nnzmat()*sizeof(double);
...
/* check that buffer size is large enough */ 
 ...
 if(nsteps > 0)
 {
   ...
   for(int step=1; step<nsteps; step++)
   {
      ...
      // copy previously recv'd data to local data -- now stored in send_buffer
      // uses previously set internal pointers to data to accomplish this
      // See PackedCommunicationBuffer class for details
...
/* check that buffer size is large enough */ 
 ...
 while(step < nsteps)
 {
   ...
   packed_buffer.insertRowsFromRecvBuf(amat, packed_buffer.recvBuffer(), append);   
...
/* Augment the local matrix - data distribution */
template <class T>
void DataDistribution::updateLocalRows(VariableSizeMatrix<T>& vsmat, const bool append)
{
  /* Local variables */
  short  disp;
  lsize_ = vsmat.n();
...
//template <class T>
void DataDistribution::reduceDataSizes(const short dir, const VariableSizeMatrix<sparserow>& lmat, int *maxsize, int *nzmax)
{  
// use barriers to ensure processors begin at the same time before timing this routine
//  MGmol_MPI& mmpi = *(MGmol_MPI::instance());
//  mmpi.barrier();  
...
template void DataDistribution::augmentLocalData(VariableSizeMatrix<SparseRow>& vsmat, const bool append, const bool bcflag);
template void DataDistribution::augmentLocalData(VariableSizeMatrix<SparseRowAndTable>& vsmat, const bool append, const bool bcflag);
template void DataDistribution::updateLocalRows(VariableSizeMatrix<SparseRow>& vsmat, const bool append);
template void DataDistribution::updateLocalRows(VariableSizeMatrix<SparseRowAndTable>& vsmat, const bool append);
