...
typedef enum PCILUTYPE  {PCILUK, PCMILUT, PCILUT, PCILU0} PCILUTYPE;
...
class PreconILU{
  ...
  void qsplitC (std::vector<T>& a, std::vector<int>& ind, const int n, const int Ncut);  
...
//  int ilu02( LinearSolverMatrix<T2>& csmat);	     	   
  ...
  public:
    PreconILU (LinearSolverMatrix<lsdatatype>& csmat, const float droptol, const int maxfill, const int lof=level_of_fill);		// construct ILU struct    
    void setup ( LinearSolverMatrix<lsdatatype>& csmat, const PCILUTYPE type);		// perform ILU factorization
    int nnz_ilu ();		// number of nonzero entries in ILU precon
    void LUsolve(double* const y, double* const x)const; // triangular solve with L and U factors
    void LUsolve(float* const y, float* const x)const; // triangular solve with L and U factors
