 ...
 */  
 template <class T>
void LinearSolverMatrix<T>::insertMatrixElement(const int pos, const int row, const T val, const INSERTMODE mode)
{
 ...
 */  
template <class T> 
void LinearSolverMatrix<T>::insertMatrixEntry(const int pos, const int row, const T val)
{
    matrix_insert_tm_.start();
    assert(pos < (int)x_.size());
...
template <class T>
void LinearSolverMatrix<T>::initRow(const int len, const std::vector<int>& cols, const std::vector<T>& vals)
{
    /* begin */
    matrix_insert_tm_.start();
...
/* initialize a single row of the matrix  */
template <class T>
void LinearSolverMatrix<T>::initRow(const std::vector<int>& cols, const std::vector<T>& vals)
{
    /* begin */
    matrix_insert_tm_.start();
...
template <class T>
void LinearSolverMatrix<T>::initRow(const int len, const std::vector<int>::iterator cols_begin, const TvecIterator vals_begin)
{
    /* begin */
    matrix_insert_tm_.start();
    /* update nonzero column pointer array */
...
template <class T>
void LinearSolverMatrix<T>::initRowNNZPattern(const int len, const std::vector<int>& cols)
{
    /* begin */
    matrix_insert_tm_.start();
    /* update nonzero column pointer array */
...
/* initialize a single row of the matrix - Non-zero pattern only (iterator version) */
template <class T>
void LinearSolverMatrix<T>::initRowNNZPattern(const int len, const std::vector<int>::iterator begin)
{
    /* begin */
    matrix_insert_tm_.start();
    /* update nonzero column pointer array */
...
template <class T>
void LinearSolverMatrix<T>::init(const VariableSizeMatrix<sparserow>& vsmat, 
                                 const bool rescale)
{   
    assert(&vsmat != NULL);
    ...
    if(rescale)
    {
       ...
       for(int i=0; i<n; i++)
       {
           ...
           /* rescale row */
           dscal(&len, (double *)&scale_vec_[i], (double *)getPtrToData(p_[i]), &one); 
       }      
    }
    else
    {
...
template <class T>
void LinearSolverMatrix<T>::initSquareMat(const VariableSizeMatrix<sparserow>& vsmat, const bool rescale)
{   
    assert(&vsmat != NULL);
...
template <class T>
void LinearSolverMatrix<T>::matvec(const std::vector<T>& v, std::vector<T>& w )const
{
    matvec_tm_.start();
...
template <class T>
void LinearSolverMatrix<T>::Usolve(double* const x, const std::vector<T>& diag)const
{
...
template <class T>
void LinearSolverMatrix<T>::Usolve(float* const x, const std::vector<T>& diag)const
{
    assert((int)diag.size() == n_);
