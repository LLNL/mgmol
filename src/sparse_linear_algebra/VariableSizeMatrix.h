...
public:
    VariableSizeMatrix (const std::string name, const int alloc_size, const MPI_Comm comm=MPI_COMM_NULL);		// setup data structures
    VariableSizeMatrix (const VariableSizeMatrix& A, const bool copy_table=true);		// Copy constructor
    template <class T2>
    VariableSizeMatrix (const VariableSizeMatrix<T2>& A, const bool copy_table=true);		// Copy constructor
    
    VariableSizeMatrix<T>& operator=(const VariableSizeMatrix<T>& a);

/* initialize a local row of the local matrix */
    void updateLocalRowSquareMatrix(const int count, const int lrindex, const int* const cols, const double* const vals, const INSERTMODE mode); /* update current local row by considering only column indices for which there are rows in the local matrix. ie. ensure a square matrix is preserved */
    void insertNewRow(const int ncols, const int row, const int* cols, const double* vals, const bool append);				/* Augment current matrix by inserting a new row */
    ...
    void print(std::ostream& os, const std::vector<int>&locfcns,  int nrows=NUM_PRINT_ROWS )const;    
...
//    void printMatMM(ofstream& outfile);				/* print MM matrix */         
    ...
    double getTraceDiagProductWithMat(const std::vector<double>& ddiag); /* return sum_i ( ddiag[i]*Mat[i][i] ) */   
...
void updateTableValue(const int key)
{
    (*table_).insert(key);
}
...
void getRowEntries(const int lrindex,
...
void row_daxpy(const int lrindex, const int size, const double alpha, double * y)
{
...
void initializeLocalRow(const int ncols, const int lrindex, const int *cols, const double *vals)
{
   if(ncols)
   {
...
void updateLocalRow(const int count, const int lrindex, const int* const cols, const double* const vals, const INSERTMODE mode)
{   
   //updateRow_tm_.start();    
...
void updateLocalRowAdd(const int count, const int lrindex, const int* const cols, const double* const vals)
{   
    //updateRow_tm_.start();
...
void updateLocalRowInsert(const int count, const int lrindex, const int* const cols, const double* const vals)
{   
   //updateRow_tm_.start();    
 ...
 * is known.
*/
void updateLocalEntry(const int lrindex, const int pos, const double val, const INSERTMODE mode)
{
    /* begin */
    /* Add or insert entry */
...
void updateLocalEntryInsert(const int lrindex, const int pos, const double val)
{
    /* begin */
    /* Add or insert entry */
...
void insertMatrixElement(const int row, const int col, const double val, const INSERTMODE mode, const bool append)
{
#ifdef _OPENMP
...
double get_value(const int row, const int col)const
{
    ...
    int *rindex = (int *)getTableValue(row);
    if(rindex != NULL)
       value = data_[*rindex]->getColumnEntry(col);
...
void AmultSymBLocal(VariableSizeMatrix<T> *B, VariableSizeMatrix<T>& C, const std::vector<int>& locfcns, VariableSizeMatrix<SparseRowAndTable>& pattern, bool flag=true);

// matrix multiplication operations 
void AmultSymB(VariableSizeMatrix<T> *B, VariableSizeMatrix<T>& C, VariableSizeMatrix<SparseRowAndTable>& pattern,bool flag=true);

const std::vector<int>& lvars()const
{
   return lvars_;
}

// get reference to local row at index rindex
T& getRow(int rindex) const
{
   return *data_[rindex];
}
...
void gemv(const double alpha, const std::vector<double>& x, const double beta, std::vector<double>& y);
