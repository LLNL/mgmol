 ...
 *
 * @param tsize  The number of entries stored in the table. NOTE: tsize = 0 is OK.
 * 
 */
 ...
 * of the table. 
 * NOTE: Does NOT check if the key already exists. It will reinsert key if it already exists.
 * 
...
int Table::insert(const vector<int>& keys)
{
   ...
   //reallocate storage if needed   
   const int diff = keys.size() > (capacity_-size_) ? (capacity_-size_) : 0;
...
//  mmpi.allreduce(&maxsize, 1, MPI_MAX);
  if(onpe0 && (maxsize>space_ || maxlink_>9 ) )
      cout<<"maxlink="<<maxlink_
          <<", maxsize="<<maxsize
          <<", space="<<space_
          <<", init_size_="<<init_size_<<endl;
#endif  
