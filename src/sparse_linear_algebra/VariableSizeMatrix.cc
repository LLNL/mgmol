...
Timer  VariableSizeMatrixInterface::AmultSymBdiag_tm_("VariableSizeMatrix::AmultSymBdiag");
Timer  VariableSizeMatrixInterface::AmultSymB_ij_tm_("VariableSizeMatrix::AmultSymB_ij");
Timer  VariableSizeMatrixInterface::AmultSymBLocal_tm_("VariableSizeMatrix::AmultSymBLocal");
Timer  VariableSizeMatrixInterface::AmultSymB_tm_("VariableSizeMatrix::AmultSymB");
Timer  VariableSizeMatrixInterface::initialize_tm_("VariableSizeMatrix::Init_with_squareLocMat");
Timer  VariableSizeMatrixInterface::updateRow_tm_("VariableSizeMatrix::updateRows");
Timer  VariableSizeMatrixInterface::insertRow_tm_("VariableSizeMatrix::insertNewRow");
Timer  VariableSizeMatrixInterface::sort_col_tm_("VariableSizeMatrix::sortMatrixColumns");
/* 
...
///*
//int binarySearchI(const int * const a, const int key, const int start, const int end)
//{
...
//    int imid = start + ((end-start)>>1); //(int)floor(double((imax - imin)/2)); 
// 
...
//*/
template <class T>
VariableSizeMatrix<T>::VariableSizeMatrix (const string name, const int alloc_size, const MPI_Comm comm)
   : name_(name)
{
...
template <class T>
VariableSizeMatrix<T>& VariableSizeMatrix<T>::operator=(const VariableSizeMatrix<T>& src)
{
...
template <class T>
VariableSizeMatrix<T>::VariableSizeMatrix(const VariableSizeMatrix<T>& A, const bool copy_table):
     name_(A.name_+"_copy"),
     n_(A.n_),
     nzmax_(A.nzmax_),
     totnnz_(A.totnnz_),
     lvars_(A.lvars_)
{
     ...
     if(copy_table)
     {
        ...
        data_.reserve(n);
        int j=0;
        for(const_TvecIterator arow=A.data_.begin(); arow!=A.data_.end(); arow++)
        {
...
template <class T>
template <class T2>
VariableSizeMatrix<T>::VariableSizeMatrix(const VariableSizeMatrix<T2>& A, const bool copy_table)
{
  typedef typename std::vector<T2 *> ::const_iterator const_T2vecIterator;  
     ...
     if(copy_table)
     {
        ...
        data_.reserve(n);
        int j=0;
        for(int i=0; i<n; i++)//(const_T2vecIterator arow=A.data_.begin(); arow!=A.data_.end(); arow++)
        {
...
template <class T>
void VariableSizeMatrix<T>::copyData(const VariableSizeMatrix<T>& A, const int m)
{
   if(n_ != 0)
      reset();
 ...
 */
template <class T>
void VariableSizeMatrix<T>::insertNewRow(const int ncols, const int row, const int* cols, const double* vals, const bool append)
{
//    insertRow_tm_.start();
...
template <class T>
void VariableSizeMatrix<T>::initializeMatrixElements(const LocalMatrices<MATDTYPE>& ss,
    ...
    for (int icolor = 0; icolor < chromatic_number; icolor++){
         ...
         for (int jcolor = 0; jcolor < n; jcolor++) {
            ...
            for(short iloc=0;iloc<subdiv;iloc++){
                ...
                if(st1!=-1){
                    ...
                    if(st2!=-1){
                        ...
                        const double tmp=ssiloc[jcolor*chromatic_number+icolor];
            ...
            for(int i=0;i<=valindex;i++)
                insertMatrixElement(ist[2*i],ist[2*i+1],val[i], ADD, true);
        } // jcolor
    } // icolor
...
template <class T>
void VariableSizeMatrix<T>::print(ostream& os, const std::vector<int>& locfcns, int n)const
{
  ...
  {    
     found = false;
     for(std::vector<int>::const_iterator it = locfcns.begin(); it != locfcns.end(); ++it)
     {
     /* check if row is centered on local PE*/
        ...
        /* only one PE sends data */
           ...
           if(myid != 0)
             MPI_Send(buf, ncols, MPI_DOUBLE, 0, myid, comm);           
           break;
        }
     }
     if(myid == 0)
     {
        if(!found)
           MPI_Recv(buf, ncols, MPI_DOUBLE, MPI_ANY_SOURCE, MPI_ANY_TAG, comm, &status);
...
void VariableSizeMatrix<T>::printMatCSR(const char *fname)
{
     ...
     { 
         ...
         {
             fprintf(outfile,"%d %d %2.8e \n", lvars_[i], data_[i]->getColumnIndex(j), data_[i]->getEntryFromPosition(j));
         }
     }
...
/* Print 2x2 block*/
template <class T>
void VariableSizeMatrix<T>::printMatBlock2(const int gid0,const int gid1,ostream& os)
{
     for(int i = 0; i<n_; i++)
     { 
         if( lvars_[i]==gid0 || lvars_[i]==gid1)
         {
             ...
             for(int j=0; j<nnzrow_i; j++)
             {
                 ...
                 os<<"S("<<lvars_[i]<<","<<data_[i]->getColumnIndex(j)<<")="<<data_[i]->getEntryFromPosition(j)<<endl;
             }
         }
     }
}
...
*/
template <class T>
double VariableSizeMatrix<T>::AmultSymB_ij(VariableSizeMatrix<T> *B, const int row, const int col, Table& indexT)
{   
    double val = 0.0;
...
/*          
          ...
          if(pos != NULL)
             val += data_[lrindex]->getEntryFromPosition(*pos) * (*B).getRowEntry(lcindex, k);  
*/
          val += data_[lrindex]->getColumnEntry(jrow) * (*B).getRowEntry(lcindex, k);
       }
    }      
    else
    {
       ...
       for(int k = 0; k < nnzrow_lrindex; k++)
       {
          ...
          if(pos != -1)
          {
             val += data_[lrindex]->getEntryFromPosition(k) * (*B).getRowEntry(lcindex, pos);
          }
       }       
    }         
    AmultSymB_ij_tm_.stop();
...
double VariableSizeMatrix<T>::trace(const std::vector<int>& rows)     
{
   ...
   for(std::vector<int>::const_iterator i = rows.begin(); i!=rows.end(); ++i)
...
template <class T>
double VariableSizeMatrix<T>::getTraceDiagProductWithMat(const vector<double>& ddiag)
{
   double trace = 0.;
   const int n = n_;
   //#pragma omp parallel for reduction(+:trace)
...
template <class T>
void VariableSizeMatrix<T>::copyDataToArray(int *locvars, int *colidx, double *colvals)
{
   // copy lvars_
...
*/
template <class T>
void VariableSizeMatrix<T>::AmultSymBLocal(VariableSizeMatrix<T> *B, VariableSizeMatrix<T>& C, 
                                           const std::vector<int>& locfcns, 
                                           VariableSizeMatrix<SparseRowAndTable>& pattern, 
                                           bool flag)
{   
    assert(&C != 0);
    ...
    for(std::vector<int>::const_iterator row = locfcns.begin(); row != locfcns.end(); ++row)           
    {
        int *rindex = (int *)getTableValue(*row);
        const int i = *rindex;
...
//      #pragma omp parallel for            
        for(int j = 0; j < nb; j++)
        {
            ...
            for(int k = 0; k < nnzrow; k++)
            {
                ...
                const int *pos = (int *)pattern.getTableValue(jrow); // check if corresponding column entry exists.
                if(pos != NULL)
                    val += data_[i]->getEntryFromPosition(*pos) * (*B).getRowEntry(j, k);                 
...
*/
template <class T>
void VariableSizeMatrix<T>::AmultSymB(VariableSizeMatrix<T>* B, VariableSizeMatrix<T>& C, 
                                      VariableSizeMatrix<SparseRowAndTable>& pattern, 
                                      bool flag)
{   
    assert(&C != 0);
...
template <class T>
int VariableSizeMatrix<T>::getMaxAbsOffDiagonalRowEntry(const int gid, double& value)const
{
   double maxVal=0.;
   int maxj=-1;
...
template <class T>
void VariableSizeMatrix<T>::axpy(const double alpha, const VariableSizeMatrix<T>& B)
{
   assert(n_ == B.n());
   const int n = B.n();
...
template <class T>
void VariableSizeMatrix<T>::gemv(const double alpha, const std::vector<double>& x, const double beta, std::vector<double>& y)
{
