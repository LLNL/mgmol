...
class LinearSolver{
  ...
  double resnorm_; // fgmres residual norm
    ...
    int fgmres_mp(LinearSolverMatrix<lsdatatype>& LSMat, PreconILU<pcdatatype>& precon, const int lrindex, double *sol, const double tol, const int im, const int maxits);    
  ...
  public:
    ...
    double computeEigMin(LinearSolverMatrix<lsdatatype>& LSMat, std::vector<double>& evec); 
    // compute maximum eigenvalue of LSMat
    double computeEigMax(LinearSolverMatrix<lsdatatype>& LSMat, std::vector<double>& evec); 
...
//       int conv = fgmres_mp(LSMat, precon, lrindex, sol, tol, im, maxits);
       gmres_tm_.stop();
       return conv;
    }
    int solve(const LinearSolverMatrix<lsdatatype>& LSMat, 
