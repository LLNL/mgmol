...
#ifdef CRAY_T3E
double cbrt(double alpha){
    return pow(alpha, 0.3333333333333333333333333);
}
#endif
...
void LDAFunctional::xc_unpolarized(const RHODTYPE rh, POTDTYPE &ee, POTDTYPE &vv)
{
  ...
  if ( rh > 0.0 )
  {
    ...
    else
    {
      ...
      ec = G * inv_den;
      vc = ec * ( 1.0 + sevensixth * b1 * sqrtrs +
                  fourthird * b2 * rs ) * inv_den;
    }
    ee = (POTDTYPE)(ex + ec);
    vv = (POTDTYPE)(vx + vc);
  }else{
