...
void GrassmanCG::computeOrbitalsProdWithH(LocGridOrbitals& orbitals1, LocGridOrbitals& orbitals2, dist_matrix::DistMatrix<DISTMATDTYPE>& mat)
{
    ...
    mgmol_strategy_->computeHij(orbitals1, orbitals2, *ptr2ions_, &kbpsi_1, &kbpsi_2, mat);
...
// consolidate flag with either gather data or else return local partial contributions
void GrassmanCG::computeOrbitalsProdWithH(LocGridOrbitals& orbitals, dist_matrix::DistMatrix<DISTMATDTYPE>& mat)
{
    // initialize KBPsiMatrices
...
void GrassmanCG::parallelTransportUpdate(const double lambda, LocGridOrbitals& phi)
{
    Control& ct = *(Control::instance());  
...
//    const double fact = lambda;
    ...
    //Reset iterative index for current (transported) search direction and gradient
    //This is especially necessary for the search direction, in order to ensure correct 
    //computations with the Hamiltonian and wrt phi. 
