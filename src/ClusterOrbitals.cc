...
void ClusterOrbitals::packRegionsData()
{
   ...
   for(int i=0; i<count; i++)
   {
      ...
      double vals[3] = {(region_center)[0], (region_center)[1], (region_center)[2]};
...
{  
   ...
   avg_locfcns_global_ = (double)lrs_->globalNumLRs()/ (double)myPEenv.n_mpi_tasks();   
   ...
   comm_data_->initializeLocalRow(loc_size,subdom_id_pos_,&locfcns_[0],&vals[0]);
   // call updateCluster to communicate cluster info and initialize cluster_indexes
   updateCluster();
...
//   double domain[3] = {(*subdom_ll_)[0],(*subdom_ll_)[1],(*subdom_ll_)[2]};
//   const int max_steps[3] = {subdom_steps_, subdom_steps_, subdom_steps_};
//   DataDistribution regions_distributor("Init_regions_data",max_steps, myPEenv, domain);
//   regions_distributor.augmentLocalData(*comm_data_, true);
...
{  
   ...
   subdom_bias_ = subdom_bias_ + alpha_*cluster_range_radius_*cluster_range_radius_*(pow(ratio, 0.67) - 1);
}
...
void ClusterOrbitals::unpackAndSetSubdomData()
{
   ...
   for(std::map<int, Vector3D>::iterator it=subdoms_data_.begin(); it != subdoms_data_.end(); ++it)
   {
      subdoms_index_.push_back(it->first);
   }
   subdom_id_pos_ =  std::distance(subdoms_data_.begin(), subdoms_data_.find(subdom_id_));
}
...
void ClusterOrbitals::updateBiasData()
{
   ...
   // neighbors is fixed. each subdomain communicates their own uniquely computed bias,
   // hence we can use updateLocalRows.
...
double ClusterOrbitals::computeSquaredDistanceBetweenCenters(const Vector3D& center1, const Vector3D& center2)const
{
    Control& ct = *(Control::instance());
...
///*   
   ...
   {
      if(onpe0)vtkfile.open (ct.load_balancing_output_file.c_str(),std::ios::out);
      writeVTKHeader(myPEenv.n_mpi_task(0), myPEenv.n_mpi_task(1), myPEenv.n_mpi_task(2), vtkfile);
...
//*/
   ...
   while(iters_ < maxiters)
   {
      ...
      if(ct.verbose > 1)
      {
         ...
         mmpi.allreduce(&locmax,1,MPI_MAX);   
         if((onpe0))cout<<"iter = "<<iters_<<", global max cluster size = "<<locmax<<", conv = "<<conv<<endl;   
      }
...
//   mmpi.barrier();
   ...
   locmax = (int)cluster_indexes_.size();
   mmpi.allreduce(&locmax,1,MPI_MAX);   
   if(onpe0)cout<<"Final Stats: num. iters = "<<iters_<<", global max cluster = "<<locmax<<", conv = "<<conv<<endl;   
...
//   mmpi.barrier();
   ...
   const int max_steps[3] = {subdom_steps_, subdom_steps_, subdom_steps_};
//   distributor_ = new DataDistribution("computeClusters",cluster_range_radius_, myPEenv, domain);
   distributor_ = new DataDistribution("computeClusters",max_steps, myPEenv, domain);
...
void ClusterOrbitals::computeLocalRegionOwnership()
{
   ...
   for(std::map<int, Vector3D>::iterator region = regions_data_.begin(); region != regions_data_.end(); ++region)
   {
      // compute initial minimum distance.
      std::map<int, Vector3D>::iterator subdom = subdoms_data_.begin();
      double mindist = computeSquaredDistanceBetweenCenters(subdoms_data_[subdom_id_], region->second) - bias_data_[subdom_id_pos_];
      int subdom_index_pos = 0;
      int min_subdom_pos = subdom_id_pos_;
      // loop over remaining subdomains
      while(subdom != subdoms_data_.end())
      {
         double distance = computeSquaredDistanceBetweenCenters(subdom->second, region->second) - bias_data_[subdom_index_pos];
         if(distance < mindist)
         {
...
void ClusterOrbitals::writeVTKHeader(const int npx, const int npy, const int npz, std::ofstream& os)
{
   ...
   if(onpe0)
   {
     ...
     os << "CELLS "<<ncells<< " " << ncells*(points_per_cell+1) <<endl;
...
void ClusterOrbitals::writeVTKDataset(const std::string name, const pb::PEenv& myPEenv, std::ofstream& os)
{
