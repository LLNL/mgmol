...
void addTrack(long addr, long asize)
{
    ...
    if(total_size>high_water_mark)
    {
        ...
        if(onpe0)printf("MGmol allocation: new high_water_mark=%ld\n",high_water_mark);
    }
    
    if( asize>THRESHOLD4PRINT )  
    {
        if(onpe0)printf("MGmol nPos %d: addr=%ld, size=%ld, filename=%s, line=%d\n",
                        nPos,addr,asize,
                        MGmolMem::Stamp::filename_.c_str(),
...
bool removeTracking(long addr)
{
    ...
    if( !bFound )
        if(onpe0)printf("WARNING: Unable to find allocation for delete[%ld]\n",addr);
