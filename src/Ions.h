...
class Ions
{
    ...
    std::vector<Ion*> overlappingNL_ions_;  // with projectors overlapping local sub-domain
    std::vector<Ion*> overlappingVL_ions_;  // with local potential overlapping local sub-domain
    ...
    // compute boundary for box containing all atoms to be known on local processor
    // that is values for list_boundary_left_, list_boundary_right_
    void setupListIonsBoundaries(const double rmax);
    
    void augmentIonsData(const int nsteps, const int dir, const int disp, const int locSize, const int maxLocSize, std::vector<IonData>& data, int * offset);
    ...
    int lstep_[3];			// number of steps to the left for each dimension to gather data
    int rstep_[3];			// number of steps to the right for each dimension to gather data
    
    bool setup_;
    
    void gatherLockedData(std::vector<int>& locked_data, const int root)const;    
    void computeNumIons(void); 
    int readNatoms(const std::string input_file,
                   const bool cell_relative);
    int readNatoms(ifstream *tfile,
                   const bool cell_relative);
    int readAtomsFromXYZ(const std::string filename, 
                         const bool cell_relative);
    void setupContraintsData(std::vector<Ion*>&);
    void clearStepperData();
    void initStepperData();
    void computeMaxNumProjs();
    void augmentIonicData(DistributedIonicData&,
                          const int nsteps, const int dir, const int disp, 
                          const int locSize, const int maxLocSize, 
                          DistributedIonicData& data);

    void gatherNames(std::map<int, std::string>& names, const int root, const MPI_Comm comm)const;
    void gatherPositions(std::vector<double>& positions, const int root=0)const;
    void gatherForces(std::vector<double>& forces, const int root=0)const;    

    void gatherNames(vector<string>& names, const int root, const MPI_Comm comm)const;
    void gatherPositions(vector<double>& positions, const int root, const MPI_Comm comm)const;
    void gatherLockedNames(vector<string>& names, const int root, const MPI_Comm comm)const;
    void gatherIndexes(vector<int>& indexes, const int root, const MPI_Comm comm)const;
    void gatherNLprojIds(vector<int>& nlprojids, const int root, const MPI_Comm comm)const;
    void gatherVelocities(vector<double>& velocities, const int root, const MPI_Comm comm)const;
    void gatherAtomicNumbers(vector<int>& atnumbers, const int root, const MPI_Comm comm)const;
    void gatherRandStates(vector<unsigned short>& rstates, const int root, const MPI_Comm comm)const;
    void gatherForces(vector<double>& forces, const int root, const MPI_Comm comm)const;
...
public:
   ...
   ~Ions();
    ...
    const std::vector<Ion*>& overlappingNL_ions()const{ return overlappingNL_ions_; }
    const std::vector<Ion*>& overlappingVL_ions()const{ return overlappingVL_ions_; }
    ...
    void syncForces();
    void setVelocities(const std::vector<double>&, const std::vector<double>&, const double);
    void setTau0();
    ...
    //void syncNames(const int nions, std::vector<string>& local_names, std::vector<string>& names);
    double getMaxVlRadius()const
    {
        return max_Vl_radius_;
    }
    ...
    std::vector<double*>& getTaupInteractingIons()
    {
        return interacting_taup_;
    }
