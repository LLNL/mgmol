...
{
    for(int i=0;i<m_;i++)assert( xi_[i]!=0 );
    for(int i=0;i<m_;i++)assert( fi_[i]!=0 );
    
    for(int i=0;i<m_;i++)delete xi_[i];
    for(int i=0;i<m_;i++)delete fi_[i];
}
    ...
    if( mm_>0 )
    {
        ...
        while( flag )
        {
            ...
            while( mm_>1 )
            {
                ...
                dsyev(&jobz, &uplo, &mm_, &tmp_mat[0], &m_, w, work2, &lwork2, &info);
                ...
                if( det<min_det_mat )
                {
                    if( onpe0 )(*MPIdata::sout)<<"Det. Anderson matrix="<<det<<", set m="<<mm_<<endl;
                    mm_--;
                }else
                    break;
            }
            ...
            dpotrs(&uplo, &mm_, &ione, &tmp_mat[0], &m_, &theta_[0], &m_, &info);
            
            delete[] tmp_mem;
            if(info!=0)
            {
                (*MPIdata::sout)<<"AndersonMix, dpotrs: info="<<info<<endl;
                exit(0);
            }
            ...
            for(int j=0;j<mm_;j++)
            {
                if( theta_[j]> max_theta )
                {
                    if( mm_>1 )
                    {
                        mm_--;
                        if( onpe0 )
                            (*MPIdata::sout)<<"Warning: theta["<<j<<"]="<<theta_[j]
