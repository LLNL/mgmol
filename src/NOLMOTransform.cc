...
void NOLMOTransform::init_transform(const dist_matrix::DistMatrix<DISTMATDTYPE>& ls, 
                                    const bool reset_positions)
{
  if(bcr_->onpe0())
...
////////////////////////////////////////////////////////////////////////////////
void NOLMOTransform::setia(vector<int>& iiu)
{
  return;
}
////////////////////////////////////////////////////////////////////////////////
...
{  
  ...
  if(bcr_->onpe0())
  if ( delta > tol )
    (*MPIdata::sout) << " NOLMOTransform: decrease was " << setprecision(8) << delta
...
double NOLMOTransform::nolmo(int maxsweep, double tol)
{
  ...
  dist_matrix::DistMatrix<DISTMATDTYPE> grads("grad",*bcr_,nst_,nst_,nst_,bsize_);
  ...
  for(int k=0;k<m;k++){
    ra[k]=new
    dist_matrix::DistMatrix<DISTMATDTYPE>(names[k],*bcr_,nst_,nst_,nst_,bsize_);
  }
  ...
  for(int k=0;k<m;k++){
     // b <- b * a0**-1
     b_[k]->trsm(side, uplo, trans, diag, 
            1., *a_);
  }
  side='l';
  trans='t';
...
#endif
  for(int k=0;k<m;k++){
    ba[k]=new dist_matrix::DistMatrix<DISTMATDTYPE>(names[k],*bcr_,nst_,nst_,nst_,bsize_);
  }
  ...
  while( isweep < maxsweep && delta > tol )
  {
    ...
    oldtspread = tspread;
    tspread = getFunctionalValue(ara,aba,inv_alphan); 
    //(*MPIdata::sout) << " average spread (trial step):   " << sqrt(tspread/nst_) << endl;
  ...
  Ttrsm(side, uplo, trans, diag, nst_, nst_, 
        one, &lst_[0], nst_, &mat_[0], nst_);
...
double NOLMOTransform::nolmo_fixedCenters(const int maxsweep, const double tol)
{
  ...
  for(int k=0;k<m;k++)
  {
    ra[k]=new dist_matrix::DistMatrix<DISTMATDTYPE>(names[k],*bcr_,nst_,nst_,nst_,bsize_);
  }
  ...
  assert( tspread>=0. );
  if(bcr_->onpe0())
    (*MPIdata::sout) << " initial functional value with fixed centers: " << tspread<< endl;
  ...
  while( isweep < maxsweep && delta > tol )
  {
    ...
    assert( nst_>0 );
    if(bcr_->onpe0())
      (*MPIdata::sout) << " functional value with fixed centers: " << tspread << endl;
  ...
  {
    (*MPIdata::sout)<<setprecision(12);
    (*MPIdata::sout)<<isweep<<" sweeps in nolmo_fixedCenters"<<endl;
    (*MPIdata::sout) << " tspread:   " << tspread << endl;
    (*MPIdata::sout) << " delta: " << delta << endl;
  }
...
void NOLMOTransform::gatherTransformMat()
{
    ...
    if( a_->active() )
      a_->copyDataToVector(sendbuf);
    DISTMATDTYPE* recvbuf=&mat_[0];
    assert( recvbuf!=NULL );
    MGmol_MPI& mmpi = *(MGmol_MPI::instance());
    MPI_Comm comm=mmpi.commSameSpin();
    MPI_Allgatherv(&sendbuf[0],nst_*nloc,MPI_DOUBLE,
...
double NOLMOTransform::getFunctionalValueFixedCenters(const vector<vector<double> >& ara,
                                          const vector<vector<double> >& aba,
                                          const vector<double>& inv_alphan)
{
  ...
  for ( int i = 0; i < n; i++ )
  {
    ...
    for(int k=0;k<2*NDIM;k++)
    {
      ...
      spread2 += ((aba[k][i]*inv_alphan[i])+vtmp0*vtmp0-2.*vtmp0*vtmp);
    }
...
void NOLMOTransform::getNorms(vector<double>& alphan,
                              vector<double>& inv_alphan)
{
