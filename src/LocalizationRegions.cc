...
Timer  LocalizationRegions::setupLocalRegionsFromOverlapRegions_tm_("LocalizationRegions::setupLocRegFromOvlpRegs");
...
void LocalizationRegions::extrapolateCentersLinear(const bool force, const bool move_centers)
{
    Control& ct = *(Control::instance());
    if ( onpe0 && ct.verbose>1 )
        (*MPIdata::sout)<<"LocalizationRegions::extrapolateCentersLinear()..."<<endl;
                               ...
                               ++it)
    {
        ...
        {
            *it_old[old_centers_.size()-1-i] = *it_old[old_centers_.size()-2-i];
        }   
        *it_old[1]=it->center;
    ...
    if( num_extrapolations_ < old_centers_.size() )
        num_extrapolations_++;
...
void LocalizationRegions::extrapolateCentersVerlet(const bool force, const bool move_centers)
{
    Control& ct = *(Control::instance());
    if ( onpe0 && ct.verbose>1 )
        (*MPIdata::sout)<<"LocalizationRegions::extrapolateCentersVerlet()..."<<endl;
                               ...
                               ++it)
    {
        ...
        *it_old[2]=*it_old[1];
        // save extrapolated center into old1 (to be used for next extrapolation)
        *it_old[1]=it->center;
...
void LocalizationRegions::extrapolateCentersQuadratic()
{
    Control& ct = *(Control::instance());
    if ( onpe0 && ct.verbose>1 )
        (*MPIdata::sout)<<"LocalizationRegions::extrapolateCentersQuadratic()..."<<endl;
...
float LocalizationRegions::updateRadii(const SpreadsAndCenters& sc,
                                       const float ratio)
{
    for(vector<LRData>::iterator it =overlap_regions_.begin();
                                 it!=overlap_regions_.end();
...
float LocalizationRegions::updateRadiiConstVol(const SpreadsAndCenters& sc)
{
    ...
    if ( onpe0 )
        (*MPIdata::sout)<<"Ratio Rc/spread: "<<ratio<<endl;
...
float LocalizationRegions::updateRadii(const OrbitalsTransform* ot,
                                        const float ratio)
{
    for(vector<LRData>::iterator it =overlap_regions_.begin();
                                 it!=overlap_regions_.end();
...
float LocalizationRegions::moveTo(const vector<Vector3D>& target_centers, const vector<Vector3D>& shifts)
{
    ...
    while(it!=overlap_regions_.end())
    {
        ...
        Vector3D delta=target_centers[i].vminimage(region_center, cell_, ct.bcPoisson);        
...
void LocalizationRegions::bcastLRs()
{
    ...
    {
        cerr<<"ERROR!!!! LocalizationRegions::bcast(), Failure in MPI_Bcast of 'nglobal_'!!!"<<endl;
        MPI_Abort(comm, 0);
    }
    ...
    // Bcast centers
    {
        ...
        retbcast=MPI_Bcast(&buffer_centers[0], 3*nglobal_, MPI_DOUBLE, 0, comm);
        if( retbcast!=MPI_SUCCESS )
        {
            cerr<<"ERROR!!!! LocalizationRegions::bcast(), Failure in MPI_Bcast of 'centers'!!!"<<endl;
            MPI_Abort(comm, 0);
        }
        vector<LRData>::iterator it=all_regions_.begin();
        int i=0;
    ...
    // Bcast radius
    {
        ...
        if( retbcast!=MPI_SUCCESS )
        {
            cerr<<"ERROR!!!! LocalizationRegions::bcast(), Failure in MPI_Bcast of 'radius'!!!"<<endl;
            MPI_Abort(comm, 0);
        }
    ...
    //bcast gid
    {
        vector<short> bufsizes; bufsizes.reserve(nglobal_);
        vector<LRData>::iterator it=all_regions_.begin();
        while( it!=all_regions_.end() )
        {
    ...
    if( retbcast!=MPI_SUCCESS )
    {
        cerr<<"ERROR!!!! LocalizationRegions::bcast(), Failure in MPI_Bcast of 'volume'!!!"<<endl;
        MPI_Abort(comm, 0);
    }
#endif
    if( ct.verbose>0 )
...
void LocalizationRegions::printAllRegions(ostream& os)
{
    ...
    if( onpe0 )
    {
        ...
        for(int i=0;i<nglobal_;i++)
        {
            ...
            os<<", radius "<<all_regions_[i].radius
                <<", multiplicity "<<all_regions_[i].gid.size()
                <<endl;
        }
    }
}
...
float LocalizationRegions::imposeMinDistanceBetweenCentersGlobal(const float drmin)
{
    ...
    while( npairs>0 ){
        ...
        for(int index0=0; index0<nglobal_; index0++)
        {
            ...
            for(int index1=0; index1<index0; index1++)
            {
                ...
                Vector3D dr=all_regions_[index1].center.vminimage(all_regions_[index0].center, cell_, ct.bcPoisson);
        ...
        if( npairs>0 )
        {
            ...
            {
                all_regions_[index].center[0]=all_regions_[index].center[0]+forces[3*index+0];
                all_regions_[index].center[1]=all_regions_[index].center[1]+forces[3*index+1];
                all_regions_[index].center[2]=all_regions_[index].center[2]+forces[3*index+2];
...
float LocalizationRegions::getStatesWithClosestCenters(
    int* const st1, int* const st2,
    const set<SymmetricPair>& exclude_set)const
{
    float drmin2=1.e12;
    Control& ct = *(Control::instance());
    ...
    for(int index0=0; index0<(int)overlap_regions_.size(); index0++)
    {
        for(int index1=0; index1<index0; index1++){
            ...
            Vector3D dr=overlap_regions_[index1].center.vminimage(overlap_regions_[index0].center, cell_, ct.bcPoisson);
...
bool LocalizationRegions::isLRcenterLocal(const LRData& lr)const
{
    ...
    for(short i=0;i<3;i++)
    {
        while( t[i]>=subdom_end_[i]   ) t[i]-=mygrid.ll(i);
        while( t[i]<subdom_origin_[i] ) t[i]+=mygrid.ll(i);
...
void LocalizationRegions::setupLocalRegionsFromOverlapRegions()
{
    ...
    if( ct.verbose>0 )
        printWithTimeStamp("LocalizationRegions::setupLocalRegionsFromOverlapRegions()...",(*MPIdata::sout));
...
void LocalizationRegions::setupSubdomainGeometry()
{
    ...
    for(short i=0; i<3; i++)
        subdom_lower_left_[i] = (double)myPEenv.my_mpi(i) * subdom_lattice_[i] + subdom_origin_[i];
    for(short i=0; i<3; i++)
        subdom_upper_right_[i] = subdom_lower_left_[i] + subdom_lattice_[i];
    for(short i=0; i<3; i++)
        subdom_center_[i] = 0.5*(subdom_lower_left_[i] + subdom_upper_right_[i]);
...
bool LocalizationRegions::addOverlappingRegion(const LRData& region)
{
    ...
    for(short i=0;i<3;i++)
    {
        while( t[i]>=subdom_center_[i]+0.5*mygrid.ll(i) ) t[i]-=mygrid.ll(i);
        while( t[i]< subdom_center_[i]-0.5*mygrid.ll(i) ) t[i]+=mygrid.ll(i);
...
void LocalizationRegions::computeOverlapRegionsGlobal()
{
    ...
    if( ct.verbose>0 )
        printWithTimeStamp("LocalizationRegions::computeOverlapRegionsGlobal()...",(*MPIdata::sout));
    ...
    if( ct.verbose>0 )
        printWithTimeStamp("LocalizationRegions::computeOverlapRegionsGlobal() done...",(*MPIdata::sout));
...
{    
                                       ...
                                       it++)
    {
        ...
        {
            //subdiv center
            double  cc[3]={0.5*(2.*subdom_lower_left_[0]+(2*iloc+1)*loc_length),
                           0.5*(2.*subdom_lower_left_[1]+subdom_div_lattice_[1]),
                           0.5*(2.*subdom_lower_left_[2]+subdom_div_lattice_[2])};
            double  t[3]={center[0],center[1],center[2]};
            for(short i=0;i<3;i++){
                while( t[i]>=cc[i]+0.5*mygrid.ll(i) ) t[i]-=mygrid.ll(i);
                while( t[i]< cc[i]-0.5*mygrid.ll(i) ) t[i]+=mygrid.ll(i);
            ...
            if(p[1]<0.)p[1]=0.;
            if(p[2]<0.)p[2]=0.;
            if(p[0]>(iloc+1)*loc_length-hgrid0)p[0]=(iloc+1)*loc_length-hgrid0;
            if(p[1]>subdom_div_lattice_[1])p[1]     =subdom_div_lattice_[1];
            if(p[2]>subdom_div_lattice_[2])p[2]     =subdom_div_lattice_[2];
            double d2=(p[0]-t[0])*(p[0]-t[0])
                     +(p[1]-t[1])*(p[1]-t[1])
...
void LocalizationRegions::syncCenters()
{
    ...
    MGmol_MPI& mmpi(*(MGmol_MPI::instance()));
    Control& ct = *(Control::instance());
    if( ct.verbose>0 )printWithTimeStamp("LocalizationRegions::syncCenters()...",(*MPIdata::sout));
    ...
    cout<<"LocalizationRegions::syncCenters() -- Total Nb. gids = "<<num_gid
        <<endl;
...
#endif
    if( ct.verbose>0 )printWithTimeStamp("LocalizationRegions::syncCenters() done...",(*MPIdata::sout));
    syncCenter_tm_.stop();
}
...
void LocalizationRegions::updateOverlapRegions()
{
   ...
   for(short dir=0; dir<3; dir++)
   {
      ...
      for(short disp=-1;disp<2; disp=disp+2)
      {
         ...
         const int bsiz = data_pos_[2] + 3*(old_centers_.size()+2)*ndata*sizeof(double);   
...
void LocalizationRegions::augmentOverlapRegions(const short dir, const short disp, const int buffer_size, 
                                                const vector<LRData>& ldata, set<int>& overlap_gid_set)
{   
   ...
   *(iptr++) = lsize;  
   ...
   /* pack overlap gids */   
   for(vector<LRData>::const_iterator it =ldata.begin(); it!=ldata.end(); it++)
      *(iptr++) = it->gid[0];
   ...
   for(vector<LRData>::const_iterator it =ldata.begin(); it!=ldata.end(); it++)
   {
      for(short i=0; i<3; i++) *(dptr++) = it->center[i];
      ndoubles+=3;
   }
  ...
  /* Send and receive data */
   ...
   for ( int i=0; i<old_centers_.size(); i++ )
   {
      old_centers[i] = (double *)(rbuf + offset + padding + 3*(i+1)*nrecv*sizeof(double));//recv_centers + 3*nrecv;
   } 
   ...
   for(int i=0; i<nrecv; i++)
   {
      ...
      Vector3D region_center(recv_centers[idx], recv_centers[idx+1], recv_centers[idx+2]);
      LRData region;
      ...
      for ( int i=0; i<old_centers_.size(); i++ )
      {
         assert( offset + padding + 3*(i+1)*nrecv*sizeof(double) + (idx+2)*sizeof(double) < rsiz );
         oc.push_back(Vector3D(old_centers[i][idx], old_centers[i][idx+1], old_centers[i][idx+2]));
...
void LocalizationRegions::getMatrixDistances(SquareLocalMatrices<MATDTYPE>& mat,
...
void LocalizationRegions::push_back_local(const Vector3D& center, 
                                          const float radius, 
...
void LocalizationRegions::push_back_global(const Vector3D& center, 
                                           const float radius, 
                                           const std::vector<int>& gid)
{
    assert( radius>0.1 );
...
void LocalizationRegions::randomizeGids()
{
    ...
    random_shuffle ( all_regions_.begin(), all_regions_.end(),
                          myrandom );
                                     ...
                                     ++it)
    {
        ...
        for(vector<int>::iterator git =gid.begin();
                                  git!=gid.end();
                                ++git)
        {
            *git = new_gid;
...
double LocalizationRegions::distanceBetweenCenters(const int gid1, const int gid2)const
{
    Control& ct = *(Control::instance());
    Mesh* mymesh = Mesh::instance();
    const pb::Grid& mygrid  = mymesh->grid();
...
void LocalizationRegions::writeOldCenters(HDFrestart& h5f_file)
{
    ...
    if( file_id>=0 )
    {
        ...
        if( h5f_file.useHdf5p() )
        {
            ...
            for(short i=s;i<ms;i++)gids.push_back(-1);
        }
        ...
        if( h5f_file.useHdf5p() )
        {
            cerr<<"writeOldCenters(), parallelWrite1d not yet implemented."<<endl;
...
void LocalizationRegions::writeOldCenter(HDFrestart& h5f_file, int i)
{
    ...
    if( file_id>=0 )
    {
        ...
        if( h5f_file.useHdf5p() )
        {
            mgmol_tools::parallelWrite2d(file_id,datasetname.str(),data,dims,h5f_file.comm_active());
        }
        else
        {
