...
public:  
    ...
    virtual void setup(const double kbt, const int nel, const std::vector<std::vector<int> >& global_indexes);
    ...
    double getEigSum();
    double getLinDependent2states(int& st1, int& st2, const bool print_flag=false)const;    
...
{
  if( onpe0 )
    os<<" Matrix Theta ... n = "<<(*submatT_).n() <<endl;
...
{
    return invS_->precondIsNew();
}

bool isGramMatAugmented()
{
   return invS_->isGramAugmented();
}

double computeTraceInvSmultMat(const SquareLocalMatrices<MATDTYPE>& mat, const bool consolidate);
double computeTraceDMmultMat(const SquareLocalMatrices<MATDTYPE>& mat, const bool consolidate);
double computeTraceInvSmultMatmultTheta(SquareLocalMatrices<MATDTYPE>& mat, const bool consolidate);
double computeTraceInvSmultMat(VariableSizeMatrix<sparserow>& vsmat, const bool consolidate);
virtual void stripDM(){ return; };
virtual void dressupDM(){ return; };

const VariableSizeMatrix<sparserow>& getH()
{
    return *matHB_;
}
void assignH(const VariableSizeMatrix<sparserow>& matH)
{
    *matHB_=matH;
}
