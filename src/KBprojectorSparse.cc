...
KBprojectorSparse::KBprojectorSparse(const Species& sp):
    species_(sp)
{
    ...
    if( work_nlindex_.size()==0 )
        work_nlindex_.resize(omp_get_max_threads());
    if( work_proj_.size()==0 )
        work_proj_.resize(omp_get_max_threads());
    ...
    for(short l=0;l<=maxl_;l++) multiplicity_.push_back(sp.getMultiplicity(l));
    
    for(short l=0;l<=maxl_;l++)assert( multiplicity_[l]>0 || l==llocal_);
...
void KBprojectorSparse::setNLindex(const short iloc, const int size_nl, 
                             const int* const pvec)
{
    ...
    {
        assert( i<lnumpt );
        if( (pvec[i] < iloc*lnumpt)
         || (pvec[i] >= (iloc+1)*lnumpt) )
        {
             (*MPIdata::sout)<<" iloc="<<iloc<<", i="<<i<<", pvec="<<pvec[i]<<endl;
            exit(2);
        }
...
void KBprojectorSparse::init_work_nlindex(const short iloc,
    ...
    for(short l = 0;l <= maxl_;l++)
    {
        ...
        if(l != llocal_)
        {
            switch(l)
            {
                ...
                default:
                    (*MPIdata::sout)<<"KBprojectorSparse::setProjectors(): "
...
void KBprojectorSparse::setSProjector(const short iloc, const int icount)
{
    ...
    for(short p=0;p<multiplicity_[0];++p)
    {
            ...
            for(int iy = 0;iy < range_kbproj_;iy++)
            {
                ...
                for(int iz = 0;iz < range_kbproj_;iz++)
                {
                    if(is_in_domain[idx])
                    {
                        ...
                        rtptr[jcount] = (KBPROJDTYPE)(nlproj.cubint(r)*factor);
        ...
        if(jcount != icount)
        {
            (*MPIdata::sout)<<"KBprojectorSparse::setSProjector(): Problem with non-local projectors generation"<<endl;
            exit(2);
        }
...
void KBprojectorSparse::setPProjector(const short iloc, const int icount)
{
    ...
    for(short p=0;p<multiplicity_[1];p++)
    {
        ...
        if(jcount != icount)
        {
            (*MPIdata::sout)<<"setPProjector: Problem with non-local projector generation"<<endl;
            exit(2);
        }
...
void KBprojectorSparse::setDProjector(const short iloc, const int icount)
{
    ...
    for(short p=0;p<multiplicity_[2];p++)
    {
                ...
                for(int iz = 0;iz < range_kbproj_;iz++)
                {
                    if(is_in_domain[idx])
                    {
                        ...
                        if(rr>rthreshold)
                        {
                            ...
                            r4[jcount] = (KBPROJDTYPE)(0.5*t1*(x*x - y*y));
                            r5[jcount] = (KBPROJDTYPE)(0.5*t1*(sqrt3*z*z-rr*inv_sqrt3));
                        }
                        else
                        {
                            r1[jcount] = 0.;
        ...
        if(jcount != icount)
        {
            (*MPIdata::sout)<<"setDProjector: Problem with non-local generation"<<endl;
            exit(2);
        }
                ...
                for(int iz = 0;iz < range_kbproj_;iz++)
                {
                    if(is_in_domain[idx])
                    {
                        ...
                        if(rr>rthreshold)
                        {
                            ...
                            r1[jcount] = (KBPROJDTYPE)(sqrt5*t1 * (3.*x2 - y2)*y);
                            r2[jcount] = (KBPROJDTYPE)(2.*sqrt2*sqrt3*sqrt5*t1 * x* y * z);
                            r3[jcount] = (KBPROJDTYPE)(sqrt3*t1 * y*(4.*z2-x2-y2));
                            r4[jcount] = (KBPROJDTYPE)(sqrt2*t1*z*(2.*z2-3.*x2-3*y2));
                            r5[jcount] = (KBPROJDTYPE)(sqrt3*t1*x*(4.*z2-x2-y2));
                            r6[jcount] = (KBPROJDTYPE)(sqrt2*sqrt3*sqrt5*t1*(x2-y2)*z);
                            r7[jcount] = (KBPROJDTYPE)(sqrt5*t1*(x2-3.*y2)*x);
                        }
                        else
                        {
                            r1[jcount] = 0.;
...
bool KBprojectorSparse::overlapWithBox(const short index_low[3], 
...
template <typename T>
void KBprojectorSparse::axpySKet(const short iloc, 
                                 const double alpha, 
                                 T* const dst)const
{
...
template <typename T>
void KBprojectorSparse::axpyKet(const short iloc, 
...
bool KBprojectorSparse::setIndexesAndProjectors()
{
...
#if CHECK_NORM
    norm2_.resize(4);
    for(short p=0;p<multiplicity_[0];++p)norm2_[0].push_back(0.);
    if( multiplicity_.size()>1 )
        for(short p=0;p<3*multiplicity_[1];++p)norm2_[1].push_back(0.);
    if( multiplicity_.size()>2 )
        for(short p=0;p<5*multiplicity_[2];++p)norm2_[2].push_back(0.);
    if( multiplicity_.size()>3 )
        for(short p=0;p<7*multiplicity_[3];++p)norm2_[3].push_back(0.);
#endif
    ...
    if(onpe0)
    for(short p=0;p<multiplicity_[0];++p)
        cout<<"Norm2 S-projector, p="<<p<<" ="<<norm2_[0][p]*mygrid.vel()<<endl;
    ...
    if( maxl_>0 )
    {
        ...
        for(short p=0;p<multiplicity_[1];++p)
        {
            cout<<"Norm2 Px-projector, p="<<p<<" ="<<norm2_[1][3*p+0]*mygrid.vel()<<endl;
            cout<<"Norm2 Py-projector, p="<<p<<" ="<<norm2_[1][3*p+1]*mygrid.vel()<<endl;
            cout<<"Norm2 Pz-projector, p="<<p<<" ="<<norm2_[1][3*p+2]*mygrid.vel()<<endl;
        }
        ...
        if( maxl_>1 )
        {
            ...
            for(short p=0;p<multiplicity_[2];++p)
            {
                cout<<"Norm2 D-projector, p="<<p<<" ="<<norm2_[2][5*p+0]*mygrid.vel()<<endl;
                cout<<"Norm2 D-projector, p="<<p<<" ="<<norm2_[2][5*p+1]*mygrid.vel()<<endl;
                cout<<"Norm2 D-projector, p="<<p<<" ="<<norm2_[2][5*p+2]*mygrid.vel()<<endl;
                cout<<"Norm2 D-projector, p="<<p<<" ="<<norm2_[2][5*p+3]*mygrid.vel()<<endl;
                cout<<"Norm2 D-projector, p="<<p<<" ="<<norm2_[2][5*p+4]*mygrid.vel()<<endl;
            }
            if( maxl_>2 )
            {
                ...
                for(short p=0;p<multiplicity_[1];++p)
                {
                    cout<<"Norm2 F-projector, p="<<p<<" ="<<norm2_[3][7*p+0]*mygrid.vel()<<endl;
                    cout<<"Norm2 F-projector, p="<<p<<" ="<<norm2_[3][7*p+1]*mygrid.vel()<<endl;
                    cout<<"Norm2 F-projector, p="<<p<<" ="<<norm2_[3][7*p+2]*mygrid.vel()<<endl;
                    cout<<"Norm2 F-projector, p="<<p<<" ="<<norm2_[3][7*p+3]*mygrid.vel()<<endl;
                    cout<<"Norm2 F-projector, p="<<p<<" ="<<norm2_[3][7*p+4]*mygrid.vel()<<endl;
                    cout<<"Norm2 F-projector, p="<<p<<" ="<<norm2_[3][7*p+5]*mygrid.vel()<<endl;
                    cout<<"Norm2 F-projector, p="<<p<<" ="<<norm2_[3][7*p+6]*mygrid.vel()<<endl;
                }
            }
        }
    }
...
void KBprojectorSparse::getProjectors(const short iloc, 
                                      vector<const KBPROJDTYPE*>& projectors)const
{
    projectors.clear();
...
template void KBprojectorSparse::axpySKet<double>(const short iloc, const double alpha, double* const dst)const;
template void KBprojectorSparse::axpySKet<float>(const short iloc, const double alpha, float* const dst)const;

template void KBprojectorSparse::axpyKet<double>(const short iloc, const vector<double>& alpha, double* const dst)const;
template void KBprojectorSparse::axpyKet<float>(const short iloc, const vector<double>& alpha, float* const dst)const;
