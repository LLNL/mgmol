...
BlockVector<double>::~BlockVector()
{
    ...
    if( n_instances_==1 )
    {
        delete data_wghosts_; data_wghosts_=0;
        allocated_.clear();
        for(vector<double*>::iterator it=class_storage_.begin();
                                      it!=class_storage_.end();
                                      ++it)
            delete[] *it;
    }
    n_instances_--;
...
void BlockVector<T>::allocate1NewBlock()
{
    ...
    if( allocated_size_storage_==0 )
        allocated_size_storage_=(int)((T)(size_storage_)*overallocate_factor_);
        
    // check if storage required is not bigger than initial allocation
    if( size_storage_>allocated_size_storage_ )
    {
        (*MPIdata::sout)<<"BlockVector::allocate1NewBlock(): allocated_size_storage_ = "<<allocated_size_storage_<<endl;
        (*MPIdata::sout)<<"BlockVector::allocate1NewBlock(): size_storage_           = "<<size_storage_<<endl;
        (*MPIdata::sout)<<"BlockVector::allocate1NewBlock(): size_storage_ too large!!!"<<endl;
        exit(0);
    }
    
    Control& ct = *(Control::instance());
    if( onpe0 && ct.verbose >1 )
        (*MPIdata::sout)<<"BlockVector, allocate memory size "<<allocated_size_storage_<<endl;
...
void BlockVector<T>::allocate_storage()
{
    ...
    else
    {
        ...
        {
            cerr<<"Number of allocated blocks: "
                <<class_storage_.size()
    ...
    if( my_allocation_>high_water_mark )
        high_water_mark=my_allocation_;
    ...
    if( allocated_size_storage_<size_storage_ )
    {
        cerr<<"ERROR BlockVector: trying to use allocation "<<size_storage_
            <<" bigger than initialy preallocated "<<allocated_size_storage_<<"!!!"<<endl;
        ct.global_exit(0);
    }
...
BlockVector<T>::BlockVector(const BlockVector<T>& bv, const bool copy_data):
    mygrid_(bv.mygrid_), size_storage_instance_(bv.size_storage_instance_), ld_instance_(bv.ld_instance_)
{
    n_instances_++;
                                ...
                                const bool skinny_stencil)
{
    ...
    data_wghosts_=new pb::GridFuncVector<T>(true,mygrid_,
                                         bc_[0],bc_[1],bc_[2], gid,
                                         skinny_stencil);
...
template <typename T>
void BlockVector<T>::axpy(const double alpha,const int ix, const int iy, const short iloc)
{
...
template <typename T>
void BlockVector<T>::axpy(const double alpha, BlockVector<T>& bv, const int ix, const int iy, const short iloc)
{
