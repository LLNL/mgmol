...
MD_IonicStepper::MD_IonicStepper(const double dt,
                vector<short>& atmove,
                vector<double>& tau0, 
                vector<double>& taup,
                vector<double>& taum, 
...
void MD_IonicStepper::setThermostat(const int ttherm,
                   const double tkel,
                   const double thtime,
                   const double thwidth,
                   const int nconstraints)
{
    ...
    if( onpe0 && ct.verbose>0 )
        (*MPIdata::sout)<<"MD_IonicStepper::setThermostat(), ndofs= "<<ndofs_<<endl;
}
...
int MD_IonicStepper::init(HDFrestart& h5f_file)
{
    ...
    {
        if( onpe0 )
            (*MPIdata::sout)<<"MD_IonicStepper::init() --- use positions from restart file with dt="<<dt_<<endl;
    ...
    {
        if( onpe0 )
            (*MPIdata::sout)<<"MD_IonicStepper::init() --- extrapolate positions from restart file"<<endl;
        // taum_: velocities -> displacements = +dt*vel
        double alpha=dt_;
        dscal(&size_tau, &alpha, &taum_[0], &ione);
...
int MD_IonicStepper::writeForces(HDFrestart& h5f_file)
{
    ...
    if( dataspace_id<0 ){
        (*MPIdata::serr)<<"MD_IonicStepper: H5Screate_simple failed!!!"<<endl;
        return -1;
    }

    // Open dataset
    hid_t dataset_id = H5Dcreate2(file_id, "/Ionic_forces",
                                 H5T_NATIVE_DOUBLE, 
                                 dataspace_id,
                                 H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    if( dataset_id<0 ){
        (*MPIdata::serr)<<"MD_IonicStepper::H5Dcreate2 /Ionic_forces failed!!!"<<endl;
        return -1;
    }

    // Write forces
    herr_t status = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, 
                             H5S_ALL, H5P_DEFAULT, &fion_[0]);
    if( status<0 ){
        (*MPIdata::serr)<<"MD_IonicStepper::H5Dwrite forces failed!!!"<<endl;
        return -1;
    }else{
        if ( onpe0 )
        (*MPIdata::sout)<<"Ionic forces written into "
...
int MD_IonicStepper::writeTaum(HDFrestart& h5f_file)
{
    ...
    hid_t dataset_id = H5Dcreate2(file_id, "/Ionic_velocities",
                                 ...
                                 H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    if( dataset_id<0 ){
        (*MPIdata::serr)<<"MD_IonicStepper::H5Dcreate2 /Ionic_velocities failed!!!"<<endl;
        return -1;
    }

    // Write forces
    herr_t status = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, 
                             H5S_ALL, H5P_DEFAULT, &taum_[0]);
    if( status<0 ){
...
int MD_IonicStepper::write_hdf5(HDFrestart& h5f_file)
{
    ...
    if( write_flag )
    {
        ...
        if( dt_>0. )writeVelocities(h5f_file);
        else        writeTaum(h5f_file);
...
int MD_IonicStepper::run()
{
    ...
    for ( int ia = 0; ia < na; ia++ )assert( pmass_[ia]>0.1 );
    ...
    if ( dt_ > 0.0 )
    {
        ...
        {
            if ( onpe0 )
                (*MPIdata::sout)<<"Berendsen Thermostat for target temperature "
                                <<tkel_<<" [Kelvin]"<<endl;
            ...
            {
                if ( onpe0 )
                    (*MPIdata::sout)<<"Current temperature "<<tempm<<" [Kelvin]"<<endl;
        ...
        else if ( ttherm_==2 )
        {
            ...
            if ( onpe0 )
                (*MPIdata::sout)<<"Langevin Thermostat with gamma_="<<gamma_<<", factor="<<factor<<endl;
        ...
        if ( ttherm_!=2 )
            updateWithVelocityScaling(lambda);
    ...
    if( onpe0 )
    {
        ...
        for ( int ia = 0; ia < na; ia++ )
        {
            ...
            os<<setprecision(4)<<fixed
              ...
              <<pmass_[ia]*invscmass
              <<setw(10)
              <<0.5*(taup_[3*ia]  +tau0_[3*ia  ])
              <<setw(10)
              <<0.5*(taup_[3*ia+1]+tau0_[3*ia+1])
              <<setw(10)
              <<0.5*(taup_[3*ia+2]+tau0_[3*ia+2])
              <<setprecision(3)<<scientific
              <<setw(12)
              <<invdt*(taup_[3*ia  ]-tau0_[3*ia  ])
              <<setw(12)
              <<invdt*(taup_[3*ia+1]-tau0_[3*ia+1])
              <<setw(12)
              <<invdt*(taup_[3*ia+2]-tau0_[3*ia+2])
              <<endl;
        }
    }
}
...
void MD_IonicStepper::getVelocities(vector<float>& velocities)const
{
    ...
    for ( int ia = 0; ia < na; ia++ )
    {
        ...
        velocities.push_back((float)(invdt*(taup_[3*ia+1]-tau0_[3*ia+1])));
        velocities.push_back((float)(invdt*(taup_[3*ia+2]-tau0_[3*ia+2])));
    }    
}



double MD_IonicStepper::etol(void) const
{
    return 0.0;
}
