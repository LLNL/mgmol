...
MVPSolver::MVPSolver(MPI_Comm comm, ostream& os,
                           Ions& ions,
                           Rho* rho,
                           Energy* energy,
    ...
    use_old_dm_(use_old_dm)
{
    ...
    numst_= numst;
    work_ = new dist_matrix::DistMatrix<DISTMATDTYPE>("workMVP",numst_, numst_);
...
double MVPSolver::evaluateDerivative(dist_matrix::DistMatrix<DISTMATDTYPE>& dmInit, 
                                          ...
                                          const double ts0)
{
    ...
    //
    //if( onpe0 ) os_<<"evaluate numerical derivative of entropy in beta=0"<<endl;
    ...
    *work_=dmInit;
    ...
    {
        os_<<setprecision(8);
        os_<<"Numerical derivative of -TS in 0 = "<<scientific<<-dts0e<<endl;
...
// build target
{
    ...
    if( ct.verbose>2 )
    {
        ...
        if( onpe0 )
            os_<<"Number of electrons = "<<pnel<<endl;
    }
...
int MVPSolver::solve(LocGridOrbitals& orbitals)
{
...

    {
        ...
        dist_matrix::DistMatrixWithSparseComponent<DISTMATDTYPE>  h11("h11",numst_, numst_,comm_,
        ...
        for(int inner_it=0;inner_it<n_inner_steps_;inner_it++)
        {
            ...
            ProjectedMatricesInterface* current_proj_mat = (inner_it==0) ? orbitals.getProjMatrices() 
                                                                         : proj_mat_work_;
            ...
            mgmol_strategy_->addHlocal2matrix(orbitals, orbitals,
                                         h11);
            ...
            if( inner_it==0 )
            {
                ...
                e0 = energy_->evaluateTotal(ts0, projmatrices, orbitals, printE, os_);
            }
            else
            {
                ...
                e0 = energy_->evaluateTotal(ts0, proj_mat_work_, orbitals, printE, os_);
            }


            // N x N target...
            //proj_mat_work_->setHiterativeIndex(orbitals.getIterativeIndex(), pot.getIterativeIndex());

            dist_matrix::DistMatrix<DISTMATDTYPE> target("target", numst_, numst_);        
            dist_matrix::DistMatrix<DISTMATDTYPE> delta_dm("delta_dm", numst_, numst_);
            ...
            if(use_old_dm_||inner_it>0)
            {
                ...
                const double ts1 = evalEntropyMVP(proj_mat_work_,(ct.verbose>2),os_ );
                const double e1 = energy_->evaluateTotal(ts1, proj_mat_work_, orbitals, ct.verbose-1, os_);
            ...
            if( inner_it<n_inner_steps_-1 )
            {
                if( ct.verbose>2 )
                {
                    double pnel=proj_mat_work_->getNel();
                    if( onpe0 )
                        os_<<"Number of electrons for interpolated DM = "<<pnel<<endl;
                }
