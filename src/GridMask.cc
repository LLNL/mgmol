...
// Constructor
GridMask::GridMask(const unsigned short nclevels,
                   const unsigned short subdivx,
    ...
    grid_(mygrid),subdivx_(subdivx)
{
    ...
    for(unsigned short l=0;l<nlevels_;l++)
    {
        ...
        assert( loc_numpt_[l]== numpt/(subdivx*(1<<l)*(1<<l)*(1<<l)) );
    }
...
// at level "level"
int GridMask::init(const Vector3D& center,
                      const double rcut,
...
#pragma omp master
    ...
    //(*MPIdata::sout)<<"GridMask::init(), center="<<center<<", rcut="<<rcut<<endl;
    ...
    double delta=delta_;
    for(unsigned short l=0;l<level;l++)delta*=2.;

    double overlap_radius=rcut+delta;
    bool overlap=
        sub_cell_[level]->spherePossibleOverlap(center,overlap_radius,iloc,
                                                ct.bcPoisson);

    // try cheap cases first
    if( !overlap ){ // Mask=0 in whole subdomain
        assign(iloc, level, 0, 0, mask);
        init_tm_.stop();
        return 0;
    }else if( 100. < rcut ){ // Mask =1
        assign1(iloc, level);
        init_tm_.stop();
        return size;
    }
    ...
    const double start[3]={grid_.start(0),
                           grid_.start(1),
                           grid_.start(2)};

    const double  hgrid[3]={grid_.hgrid(0)*(1<<level),
                            grid_.hgrid(1)*(1<<level),
                            grid_.hgrid(2)*(1<<level)};
        
#ifdef DEBUG
    int     incy  = dim2;
    int     incx  = dim2*dim1;
    int     lcount=0;
    //(*MPIdata::sout)<<"dim=("<<subdim0_[level]<<", "<<dim1<<", "<<dim2<<")"<<endl;
#endif
    ...
    for(int ix = 0;ix < subdim0_[level];ix++)
    {
        ...
        for(int iy = 0;iy < dim1;iy++)
        {
            ...
            for(int iz = 0;iz < dim2;iz++)
            {
                ...
                assert(r>=0.);
                assert( r*r < ll[0]*ll[0]+ll[1]*ll[1]+ll[2]*ll[2]+0.1 );
                if(r < overlap_radius ){
                    icount++;
    ...
    // icount needs to be at least 1 so that the next check for overlap is consistent
    // (e.g. in restart)
    // even if no actual point was found in sphere!!!
    // Otherwise restart may attribute data to wrong orbital when changing radius
    if( level==0 )
        icount = max(1,icount);
...
void GridMask::assign(const unsigned short iloc, const unsigned short level, 
                         const int num, const int xnum, 
                         const vector<lmasktype>& val)
{
    assert(xnum>=num);
    ...
    if( num==0 && xnum==0){
        if( mask_not_zero_[level][iloc]==2 )
            lmask_[level][iloc].resize(0);
...
bool GridMask::overlap_on_pe(const GridMask& gm, const unsigned short level)const
{
...
template<typename T>
void GridMask::multiplyByMask(T *u,
                                 const unsigned short level,
...
template
void GridMask::apply(pb::GridFunc<ORBDTYPE>& gu, const unsigned short level);

template
void GridMask::multiplyByMask(float *u,
                              const unsigned short level,
                              const unsigned short iloc)const;
template
void GridMask::multiplyByMask(double *u,
                              const unsigned short level,
                              const unsigned short iloc)const;

template
void GridMask::cutWithMask(float *u,
                              const unsigned short level,
                              const unsigned short iloc)const;
template
void GridMask::cutWithMask(double *u,
                              const unsigned short level,
                              const unsigned short iloc)const;
