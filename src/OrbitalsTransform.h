...
protected:
  ...
  std::vector<dist_matrix::DistMatrix<DISTMATDTYPE>*> r_; // cosine and sine matrices for x,y,z directions
...
public:
  ...
  std::vector<dist_matrix::DistMatrix<DISTMATDTYPE>* >& r(void)
  {
    return r_;
  }
  ...
  std::vector<DISTMATDTYPE>& mat(void)
  {
    return mat_;
  }

  dist_matrix::DistMatrix<DISTMATDTYPE>& a(void)
  {
    return *a_;
  }
  ...
  Vector3D center(void)const;
    ...
    OrbitalsTransform(const int nst, const Vector3D& origin, 
                                     const Vector3D& ll);
