...
private:
   ...
   std::vector<int> locfcns_;	// cluster of variables assigned to this proc. Initially local variables centered on this proc.   
   double cluster_range_radius_;  // radius of how far to gather data to determine cluster for each proc.
   int subdom_steps_; // maximum number of steps to gather regions information from subdomains
   double subdom_bias_;	// subdomain bias
   double avg_locfcns_global_; // global average number of centered localized functions
   double avg_locfcns_; // "local" average number of centered localized functions
   double old_avg_locfcns_; // previous "local" average number of centered localized functions
   ...
   std::map<int,Vector3D>subdoms_data_; // indexes (pids) and centers of (neighboring) subdomains
   std::vector<int> subdoms_index_;  // pids of subdomains
   std::vector<double> bias_data_; // vector of bias values subdomain and its neighbors
   
   std::map<int,Vector3D> regions_data_; // gids and centers of regions in clustering search space
   std::vector<int> cluster_indexes_; // indexes of regions owned by this subdomain cluster
   ...
   void computeLocalBias();  // compute/ update local bias. Reset initial bias to zero if needed.
   void updateBiasData();
   double computeSquaredDistanceBetweenCenters(const Vector3D& center1, const Vector3D& center2)const;
   bool checkConv();
   void reset();
   void computeLocalRegionOwnership();
   void updateCluster();
   void writeVTKHeader(const int npx, const int npy, const int npz, std::ofstream& os);
   void writeVTKDataset(const std::string name, const pb::PEenv& myPEenv, std::ofstream& os);
...
public:
   ...
   int computeClusters(const short maxiters); // compute cluster of variables assigned to procs.
