...
GlobalLBFGS::GlobalLBFGS(LocGridOrbitals** orbitals,
             Ions& ions,
             Rho& rho,
             ConstraintSet& constraints,
             KBPsiMatrixInterface* g_kbpsi,
             Energy& energy,
    ...
    local_image_(local_image)
{
    ...
    stepper_
        = new LBFGS_IonicStepper(dt, gatmove_, gtau0_, gtaup_, gfion_, 
                                 20, &etot_i_[0]);

    constraints_.setup(ions_);

    if( onpe0 )
        constraints_.printConstraints((*MPIdata::sout));
...
void GlobalLBFGS::init(HDFrestart* h5f_file)
{
    ...
    if(ct.restart_info > 0){
        ...
        if(status==0){
            if( onpe0 )
                (*MPIdata::sout)<<"use restart info for LBFGS"<<endl;
...
int GlobalLBFGS::run1step()
{
    ...
    memcpy(&taup_[0],&gtaup_[local_image_*taup_.size()],taup_.size()*sizeof(double));
...
bool GlobalLBFGS::checkTolForces(const double tol)
{
    ...
    if( onpe0 )
        (*MPIdata::sout)<<setprecision(3)<<scientific<<"global_lbfgs: Max. |force| for image = "<<sqrt(f2)<<endl;
...
void GlobalLBFGS::addImageInteractionEnergy(const double eii)
{
    etot_i_[2]+=eii;
    if( onpe0 ){
        (*MPIdata::sout)<<"global_lbfgs: add NEB energy. Total energy = "<<etot_i_[2]<<endl;
    }
}
...
void GlobalLBFGS::updateRefs()
{
    ...
    if( !stepper_->check_last_step_accepted() ){
        if( onpe0 )
            (*MPIdata::sout)<<"global_lbfgs: Reset orbitals to reference orbitals "<<endl;
        ...
        ref_orbitals_->reset(ct.getMGlevels(), ct.lap_type);
        if( onpe0 )
             (*MPIdata::sout)<<"global_lbfgs: Update reference orbitals "<<endl;
        ref_orbitals_->assign(**orbitals_);
...
void GlobalLBFGS::setQuenchTol()const
{
    ...
    if( onpe0 )
        (*MPIdata::sout)<<setprecision(12)<<fixed
            <<"global_lbfgs: Set SC convergence criterion to "<<ct.tol_energy<<endl;
}
