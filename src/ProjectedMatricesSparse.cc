...
Timer ProjectedMatricesSparse::compute_inverse_tm_("ProjectedMatricesSparse::computeInverse");
Timer ProjectedMatricesSparse::compute_invB_tm_("ProjectedMatricesSparse::computeInvB");
Timer ProjectedMatricesSparse::init_gram_matrix_tm_("ProjectedMatricesSparse::initialize_Gram_Matrix");
Timer ProjectedMatricesSparse::update_theta_tm_("ProjectedMatricesSparse::updateTheta");
Timer ProjectedMatricesSparse::update_submatT_tm_("ProjectedMatricesSparse::updateSubmatT");
Timer ProjectedMatricesSparse::update_submatX_tm_("ProjectedMatricesSparse::updateSubmatX");
Timer ProjectedMatricesSparse::eigsum_tm_("ProjectedMatricesSparse::eigsum");
Timer ProjectedMatricesSparse::consolidate_H_tm_("ProjectedMatricesSparse::consolidate_sH");
Timer ProjectedMatricesSparse::eig_interval_tm_("ProjectedMatrices::computeEigenInterval");
    								 ...
    								 ClusterOrbitals *local_cluster)
{
    ...
    distributor_invS_ = BasicDataDistributors::centeredOrbitalsOverlapDistributor();    
...
void ProjectedMatricesSparse::setup(const double kbt, const int nel, const vector<vector<int> >& global_indexes)
{
...
double ProjectedMatricesSparse::getExpectationMat(VariableSizeMatrix<sparserow>* mat)
{  
   assert(invS_ != NULL);
   assert(mat != 0);
   assert(mat->n() != 0);
...
double ProjectedMatricesSparse::dotProductWithInvS(const SquareLocalMatrices<MATDTYPE>& local_product)
{
...
// assumes matrix ss contains partial contributions
double ProjectedMatricesSparse::dotProductWithDM(const SquareLocalMatrices<MATDTYPE>& local_product)
{ 
...
double ProjectedMatricesSparse::getNel()const
{
   return 2.*dim_;
}
...
double ProjectedMatricesSparse::getEigSum()
{
   ...
   /* compute trace */
   double trace = 0.0;
   for(std::vector<int>::iterator itr = locfcns.begin(); itr != locfcns.end(); ++itr)
   {
...
double ProjectedMatricesSparse::getLinDependent2states(int& st1, int& st2, const bool print_flag)const
{
   ...
   std::vector<double>::iterator idx1 = std::max_element(evec.begin(), evec.end());
   std::vector<double>::iterator idx2 = std::min_element(evec.begin(), evec.end());
   std::vector<double>::iterator idx = (fabs(*idx1) > fabs(*idx2)) ? idx1 : idx2;
   ...
   *idx = 0.;
   ...
   evec=evec_save;
#else
   // get second index by looking for largest off-diagonal element in row locvars_[locst1]
   double vmax2=0.;
   ...
   {
       cout<<""<<endl;
       cout<<"ProjectedMatricesSparse::getLinDependent2states(): MPI task "<<myid<<endl;
...
void ProjectedMatricesSparse::printGramMatrix2states(const int st1, const int st2, std::ostream& os)const
{
    MGmol_MPI& mmpi = *(MGmol_MPI::instance());
    MPI_Comm comm=mmpi.commSameSpin();
    ...
    typeDoubleInt out;
    in.val = 0.;
    if( find(locvars_.begin(),locvars_.end(),st1)!=locvars_.end() )in.val +=1.;
    if( find(locvars_.begin(),locvars_.end(),st2)!=locvars_.end() )in.val +=1.;
    in.rank = myid;    
   ...
   for(short iloc=0;iloc<subdiv_;iloc++){
       ...
       for(int icolor=0;icolor<chromatic_number_;icolor++)
       {
            ...
            for(int jcolor=0;jcolor<chromatic_number_;jcolor++)
            {
                 ...
                 if( st2 == -1) continue;
                 localM_iloc[icolor+chromatic_number_*jcolor]=submatM.get_value(st1,st2);
            }
       }
   }
}

double ProjectedMatricesSparse::computeTraceInvSmultMat(const SquareLocalMatrices<MATDTYPE>& mat, const bool consolidate)
{
   Control& ct = *(Control::instance());
   ...
   if(consolidate)
   {
      ...
      const pb::PEenv& myPEenv=mymesh->peenv();
      double domain[3]={mygrid.ll(0),mygrid.ll(1),mygrid.ll(2)};
      DataDistribution distributor("traceSqLo",(*lrs_).max_radii(), myPEenv, domain);
...
double ProjectedMatricesSparse::computeTraceDMmultMat(const SquareLocalMatrices<MATDTYPE>& mat, const bool consolidate)
{
   Control& ct = *(Control::instance());
   VariableSizeMatrix<sparserow> vsmat("VS",lsize_);
   vsmat.setupSparseRows(locvars_);
...
double ProjectedMatricesSparse::computeTraceInvSmultMat(VariableSizeMatrix<sparserow>& vsmat, const bool consolidate)
{
   if(consolidate)
   {
      //consolidate only locally centered data since trace is needed
      ...
      const pb::PEenv& myPEenv=mymesh->peenv();
      double domain[3]={mygrid.ll(0),mygrid.ll(1),mygrid.ll(2)};
      DataDistribution distributor("trace",(*lrs_).max_radii(), myPEenv, domain); 
      distributor.augmentLocalData(vsmat, false);
   }
...
double ProjectedMatricesSparse::computeTraceInvSmultMatmultTheta(SquareLocalMatrices<MATDTYPE>& mat, const bool consolidate)
{
/*
   assert(localT_ != 0);
   assert(submatT_ != 0);
...
void ProjectedMatricesSparse::computeGenEigenInterval(std::vector<double>& interval, const int maxits, const double pad)
{   
    srand(13579);
    assert(invS_ != 0);
