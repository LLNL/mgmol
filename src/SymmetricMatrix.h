...
public:
    SymmetricMatrix(const int dimension, const MPI_Comm comm):
        comm_(comm)
    {
    ...
    void getNNZPattern(std::vector<int>& ia, std::vector<int>& ja, int* const nnzrow)const
    {
        ia.push_back(0);
        int nzmax = 0;
        ...
        nzmax = (nnzrow[0]+1)*dimension_;//assumes matrix has similar nonzero row count
        ja.reserve(nzmax);
