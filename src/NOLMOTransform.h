...
private:
  ...
  double getFunctionalValueFixedCenters(const std::vector<std::vector<double> >& ara,
                            const std::vector<std::vector<double> >& aba,
                            const std::vector<double>& inv_alphan);
...
public:
  std::vector<dist_matrix::DistMatrix<DISTMATDTYPE>* >& b(void){
    return b_;
  }
  ...
  NOLMOTransform(const int nst, const Vector3D& origin, 
                                const Vector3D& ll):
      OrbitalsTransform(nst, origin, ll)
  {
    a_=new dist_matrix::DistMatrix<DISTMATDTYPE>("matrixA",*bcr_,nst_,nst_,nst_,bsize_);
    ...
    for ( int k = 0; k < 2*NDIM; k++ ){
      r_[k]=new dist_matrix::DistMatrix<DISTMATDTYPE>(names[k],*bcr_,nst_,nst_,nst_,bsize_);
    }
...
#endif
    for ( int k = 0; k < 2*NDIM; k++ ){
      b_[k]=new dist_matrix::DistMatrix<DISTMATDTYPE>(names[k],*bcr_,nst_,nst_,nst_,bsize_);
    }
    for ( int k = 0; k < 2*NDIM; k++ ){
