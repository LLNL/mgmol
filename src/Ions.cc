...
Ions::Ions(const double lat[3], const vector<Species>& sp):
    species_(sp)
{
    for(short i=0;i<3;i++){
        assert(lat[i]>0.);
        if(lat[i]>10000.){
            (*MPIdata::serr)<<"Ions constructor: lattice["<<i<<"]="<<lat[i]<<"!!!"<<endl;
            exit(2);
        }
        lattice_[i]=lat[i];
    }
...
Ions::Ions(const Ions& ions, const double shift[3]):
    species_(ions.species_)
{
    Mesh* mymesh = Mesh::instance();
    ...
    for(short i=0;i<3;++i)lattice_[i]=ions.lattice_[i];
...
#ifdef DEBUG
    if(onpe0)
        (*MPIdata::sout)<<" list_ions of size "<<list_ions_.size()<<" initialized"<<endl;
#endif
...
{        
    ...
    if( ct.verbose>0 )
        printWithTimeStamp("Ions::setup()...",cout);
...
void Ions::setupInteractingIons()
{
    ...
    {
        (*MPIdata::sout)<<"Ions::setupInteractingIons() with radius "<<rmax<<endl;
    }
    ...
    //(*MPIdata::sout)<<"Number of interacting ions = "<<interacting_ions_.size()<<endl;
...
void Ions::iiforce(const short bc[3])
{
    ...
    {            
        ...
        while(ion2!=interacting_ions_.end())
        {
            if(*ion1!=*ion2)
            {
                ...
                const double s3 = M_2_SQRTPI * exp(-r12 * r12 * invt) * sqrtinvt;
    ...
    {
        (*lion)->add_force(forces[3*ion_index+0],
                       forces[3*ion_index+1],
...
// processors. This works since ion lists are in ascending order of unique ion index
double Ions::energyDiff(const short bc[3])const
{
    ...
    const pb::PEenv& myPEenv=mymesh->peenv();
    double tenergy=0.;
    MPI_Allreduce(&energy, &tenergy, 1, 
                                  MPI_DOUBLE, MPI_SUM, myPEenv.comm());
                                  
    // take half the result to account for double counting loop over interacting_ions
    energy=0.5*tenergy;
#endif
...
void Ions::associate2PE()
{
...
#ifdef DEBUG
    int isum2=0;
    (*MPIdata::sout)<<" offset=("<<offset[0]<<","<<offset[1]<<","<<offset[2]<<")"<<endl;
#endif    
    ...
    const double end[3]    = {origin[0]+lattice_[0],origin[1]+lattice_[1],origin[2]+lattice_[2]};
    ...
    while(ion!=list_ions_.end())
    {
        ...
        {
            t[i] = (*ion)->position(i);
            while( t[i]>=end[i]   ) t[i]-=lattice_[i];
            while( t[i]<origin[i] ) t[i]+=lattice_[i];
            t[i] -= origin[i];
            t[i] -= offset[i];
        }

#if DEBUG
 (*MPIdata::sout)<<" t=("<<t[0]<<","<<t[1]<<","<<t[2]<<")"<<endl;
#endif    
        ...
        if((*ion)->here())
        {
            (*MPIdata::sout)<<" Ion "<<(*ion)->name()
                <<" centered on PE "<<myPEenv.mytask()<<endl;
            isum2++;
        }
#endif    
        ion++;
...
void Ions::printPositionsLocal(ostream& os, const int root)const
{
    ...
    if( mmpi.mypeGlobal() == root ){
        ...
        os<<endl<<" IONIC POSITIONS AND DISPLACEMENTS ON PE"<<root<<":"<<endl;
...
void Ions::writeAtomicNumbers(HDFrestart& h5f_file)
{
    ...
    if( file_id>=0 )
    {
        ...
        if( h5f_file.useHdf5p() )
        {
            mgmol_tools::parallelWrite2d(file_id,datasetname,data,dims,h5f_file.comm_active());
        }
        else
        {
...
void Ions::lockAtom(const string& name)
{
    ...
    while(ion!=local_ions_.end()){
        ...
        if ( name.compare(name_ion) == 0 ){
            (*ion)->lock();
            if( onpe0 )
                (*MPIdata::sout)<<"Lock atom "<<name<<endl;
            break;
        }
        ion++;
    }
...
void Ions::readLockedAtomNames(HDFrestart& h5f_file)
{
    ...
    for (vector<string>::const_iterator i = data.begin (), end = data.end (); i != end; ++i)
    {
        lockAtom(*i);
    }    
...
void Ions::initFromRestartFile(HDFrestart& h5_file)
{
    ...
    {
        (*MPIdata::sout)<<"Ions::setFromRestartFile(), read "
                        <<num_ions_<<" names... on PE0"<<flush<<endl;
    }
    ...
    for(int i=0;i<at_numbers.size();i++)
    {
        ...
        if( at_indexes[i]>=0 )
        {
            Ion* new_ion=new Ion(*spi,at_names[i],default_coords,default_vel, 
                                 at_indexes[i], at_nlprojIds[i]);
...
{
    Control& ct = *(Control::instance());
    if( onpe0 && ct.verbose>0 )(*MPIdata::sout)<<"Read ionic positions from hdf5 file"<<endl;
...
void Ions::writeRandomStates(HDFrestart& h5f_file)
{
    ...
    if( data[0]!=data[0] )
    {
        (*MPIdata::sout)<<"WARNING: Ions::writeRandomStates: data[0]="<<data[0]<<endl;
    }
...
void Ions::removeMassCenterMotion()
{
    ...
    while(ion!=local_ions_.end())
    {
        ...
        (*ion)->setVelocity(velocities[threei  ]-tmp[0],
                            velocities[threei+1]-tmp[1],
                            velocities[threei+2]-tmp[2]);
        ion++;
        i++;
    }
...
{
    double  maxf=0., avfx=0., avfy=0., avfz=0., maxfx=0., maxfy=0., maxfz=0.;
    int     num_atoms=0;
...
// global statistics
      ...
      if( num_movable ){
        ...
        os<<scientific;
        os<<" mean F on movable ions  = ("<<avfx<<"," <<avfy<<"," <<avfz<<")"<<endl;
        os<<" max F on movable ions   = ("<<maxfx<<","<<maxfy<<","<<maxfz<<")"<<endl;
...
void Ions::printForcesLocal(ostream& os, const int root)const
{
    ...
    if(mmpi.mypeGlobal() == root)
    {
       os<<endl<<endl<<"IONIC POSITIONS AND FORCES ON CENTERED ON PE"<<root<<":"<<endl;
...
// global statistics
    ...
    if(mmpi.mypeGlobal() == root)
    {
       if( num_movable )
       {
        ...
        os<<scientific;
        os<<" mean F on movable ions  = ("<<avg_forces[0]<<"," <<avg_forces[1]<<"," <<avg_forces[2]<<")"<<endl;
        os<<" max F on movable ions   = ("<<max_forces[0]<<","<<max_forces[1]<<","<<max_forces[2]<<")"<<endl;
...
int Ions::countIonsHere()const
{
    return (int)local_ions_.size();
}
...
int Ions::readAtoms(ifstream* tfile, const bool cell_relative)
{
    ...
    {
        if( mmpi.PE0() )
            (*MPIdata::sout) << "Read atomic positions from file " << first_string_read << endl;
...
int Ions::readAtomsFromXYZ(const string filename,
                           const bool cell_relative)
{
    ...
    double rmax = getMaxListRadius();
    if( onpe0 )
        (*MPIdata::sout)<<"Max. radius for species in XYZ file: "<<rmax<<endl;
    assert( rmax>0. );
    ...
    if( mmpi.PE0() )
    {
        ...
        if ( !(*tfile) )
        {
            (*MPIdata::serr)<< "Ions::readAtomsFromXYZ() --- ERROR: cannot open "<< filename << endl;
        }
        else
        {
            string query;
        ...
        if( natoms<1 )
            (*MPIdata::sout)<< "WARNING: Ions::readAtomsFromXYZ(), number of atoms read = "<< natoms << endl;
    }
    ...
    for(int ia=0;ia<natoms;++ia)
    {
        ...
        if (spname.compare("") == 0)
        {
            (*MPIdata::serr)<< "Ions::readAtomsFromXYZ() --- ERROR: unknown species for atomic number "<< spec[ia] << endl;
            return -1;
        }
        ...
        aname.append( ss.str() );
        Ion*  new_ion=new Ion(species_[isp],aname,&crds[3*ia],velocity,locked);
#ifdef USE_MPI
        new_ion->bcast(mmpi.commGlobal());    
#endif    

        // Populate list_ions_ list
        //cout<<"crds: "<<crds[3*ia+0]<<", "<<crds[3*ia+1]<<", "<<crds[3*ia+2]<<endl;
        if(inListIons(crds[3*ia+0], crds[3*ia+1], crds[3*ia+2]))
        {
           list_ions_.push_back(new_ion);
           if(onpe0 && ct.verbose>2 )
              (*MPIdata::sout)<<"Ion "<<aname<<" at position "
                              <<crds[3*ia+0]<<","<<crds[3*ia+1]<<","<<crds[3*ia+2]
                              <<" added to the list... on PE"<<mmpi.mypeGlobal() <<endl;
           // populate local_ions_ list
           if(inLocalIons(crds[3*ia+0], crds[3*ia+1], crds[3*ia+2]))
           {
              ...
              local_ions_.push_back(new_ion);
               if( onpe0 && ct.verbose>2 )
                 (*MPIdata::sout)<<"Ion "<<aname<<" at position "
                                 <<crds[3*ia+0]<<","<<crds[3*ia+1]<<","<<crds[3*ia+2]
                                 <<" added to the list of local ions... on PE"<<mmpi.mypeGlobal() <<endl;
           }
           else
        ...
        {
           //(*MPIdata::sout)<<"Ion "<<aname<<" NOT added to the list... on PE"<<mmpi.mype() <<endl;
           delete new_ion;
        }
    }
//    cout<<mmpi.mype()<<"...list size = "<<list_ions_.size()<<" local ions size = "<<local_ions_.size()<<endl;
...
int Ions::readNatoms(const string filename, const bool cell_relative)
{
    ...
    if( mmpi.instancePE0() )
    {
        ...
        if ( !tfile->is_open() )
        {
            (*MPIdata::serr) << " Unable to open file " << filename.data() << endl;
            return -1;
        }
        else
        {
...
int Ions::read1atom(ifstream *tfile, const bool cell_relative)
{
    ...
    if( mmpi.PE0() )
    {
        ...
        else
        {
            ...
            {
                (*MPIdata::serr)<<"ERROR: Ions::read1atom() --- getline() failed..."<<endl;
                count = -1;
            }
        }
    ...
    if( mmpi.PE0())
    {
        ...
        {
            cerr<<"ERROR: Invalid name read in input file: "<<name_read<<endl;
            ct.global_exit(2);
        }
        short dummy;
    ...
    double tmp[7]={crds[0],crds[1],crds[2],velocity[0],
...
#ifdef DEBUG
    if(onpe0)
        (*MPIdata::sout)<<"Create new Ion..."<<endl;
#endif
...
void Ions::setVelocities(const vector<double>& tau0, const vector<double>& taup, 
                         const double dt)
{
...
void Ions::setMapVL()
{
    ...
    const double origin[3]={mygrid.origin(0),
                            mygrid.origin(1),
                            mygrid.origin(2)};
...
void Ions::gatherNames(map<int,string>& names, const int root, const MPI_Comm comm)const
{
    assert( comm!=0 );
...
void Ions::gatherNames(vector<string>& names, const int root, const MPI_Comm comm)const
{
    assert( comm!=0 );
...
void Ions::gatherLockedNames(vector<string>& names, const int root, const MPI_Comm comm)const
{
    vector<string> local_names;
...
void Ions::gatherIndexes(vector<int>& indexes, const int root, const MPI_Comm comm)const
{
    vector<int> local_indexes;
...
void Ions::gatherNLprojIds(vector<int>& nlprojids, const int root, const MPI_Comm comm)const
{
...
void Ions::gatherAtomicNumbers(vector<int>& atnumbers, const int root, const MPI_Comm comm)const
{
    assert( comm!=0 );
...
void Ions::gatherRandStates(vector<unsigned short>& rstates, const int root, const MPI_Comm comm)const
{
...
void Ions::gatherPositions(vector<double>& positions, const int root, const MPI_Comm comm)const
{
...
void Ions::gatherForces(vector<double>& forces, const int root, const MPI_Comm comm)const
{
...
void Ions::gatherVelocities(vector<double>& velocities, const int root, const MPI_Comm comm)const
{
...
void Ions::setupListIonsBoundaries(const double rmax)
{
    ...
    // define extents to be at least size of local domain and at most the size of global domain
    ...
    for(short i=0; i<3; i++)
    {
         list_boundary_right_[i] = origin[i]+offset[i]+div_lattice_[i]+myrmax[i];  
    }
...
// the list of ions on this PE. This range is obtained from predetermined extents in 
// list_boundary_left[x,y,z] and list_boundary_right[x,y,z]. 
bool Ions::inListIons(const double x, const double y, const double z)
{
    ...
    if(((t[0]>=list_boundary_left_[0] && t[0]<=list_boundary_right_[0])
      ||((t[0]-lattice_[0])>=list_boundary_left_[0] && (t[0]-lattice_[0])<=list_boundary_right_[0])
      ||((t[0]+lattice_[0])>=list_boundary_left_[0] && (t[0]+lattice_[0])<=list_boundary_right_[0]))
      &&((t[1]>=list_boundary_left_[1] && t[1]<=list_boundary_right_[1])
      ||((t[1]-lattice_[1])>=list_boundary_left_[1] && (t[1]-lattice_[1])<=list_boundary_right_[1])
      ||((t[1]+lattice_[1])>=list_boundary_left_[1] && (t[1]+lattice_[1])<=list_boundary_right_[1]))
      &&((t[2]>=list_boundary_left_[2] && t[2]<=list_boundary_right_[2])
      ||((t[2]-lattice_[2])>=list_boundary_left_[2] && (t[2]-lattice_[2])<=list_boundary_right_[2])
      ||((t[2]+lattice_[2])>=list_boundary_left_[2] && (t[2]+lattice_[2])<=list_boundary_right_[2])))
         inList = true;
...
double Ions::getMaxListRadius()const
{
    ...
    Control& ct ( *(Control::instance()) );
    rmax = rmax > ct.maxDistanceAtomicInfo() ? rmax : ct.maxDistanceAtomicInfo();  
...
void Ions::augmentIonsData(const int nsteps, const int dir, const int disp, 
                           ...
                           vector<IonData>& iondata, int *offset)
{
    ...
    {
        // chars data
        MPI_Irecv(cbuff_r, cbuff_size_r, MPI_CHAR, src, 0, cart_comm_, &request1[0]);
        MPI_Irsend(cbuff, csize, MPI_CHAR, dst, 0, cart_comm_, &request1[1]);
        // integers data
        MPI_Irecv(ibuff_r, ibuff_size_r, MPI_INT, src, 0, cart_comm_, &request2[0]);
        MPI_Irsend(ibuff, isize, MPI_INT, dst, 0, cart_comm_, &request2[1]);
        // doubles data
        MPI_Irecv(dbuff_r, dbuff_size_r, MPI_DOUBLE, src, 0, cart_comm_, &request3[0]);
        ...
        *offset = (int)ions_data_.size();
        ...
        for(int i=0; i<rsize; i++)
        {
            ...
            if(inListIons(newdata.current_position[0], 
                          newdata.current_position[1], 
                          newdata.current_position[2]))
            {
...
void Ions::updateForcesInteractingIons()
{
    ...
    for(short dir = 0; dir <3; dir++)
    {
        ...
        // send right to left
        int disp = -1;
        forces_data.augmentData(lstep_[dir], dir, disp, lsize, maxsize, data2send);

        // send left to right
        disp = 1;
        forces_data.augmentData(rstep_[dir], dir, disp, lsize, maxsize, data2send); 
    }
    
    int ia=0;
    for(vector<string>::const_iterator it =interacting_names_.begin();
                                       it!=interacting_names_.end();
                                     ++it)
    {
...
void Ions::updateTaupInteractingIons()
{
    ...
    for(short dir = 0; dir <3; dir++)
    {
        ...
        // send right to left
        int disp = -1;
        taup_data.augmentData(lstep_[dir], dir, disp, lsize, maxsize, data2send);

        // send left to right
        disp = 1;
        taup_data.augmentData(rstep_[dir], dir, disp, lsize, maxsize, data2send); 
    }
...
{      
    ...
    for(short dir = 0; dir <3; dir++)
    {
        ...
        int maxsize = lsize;
        mmpi.allreduce(&maxsize, 1, MPI_MAX);
//if(onpe0)cout<<"lsize = "<<ions_data_.size()<<" list size = "<<list_ions_.size()<<endl;
        vector<IonData> data2send ( ions_data_ );

        // send right to left
        int disp = -1;
        augmentIonsData(lstep_[dir], dir, disp, lsize, maxsize, data2send, &offset);
//if(onpe0)cout<<"lsize2 = "<<ions_data_.size()<<" list size2 = "<<list_ions_.size()<<endl;
        // send left to right
        disp = 1;
        augmentIonsData(rstep_[dir], dir, disp, lsize, maxsize, data2send, &offset); 
//if(onpe0)cout<<"lsize3 = "<<ions_data_.size()<<" list size3 = "<<list_ions_.size()<<endl;      
    }
...
{
    if( inListIons(data.current_position[0], 
                   ...
                   data.current_position[2]) ) 
    {
        ...
        if(inLocalIons(data.current_position[0], 
                       data.current_position[1], 
                       data.current_position[2])) 
        {
