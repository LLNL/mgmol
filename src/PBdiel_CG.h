...
public:
    ...
    // Constructor
    PBdiel_CG(pb::Grid& grid, const short bc[3],
           const double e0, const double rho0, const double drho0):
           Poisson(grid,bc)
    {
        T oper(Poisson::grid_, e0, rho0, drho0);
        Poisson::vepsilon_=new pb::GridFunc<POTDTYPE>(Poisson::grid_,bc[0],bc[1],bc[2]);
        rhod_     =NULL;
        poisson_solver_=new PCGSolver_Diel<T, POTDTYPE>(oper, bc[0],bc[1],bc[2]);
    };
    ...
    void setup(const short nu1, const short nu2,
    ...
    void solve(const pb::GridFunc<RHODTYPE>& rho, const pb::GridFunc<RHODTYPE>& rhoc);
};
