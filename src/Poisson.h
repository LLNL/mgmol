...
public:
    // Constructor
    Poisson(const pb::Grid& grid, const short bc[3]):
            grid_(grid)
    {
        bc_[0]=bc[0];
        bc_[1]=bc[1];
    ...
    virtual void setup(const short nu1, const short nu2,
                       const short max_sweeps,
                       const double tol,
                       const short max_nlevels,
                       const bool gather_coarse_level=true)=0;

    void getVh(POTDTYPE* vh)
    {
        vh_->init_vect(vh,'d');
    }
    void getVepsilon(POTDTYPE* veps)
    {
        vepsilon_->init_vect(veps,'d');
    }
    ...
    void set_vh(const POTDTYPE* const vh){ 
        vh_->assign(vh, 'd');
    };
    void resetVh()
    {
    ...
    virtual void solve(const pb::GridFunc<RHODTYPE>& rho, const pb::GridFunc<RHODTYPE>& rhoc)=0;
    void computeVhRho(const pb::GridFunc<RHODTYPE>& rho, 
