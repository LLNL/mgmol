...
Timer PolakRibiereSolver::solve_tm_("solve");
int PolakRibiereSolver::it_scf_=0;

PolakRibiereSolver::PolakRibiereSolver(Hamiltonian* hamiltonian,
    ...
    os_(os)
{
    ...
    with_preconditioner_ = ( (ct.getPrecondType()%10)==0 && ct.getMGlevels()>=0 );
}

PolakRibiereSolver::~PolakRibiereSolver()
{
}

void PolakRibiereSolver::printEnergy(const short step)const
{   
    if ( onpe0 )
    {
...
bool PolakRibiereSolver::checkPrintResidual(const short step)const
{
    Control& ct(*(Control::instance()));
    return (ct.iprint_residual>0) ? !(step%ct.iprint_residual) : false;
}

void PolakRibiereSolver::dielON()
{
...
bool PolakRibiereSolver::testUpdatePot()const
{
    Control& ct(*(Control::instance()));
    return ( it_scf_>ct.max_changes_pot );
}

bool PolakRibiereSolver::checkConvPot()const
{
    Control& ct(*(Control::instance()));
    Potentials& pot(hamiltonian_->potential());
...
bool PolakRibiereSolver::checkWolfeConditions(const double trial_step_energy, const double alpha_k)const
{
    assert( sigma_a_>0. );
    assert( sigma_b_>0. );
    
    //const double dk  =-1.*r_k_->dotProduct(*p_k_);
    const double dkm1=-1.*r_km1_->dotProduct(*p_k_,2); // inverse(S) already included in r_km1_
    
    const bool wolfe0 = ( trial_step_energy <= eks_history_[0]+sigma_a_*alpha_k*dkm1 );
    ...
    if( !wolfe0 && onpe0 )
    {
        ...
        os_<<"trial_step_energy                     ="<<trial_step_energy<<endl;
        os_<<"eks_history_[0]="<<eks_history_[0]<<endl;
        os_<<"eks_history_[0]+sigma_a_*alpha_k*dkm1="<<eks_history_[0]+sigma_a_*alpha_k*dkm1<<endl;
    }
...
int PolakRibiereSolver::checkConvergenceEnergy(const short step, const short max_steps)
{
    Control& ct ( *(Control::instance()) );
...
double PolakRibiereSolver::evaluateEnergy(const LocGridOrbitals& orbitals, const bool print_flag)
{
    // Get the new total energy
    const double ts=0.5*proj_matrices_->computeEntropy(); // in [Ha]
    const double energy=energy_->evaluateTotal(ts, proj_matrices_, orbitals, print_flag, os_);
...
// Polak-Ribiere
double PolakRibiereSolver::computeBeta(LocGridOrbitals& work_orbitals)const
{
...
int PolakRibiereSolver::solve(LocGridOrbitals& orbitals,
         ...
         double& last_eks)
{
    ...
    for(short step = 0;step <= max_steps;step++)
    {
        ...
        if( dm_success==0 ) // if DM determined at previous step is good
        {
            ...
            double normRes=mgmol_strategy_->computeResidual(orbitals,work_orbitals,*r_k_,
                (print_res ||ct.checkResidual()),
                ct.checkResidual() );
            ...
            //SquareLocalMatrices<MATDTYPE>& localX( proj_matrices_->getLocalX() );
            //r_k_->multiplyByMatrix(localX);
        ...
        if( wolfe && dm_success==0 ) // last step was accepted
        {
            ...
            {
                if( onpe0 )os_<<endl<<endl<<" PolakRibiereSolver: convergence achieved for delta E..."<<endl;
                break;
            }
            ...
            else
            {
                ...
                if(beta<0.)beta=0.;
                //const double beta=0.;
                if( onpe0 && ct.verbose>1 )os_<<" PolakRibiereSolver: beta="<<beta<<endl;
                p_k_->scal(beta);
                p_k_->axpy(1.,*z_k_);
        ...
        {
            alpha_k*=0.5;
            if( onpe0 )os_<<"PolakRibiereSolver: Wolfe conditions not satisfied, reduce alpha to "<<alpha_k<<"..."<<endl;
        ...
        {
            bool wannier_pairs=mgmol_strategy_->rotateStatesPairsOverlap(orbitals, work_orbitals,
                                                                         ct.getThresholdEigenvalueGramQuench());
        
            if( wannier_pairs )
            {
                orbitals.computeGramAndInvS();
                // reset reference energy since rotation may be too much of a perturbation
                eks_history_[0]=1.e8; 
                eks_history_[1]=1.e8; 
                if( onpe0 )os_<<"PolakRibiereSolver: reset reference energy"<<endl;
            }
        }
