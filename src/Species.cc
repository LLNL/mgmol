...
void Species::read_1species(const string& filename)
{
    ...
    if( mmpi.instancePE0() )
    {
        ...
        if(type_flag_==1)(*MPIdata::sout)<<" Divide ref. state by radius"<<endl;
        if(type_flag_==2)(*MPIdata::sout)<<" Divide projector by radius"<<endl;
#endif
    ...
    if( mmpi.instancePE0() )
    {
            ...
            for(short l=0;l<num_potentials_;l++)
            {
                ...
                if( l!=llocal_ )
                {
                    ...
                    for(short p=0;p<nproj;++p)
                    {
                        ...
                        ss>>ekb_[ll][p];
                        cout<<"Read ekb_["<<ll<<"]["<<p<<"]="<<ekb_[ll][p]<<endl;
                    }
                }
                else
                {
        ...
        else // single KB projectors
        {
            ...
            {
                assert( l<multiplicity_.size() );
                if( l!=llocal_ )multiplicity_[l]=1;
                else            multiplicity_[l]=0;
            }
...
void Species::gauss_filter_local_pot(const double rcut,
                                     const bool printFlag)
{
    if( printFlag )
        (*MPIdata::sout)<<"Filter local potential"<<endl;
    local_pot_.gauss_filter(rcut);
}

void Species::gauss_filter_kbp(const short l, const short p, const double rcut,
                               const bool printFlag)
{
    if( printFlag )
        (*MPIdata::sout)<<"Filter non-local potential"<<endl;
    assert( l<kbp_.size() );
...
void Species::initLocalPotential(const bool flag_filter,
                                 const double hmax, 
                                 ofstream *tfile,
                                 const bool printFlag)
{
    ...
    {
        (*tfile)<<"# "<<name_<<", neutralized potential, l="<<llocal_<<endl;
    ...
    if( flag_filter )
    {
        ...
        if( printFlag && (tfile!=NULL))
        {
            if( ct.verbose>0 )cout<<"Write local potential in file..."<<endl;
...
void Species::initPotentials(const bool flag_filter,
                             const double hmax,
                             const bool printFlag)
{
    ...
    if( printFlag )
    {
        ...
        if( ct.verbose>0 ) (*MPIdata::sout)<<"Initialize radial potentials for species "<<name_<<" on MPI rank "<<mpi_rank<<endl;
    ...
    if(type_flag_==2)
        initNonlocalMultiProjectorsPotentials(flag_filter,hmax,tfile,true,printFlag);
    else if(type_flag_==3)
        initNonlocalMultiProjectorsPotentials(flag_filter,hmax,tfile,false,printFlag);
    else if( h1s_==0. )
                                       ...
                                       const bool printFlag)
{
    ...
    for(short ll = 0;ll <= max_l_;ll++)
    {
        ...
        { 
            ...
            if( printFlag && (tfile!=NULL))
            {
                (*tfile)<<"# "<<name_<<" filtered projector, l="<<ll<<endl;
                print_kbp(*tfile,ll);
            } 
                                                    ...
                                                    const bool printFlag)
{
    ...
    if( printFlag && ct.verbose>2 )
    {
        (*MPIdata::sout)<<"Species::initNonlocalMultiProjectorsPotentials()"<<endl;
    }
    ...
    for(short ll = 0;ll <= max_l_;ll++)
    {
        if(ll != llocal_)
        {
            ...
            { 
                ...
                //if( printFlag ) (*MPIdata::sout)<<"Copy radial projector"<<endl; 
                for(int idx = 0;idx < n_rad_points_;idx++)
                {
                    work[idx] = pot[idx];
                } 
                ...
                if( printFlag && (tfile!=NULL))
                {
                    (*tfile)<<"# "<<name_<<", projector, l="<<ll<<", p="<<p<<endl;
                    // output non-local projector
                    for(int idx = 0;idx < n_rad_points_;idx+=3)
                    {
                ...
                if( flag_filter )
                {
                    ...
                    // Now cut it off smoothly in real space 
                    gauss_filter_kbp(ll,p,nlrcut,(printFlag && ct.verbose>2));
                }
                else
                {
                ...
                {
                    (*tfile)<<"# "<<name_<<" filtered, l="<<ll<<", p="<<p<<endl;
                    print_kbp(*tfile,ll);
                } 
                
                // Evaluate the normalization coefficient for the projector
                //double  norm2 = rwork.radintf2();
                //if( printFlag )cout<<"Normalization factor for projector l="<<ll<<", p="<<p<<": "<<norm2<<endl;
                //kbp_[ll][p].scale(sqrt(1./norm2));
                if( printFlag )cout<<"Norm2 radial KB projector="<<kbp_[ll][p].norm2()<<endl;
...
void Species::syncKBP(const int root)
{
    ...
    if( l!=llocal_ )
    {
        ...
        MPI_Bcast(&kb_coeff_[l][0], multiplicity_[l], MPI_DOUBLE, root, comm_);
        MPI_Bcast(&kb_sign_[l][0], multiplicity_[l], MPI_SHORT, root, comm_);
