...
template <typename T>
Preconditioning<T>::Preconditioning(const short lap_type, 
                                 const short maxlevels,
                                 const pb::Grid& grid,
                                 const short bc[3])
{
    max_levels_=maxlevels;
    lap_type_  =lap_type;
    for(short i=0;i<3;i++)bc_[i]=bc[i];
...
Preconditioning<T>::Preconditioning(const Preconditioning& precond)
{
    ...
    grid_.push_back(mygrid);
    for(short i=0;i<3;i++)bc_[i]=precond.bc_[i];    
}
...
void Preconditioning<T>::mg(pb::GridFuncVector<T>& gfv_v, 
                            const pb::GridFuncVector<T>&  gfv_f,
                            const short level)
{
#ifdef PRINT_OPERATIONS
    if( onpe0 )(*MPIdata::sout)<<"Preconditioning::mg() at level "<<level<<endl;
#endif
    short ncycl=2;
...
template <typename T>
void Preconditioning<T>::app_mask(pb::GridFuncVector<T>& gvu, 
...
template <typename T>
void Preconditioning<T>::app_mask(pb::GridFunc<T>& gu, 
                               const short level,
