...
public:
  ...
  explicit Vector3D(const double xv = 0, const double yv = 0, const double zv = 0)
  {
    x_[0]=xv;
    x_[1]=yv;
    x_[2]=zv;
  ...
  const double& operator[](const short i)const
  {
    return x_[i];
  }
  ...
  {
    return fabs(x_[0]-rhs.x_[0])<tol 
        && fabs(x_[1]-rhs.x_[1])<tol 
  ...
  double minimage ( const Vector3D& w, const Vector3D& l, const short bc[3])const;
  Vector3D vminimage( const Vector3D& w, const Vector3D& l, const short bc[3] )const;
  ...
  friend Vector3D normalized( const Vector3D a )
  {
    return a / length( a );
  }
