...
public:
  ...
  MLWFTransform(const int nst, const Vector3D& origin, 
                               const Vector3D& ll):
      OrbitalsTransform(nst, origin, ll)  
  {
    ...
    if( nst>0 )
    {
      a_=new dist_matrix::DistMatrix<DISTMATDTYPE>("matrixA",*bcr_,nst_,nstcol_,nst_,bsize_);
      ...
      for ( int k = 0; k < 2*NDIM; k++ ){
        r_[k]=new dist_matrix::DistMatrix<DISTMATDTYPE>(names[k],*bcr_,nst_,nstcol_,nst_,bsize_);
      }
    }
  };
