...
int FIRE_IonicStepper::init(HDFrestart& h5f_file)
{
    ...
    if( file_id>=0 && onpe0 ){
        ...
        if( dataset_id<0 ){
            if( onpe0 ){
                (*MPIdata::sout)<<warning_msg<<"H5Dopen failed for /Ionic_velocities"<<endl;
                (*MPIdata::sout)<<"Set velocities to zero"<<endl;
            }
            memset(&taup_[0],0,taup_.size()*sizeof(double));
        }else{
            if( onpe0 )(*MPIdata::sout)<<"Read Ionic velocities from "
                           <<h5f_file.filename()<<endl;
            herr_t status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, 
                                    H5S_ALL, H5S_ALL,
                                    H5P_DEFAULT, &taup_[0]);
            if( status<0 ){
            ...
            if( taup_[0]!=taup_[0] )
            {
                (*MPIdata::serr)<<error_msg<<"taup_[0]="<<taup_[0]<<endl;
                return -1;
            }
        }
...
int FIRE_IonicStepper::run()
{
    ...
    for ( int ia = 0; ia < na; ia++ ){
        if ( atmove_[ia] ){
            for ( int j = 0; j < 3; j++ ){
                pp += (tau0_[3*ia+j] - taum_[3*ia+j]) * fion_[3*ia+j];
                normf2 += fion_[3*ia+j]*fion_[3*ia+j];
                normv2 += (tau0_[3*ia+j] - taum_[3*ia+j])*(tau0_[3*ia+j] - taum_[3*ia+j]);
            }
        }
    }
    assert( pp==pp );
    ...
    if( pp<=0. )
    {
        ...
        alpha_=alpha_init_;
        // freeze system
        if( onpe0 )(*MPIdata::sout)<<"FIRE_IonicStepper: freeze system..."<<endl;
        if( onpe0 )(*MPIdata::sout)<<"FIRE_IonicStepper: new dt   ="<<dt_<<endl;
        for ( int ia = 0; ia < na; ia++ ){
             for ( int j = 0; j < 3; j++ ){
                taup_[3*ia+j] = 0.;
            }
        ...
        for ( int ia = 0; ia < na; ia++ ){
            if ( atmove_[ia] ){
                for ( int j = 0; j < 3; j++ ){
                    taup_[3*ia+j] = (1.-alpha_)*(tau0_[3*ia+j] - taum_[3*ia+j])
        ...
        if( npp_>nmin_ )
        {
            ...
            alpha_*=falpha_;
            if( onpe0 )(*MPIdata::sout)<<"FIRE_IonicStepper: new dt   ="<<dt_<<endl;
            if( onpe0 )(*MPIdata::sout)<<"FIRE_IonicStepper: new alpha="<<alpha_<<endl;
        }
    }
...
double FIRE_IonicStepper::etol(void) const
{
    return 0.0;
}
