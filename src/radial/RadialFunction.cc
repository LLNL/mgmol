...
void RadialFunction::read(const int nrow, const int ncol, 
                          ifstream *tfile)
{
    assert( ncol>1 );
    assert( nrow>1 );
...
double RadialFunction::radint(const int j)
{
    ...
    if(f[n-1]*x_[n-1]*x_[n-1]>5.e-3)
    {
        ...
        (*MPIdata::sout)<<"WARNING radint(): f[n-1]*x_[n-1]*x_[n-1]="<<f[n-1]*x_[n-1]*x_[n-1]<<endl;
...
void 
RadialFunction::inv_ft(const vector<double>& rgrid, 
                       vector<double>& rfunc, 
                       const int lval,
                       const int j)
{
...
#ifdef DEBUG
    if( onpe0 )
        (*MPIdata::sout)<<" Inv. FT for l="<<lval<<endl;
#endif
    ...
    switch(lval) {
        ...
        default:
            ...
            (*MPIdata::serr)<<"invft: angular momentum state > 2 not implemented"<<endl;
            exit(1);
...
void 
RadialFunction::compute_ft(const vector<double>& ggrid, 
                           vector<double>& gcof, 
                           const int lval,
                           ofstream* tfile,
                           const int j)
        ...
        switch(lval)
        {
            ...
            case 3:
             ...
             default:
                 ...
                 (*MPIdata::serr)<<"rft: angular momentum "<<lval
                     <<" not implemented\n";
                 exit(1);
...
void 
RadialFunction::rft(RadialFunction& filt_func, 
                    const int lval,
                    const double hmax,
                    const bool print,
                    const int j,
                    const bool printFlag)
{
    ...
    if( printFlag && ct.verbose>0 )
        (*MPIdata::sout)<<"RadialFunction::rft(),"
            <<" Cutoff for pseudopotential: "<<gcut*gcut<<"[Ry]"<<endl;
...
void RadialFunction::printLocalPot(const string& name, const short ll, 
                                   ofstream *tfile)
{
    assert( tfile!=0 );
