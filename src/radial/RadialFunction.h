...
public:
    ...
    RadialFunction(const std::vector<double>& x, const std::vector<std::vector<double> >& y)
    {
        drlin_=x[1]-x[0];
        assert( drlin_ >1.e-15 );
        invdr_=1./drlin_;
    ...
    void assign(const std::vector<double>& x, const std::vector<std::vector<double> >& y)
    {
        assert( x.size()==y[0].size() );
        x_=x;
        y_=y;
    ...
    void assign(const std::vector<double>& y, const double drlin, const int j=0)
    {
        drlin_=drlin;
        assert( drlin_ >1.e-15 );
        invdr_=1./drlin_;
    ...
    void inv_ft(const std::vector<double>& rgrid, 
                std::vector<double>& rfunc, 
                const int lval,
                const int j=0);
    void compute_ft(const std::vector<double>& ggrid, 
                    std::vector<double>& gcof, 
                    const int lval,
                    std::ofstream* tfile,
                    const int j=0);
    void rft(RadialFunction& filt_func, 
             const int lval,
             const double hmax,
             const bool,
             const int j=0,
    ...
    void printLocalPot(const std::string& name, const short ll, std::ofstream *tfile);
};
