...
////////////////////////////////////////////////////////////////////////////////
void MLWFTransform::setia(vector<int>& iiu)
{
  iu_=iiu;
}
////////////////////////////////////////////////////////////////////////////////
void MLWFTransform::compute_transform(const int maxsweep, const double tol)
{
  ...
  if( bcr_->onpe0() )
  if ( delta > tol )
    (*MPIdata::sout) << " MLWFTransform: decrease was " << setprecision(12) << delta
  ...
  if( mype<npecol ){
    ...
    while ( isweep < maxsweep && delta > tol )
    {
      ...
      for (int round=0; round<nn-1; round++)
      {
          ...
          //if( iu_[rp+nst_*rq] )
          {
...
#if DEBUG
            (*MPIdata::sout)<<"compute actual pair: "<<rp<<", "<<rq<<endl;
#endif
            ...
            for ( int k = 0; k < m; k++ )
            {
              ...
              if(rp<nst_ && rq<nst_){
                ...
                if( !(fabs(r_loc[k][rp+nst_*cq]-r_loc[k][rq+nst_*cp])<0.00001) )
                {
                    (*MPIdata::sout)<<"mype="<<mype<<", k="<<k<<endl;
                    (*MPIdata::sout)<<"mype="<<mype<<", r_loc: rp="<<rp<<", cq="<<cq<<", "<<r_loc[k][rp+nst_*cq]<<endl;
                    (*MPIdata::sout)<<"mype="<<mype<<", r_loc: rq="<<rq<<", cp="<<cp<<", "<<r_loc[k][rq+nst_*cp]<<endl;
                }
                assert(fabs(r_loc[k][rp+nst_*cq]-r_loc[k][rq+nst_*cp])<0.00001);
              }
            ...
            // update the orthogonal transformation u with the 2*2 rotation R(p,q)
            //
            //           |  c   s |
            //  R(p,q) = |        |
            //           | -s   c |
...
#endif
            int ione=1;
            drot(&nst_, &u_loc[ncp], &ione, &u_loc[ncq], &ione, &c, &s);
            ...
            for ( int k = 0; k < m; k++ )
            {
              // A := A * R(p,q)^T
              int ione=1;
              drot(&nst_, &r_loc[k][ncp], &ione, &r_loc[k][ncq], &ione, &c, &s);
            }
        ...
        for(int pe=0;pe<npecol;pe++){
          ...
          for (int ip=0; ip<n_loc_2; ip++)
          {
            ...
            //if( iu_[rowp+nst_*rowq] )
            {
              ...
              {
                // A := R(p,q) * A
                drot(&n_loc, &r_loc[k][rowp], &nst_, &r_loc[k][rowq], &nst_, &c, &s);
              }
            }
          } // ip
        ...
        if(mype!=npecol-1){
          for ( int k=0; k<m; k++ )
            memcpy(&sbuf1[k*nst_], &r_loc[k][column[(n_loc_2-1)]*nst_], 
                   nst_*sizeof(DISTMATDTYPE) );
          memcpy(&sbuf1[m*nst_], &u_loc[column[(n_loc_2-1)]*nst_], nst_*sizeof(double) );
        }
        // copy column n_loc-1 into buffer to transfer left
        if(mype!=0){
          for ( int k=0; k<m; k++ )
            memcpy(&sbuf2[k*nst_], &r_loc[k][column[(n_loc-1)]*nst_], nst_*sizeof(double) );
          memcpy(&sbuf2[m*nst_], &u_loc[column[(n_loc-1)]*nst_], nst_*sizeof(double) );
        }
        ...
        if (mype!=npecol-1)
        {
          MPI_Irecv(&rbuf2[0], (m+1)*nst_, MPI_DOUBLE, destright, destright, 
                    comm_, req);
          MPI_Irecv(&actual[column[(n_loc_2-1)]], 1, 
                    MPI_INT, destright, destright+npecol, comm_, req+4);
        }

        if(mype!=0)
        {
          MPI_Irecv(&rbuf1[0], (m+1)*nst_, MPI_DOUBLE, destleft, destleft, 
                    comm_, req+1);
          MPI_Irecv(&actual[column[(n_loc-1)]], 1, 
                    MPI_INT, destleft, destleft+npecol, comm_, req+5);

        }
        
        if(mype!=0)
        {
          MPI_Isend(&sbuf2[0], (m+1)*nst_, MPI_DOUBLE, destleft, mype, 
                    comm_, req+2);
          MPI_Isend(&scoll, 1, MPI_INT, destleft, mype+npecol, 
                    comm_, req+6);
          //(*MPIdata::sout)<<"mype="<<mype<<", send col left "<<scoll<<endl;
        }

        if(mype!=npecol-1)
        {
          MPI_Isend(&sbuf1[0], (m+1)*nst_, MPI_DOUBLE, destright, mype, 
                    comm_, req+3);
          MPI_Isend(&scolr, 1, MPI_INT, destright, mype+npecol, 
                    comm_, req+7);
          //(*MPIdata::sout)<<"mype="<<mype<<", send col right "<<scolr<<endl;
        }    
        ...
        {
          if (mype!=npecol-1)
            memcpy(&r_loc[k][column[(n_loc_2-1)]*nst_], &rbuf2[k*nst_], 
                   nst_*sizeof(double) );
          if (mype!=0)
            memcpy(&r_loc[k][column[(n_loc-1)]*nst_], &rbuf1[k*nst_], 
                   nst_*sizeof(double) );
        }
        if (mype!=npecol-1)
          memcpy(&u_loc[column[(n_loc_2-1)]*nst_], &rbuf2[m*nst_], nst_*sizeof(double) );
        if (mype!=0)
          memcpy(&u_loc[column[(n_loc-1)]*nst_],&rbuf1[m*nst_], nst_*sizeof(double) );
#endif
      ...
      for ( int k = 0; k < m; k++ ){
        ...
        { 
          ...
          if (rp<nst_)
          {
            sum += r_loc[k][rp+nst_*ip]*r_loc[k][rp+nst_*ip];
            for ( int i = 0; i < nst_; i++ )
              sum_off += r_loc[k][i+nst_*ip] * r_loc[k][i+nst_*ip];
            sum_off -= r_loc[k][rp+nst_*ip] * r_loc[k][rp+nst_*ip];
          }
        }
      }
    ...
    for ( int ip = 0; ip < n_loc; ip++ )
    {
      ...
      if( !( actual[ip]<(mype+1)*n_loc || actual[ip]==-1 ) ){
        (*MPIdata::sout)<<"mype="<<mype<<", ip="<<ip<<", actual[ip]="<<actual[ip]<<endl;
      }
      ...
      if ( actual[ip] < nst_ )
      {
        ...
        for ( int k = 0; k < m; k++ ){
          assert( ip<r_[k]->nb() );
          r_[k]->assignColumn(&r_loc[k][nst_*ip],actual[ip]-offset_);
        }
      }
    }
  ...
  for ( int pe = 0; pe < npecol; pe++ )
  {
    ...
    for ( int ip = 0; ip < n_loc; ip++ )
    {
      ...
      if ( lip < nst_ )
      {
        if ( mype == root )
        {
          memcpy(&mat_[nst_*lip], &u_loc[nst_*ip], nst_*sizeof(double) );
        }
#ifdef SCALAPACK
