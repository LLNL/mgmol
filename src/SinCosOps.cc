...
void SinCosOps::compute(const LocGridOrbitals& orbitals,
...
void SinCosOps::computeSquare(const LocGridOrbitals& orbitals,
                              vector<vector<double> >& a)
{
    assert( a.size()==6 );
    for(short i=0;i<6;i++)
            ...
            for(int jstate=0;jstate<=icolor;jstate++){
                ...
                if(j!=-1){
                    ...
                    for(int iz = 0;iz < dim2;iz++) {
                        ...
                        double  alpha=orbitals.psi(jstate)[index]
                                ...
                                const int dim_index)
{
    ...
    const double inv_2pi=0.5*M_1_PI;
    const double alphax=grid.ll(dim_index)*grid.ll(dim_index)
                        *inv_2pi*inv_2pi;
    ...
    for(int ic=0;ic<2;ic++)
    {
        ...
        for(short iloc=0;iloc<orbitals.subdivx_;iloc++){
            ...
            if( orbitals.overlapping_gids_[iloc][mycolor]==st[ic] ){
                ...
                if( color_st[jc]>=0 )
                {
                    if( orbitals.overlapping_gids_[iloc][color_st[jc]]==st[jc] ){
                        const ORBDTYPE* const ppsij=orbitals.psi(color_st[jc]);
                        assert( ppsij!=0 );
                        ...
                        vector<vector<double> >& a)
{
    ...
    for(short iloc=0;iloc<orbitals1.subdivx_;iloc++){
        ...
        for(int color=0;color<orbitals1.chromatic_number_;color++){
            ...
            if(i!=-1)
            for(int jstate=0;jstate<orbitals2.chromatic_number_;jstate++){
                ...
                if(j!=-1){
                    ...
                    for(int iz = 0;iz < dim2;iz++) {
                        ...
                        const int    index=ix*incx+iy*incy+iz;
                        const double alpha=(double)orbitals1.psi(color)[index]
                                          *(double)orbitals2.psi(jstate)[index];
                        atmp[0]+=alpha*cosx[ix];
                        atmp[1]+=alpha*sinx[ix];
                        atmp[2]+=alpha*cosy[iy];
                        atmp[3]+=alpha*siny[iy];
        ...
        for(short icolor=0;icolor<size;icolor++)
        {
            ...
            if(gid!=-1)
            {
                ...
                for(int iz = 0;iz < dim2;iz++)
                {
                    ...
                    const int    index=ix*incx+iy*incy+iz;
                    const double alpha=(double)psii[index]
                                      *(double)psii[index];
                    atmp[0]+=alpha*cosx[ix];
                    atmp[1]+=alpha*sinx[ix];
                    atmp[2]+=alpha*cosy[iy];
