...
void noMoreMemory()
{
    cerr<<"Unable to satisfy request for memory for MPI task "<<mype<<endl;
...
void setSparseDistMatriConsolidationNumber(const int npes)
{
    ...
    if( remainder>0 )
    for(int i=consolidation_number-2;
            i<consolidation_number+8;
            i++)
    {
        if( (npes%i)==0 ){
    ...
    dist_matrix::SparseDistMatrix<DISTMATDTYPE>::setConsolidationNumber(consolidation_number);
    
    if( onpe0 )
        dist_matrix::SparseDistMatrix<DISTMATDTYPE>::printConsolidationNumber(*MPIdata::sout);
}
...
void arrayops(const double* const a, const double* const b, 
              const double s, const double e, const int dim,
              double* result)
{
    for (int i = 0; i< dim; i++){
...
{
    cerr<<"Function "<<name<<" not implemented and should not be called!!!"<<endl;
    MPI_Finalize();
    exit(0);
...
double minQuadPolynomial(const double e0,
                         const double e1,
                         const double de0,
...
double minQuadPolynomialFrom3values(const double e0,
                         const double e1,
                         const double ehalf,
                         const bool print_flag,
                         ostream& os)
{
    ...
    {
        if( e1<e0 )return 1.;
        else       return -1.; //failed to find minimum
    }
