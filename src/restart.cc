...
int MGmol::read_rho_and_pot_hdf5(HDFrestart& file, 
                          Rho& rho)
{
...
int MGmol::write_hdf5(const string filename, 
...
int MGmol::write_hdf5(HDFrestart& h5f_file, 
                vector< vector<RHODTYPE> >& rho, 
                Ions& ions,
                LocGridOrbitals& orbitals,
                LocalizationRegions& lrs)
{
    ...
    if( ct.out_restart_info>1 )
    {
        ...
        int ierr = h5f_file.write_1func_hdf5(pot.vtot(),"Vtotal",&ll[0],&origin[0]);
        if( ierr<0 )return ierr;

        // Write the hartree potential
        ierr = h5f_file.write_1func_hdf5(pot.vh_rho(),"Hartree",&ll[0],&origin[0]);
        if( ierr<0 )return ierr;

        // Write 
        if( ct.diel ){
            ierr = h5f_file.write_1func_hdf5(pot.vepsilon(),"VDielectric",&ll[0],&origin[0]);
        }
        if( ierr<0 )return ierr;
        
        // Write the Density
        ierr = h5f_file.write_1func_hdf5(&rho[0][0],"Density",&ll[0],&origin[0]);
        if( ierr<0 )return ierr;

        // Write external potential
        ierr = h5f_file.write_1func_hdf5(pot.vext(),"Vext",&ll[0],&origin[0]);
        if( ierr<0 )return ierr;
    }
...
int MGmol::read_restart_lrs(HDFrestart& h5f_file, 
                     const string& dset_name)
{
    Control& ct = *(Control::instance());
    if( ct.verbose>0 )
        printWithTimeStamp("MGmol::read_restart_lrs()...",(*MPIdata::sout));
...
int MGmol::read_restart_data(HDFrestart& h5f_file, 
                      Rho& rho, 
                      LocGridOrbitals& orbitals)
{
    Control& ct = *(Control::instance());
    if( ct.verbose>0 )printWithTimeStamp("MGmol::read_restart_data()...",(*MPIdata::sout));
    ...
    if( ct.restart_info>2 )
    {
        ...
        if( ierr<0 )
        {
            (*MPIdata::serr)<<"MGmol::read_restart_data(): error in reading LocGridOrbitals"<<endl;
            return ierr;
        }
    }
