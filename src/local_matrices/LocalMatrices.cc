...
template <class T>
void LocalMatrices<T>::syrk(const int iloc, const int k,
          const double* const a, const int lda)
{
    ...
    const char trans='t';
    MPsyrk(uplo, trans, m_, k, one, a, lda,
          zero, ssiloc, m_);
...
// This one is for debug purposes, and can be removed if unused
template <class T>
void LocalMatrices<T>::gemm(const int iloc, const int ma,
          const double* const a, const int lda,
...
template <class T>
void LocalMatrices<T>::gemm(const char transa, const char transb, const double alpha,
                            const LocalMatrices& matA, const LocalMatrices& matB, const double beta)
{
    ...
    for(short iloc=0;iloc<subdiv_;iloc++)
    {
        ...
        MPgemm(transa, transb, m_, n_, nca, alpha, amat, lda, bmat, ldb, beta, c, m_);
    }
}
            ...
            for(short iloc=0;iloc<subdiv_;iloc++)
            {
                ...
                if(st1!=-1)
                {
                    ...
                    if(st2!=-1)
                    {
                        ...
                        const T tmp=ssiloc[jcolor*chromatic_number+icolor];
...
template <class T>
void LocalMatrices<T>::fillDistMatrix(dist_matrix::DistMatrix<DISTMATDTYPE>& dst,
                                   const vector<vector<int> >& global_indexes,
                                   const double tol)const
{
    //if(onpe0)(*MPIdata::sout)<<"LocGridOrbitals::add_matrix_elements() for object "
    //             <<mat.name()<<endl;
    ...
    dist_matrix::SparseDistMatrix<DISTMATDTYPE> sm(comm,dst,remote_tasks_DistMatrix_,
                                             sparse_distmatrix_nb_tasks_per_partitions_);
#else
    dist_matrix::SparseDistMatrix<DISTMATDTYPE> sm(0,dst,remote_tasks_DistMatrix_);
#endif
                         ...
                         const dist_matrix::SubMatricesIndexing<DISTMATDTYPE>& submat_indexing)
{
    ...
    dist_matrix::SubMatrices<DISTMATDTYPE> submat("submat",global_indexes,
                                 comm,
                                 mat,
                                 submat_indexing);
    submat.gather( mat );
    ...
    for(short iloc=0;iloc<subdiv_;iloc++)
    {
        ...
        for(short icolor=0;icolor<chromatic_number;icolor++)
        for(short jcolor=0;jcolor<chromatic_number;jcolor++)
            local_mat[icolor+chromatic_number*jcolor]= ( T ) submat.val(icolor,jcolor,iloc);
    }
}
...
template <class T>
void LocalMatrices<T>::setMaskThreshold(const T min_threshold,
