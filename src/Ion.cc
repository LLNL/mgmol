...
void Ion::set_lstart(const int i,
                     const double cell_origin, const double hgrid)
{
...
double Ion::minimage(const Ion& ion2, 
                     const double cell[3],
                     const short periodic[3],
                     double xc[3])const
{
    for( short i=0;i<3;i++){
        assert(cell[i]>0.);
        ...
        if( periodic[i] ){
            ...
            if( xc[i] > half_lattice )xc[i]-=cell[i];
            else if( xc[i] < -half_lattice )xc[i]+=cell[i];
...
double Ion::distance(const Ion& ion2, 
                     double *xc, double *yc, double *zc)const
{
...
double Ion::distance(const Ion& ion2,
                     const double ax, const double ay, const double az)const
{
    assert(ax>0.);
    assert(ay>0.);
    assert(az>0.);
...
double Ion::minimage(const double point[3],
                     const double cell[3],
                     const short periodic[3])const
{
    double xc[3];
...
{
     // random state 
     setRandomState(data.rand_state[0],
                    data.rand_state[1],
                    data.rand_state[2]);  
     // previous position
     setOldPosition(data.old_position[0],
                    data.old_position[1],
                    data.old_position[2]);
     // velocity
...
void Ion::getKBcoeffs(vector<double>& coeffs)
{
...
double Ion::energyDiff(Ion& ion, const double lattice[3], const short bc[3])const
{
    const double r = minimage(ion, lattice, bc);
    assert( r>0. );
    
    const double t1 = sqrt(    getRC() * getRC() +
                           ion.getRC() * ion.getRC() );
