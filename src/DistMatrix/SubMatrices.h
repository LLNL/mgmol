...
class SubMatrices
{
  ...
  public:
    ...
    static Timer gather_tm()
    {
        return gather_tm_;
    }  

    static Timer gather_comp_tm()
    {
        return gather_comp_tm_;
    }  

    static Timer gather_comm_tm()
    {
        return gather_comm_tm_;
    }  

    SubMatrices<T>(const string& name,
        const vector<vector<int> >& indexes, 
        MPI_Comm comm,
        const DistMatrix<T>& mat,
        const SubMatricesIndexing<T>& );
