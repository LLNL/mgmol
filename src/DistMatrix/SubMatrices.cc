...
// $Id: SubMatrices.C,v 1.16 2010/11/24 21:06:00 jeanluc Exp $
...
SubMatrices<T>::SubMatrices(
    ...
    MPI_Comm comm,
    const DistMatrix<T>& mat,
    const SubMatricesIndexing<T>& submat_indexing):
        object_name_(name),
        comm_(comm),
        submat_indexing_(submat_indexing)
{
#if USE_MPI
...
TEMP_DECL
void SubMatrices<double>::gather(const DistMatrix<double>& mat)
{      
#ifdef DEBUG
   if( mype_==0 )
      cout<<"SubMatrices<double>::gather()"<<endl;
#endif
   gather_tm_.start();
   ...
   for(int pe=1;pe<npes_;pe++)
       remote_displ[pe]=(type_displ)(submat_indexing_.getRemoteSize(pe-1)+remote_displ[pe-1]);
   vector<type_displ> my_displ(npes_distmat_,0);
   for(int pe=1;pe<npes_distmat_;pe++)
       my_displ[pe]=(type_displ)(submat_indexing_.getMySize(pe-1)+my_displ[pe-1]);

   // build buffer for data array to send
   vector<double> buf_remote_val(tot_remote_size,0.);
   if(send_)
   ...
   vector<double> buf_my_val(my_size,0.);
...
//program enter in the next MPI call. 
    ...
    double* recvbuf=&buf_my_val[0];
    double* sendbuf=&buf_remote_val[0];
  
    int nelements=submat_indexing_.getMySize(mype_);
    if( nelements>0 )memcpy(recvbuf+my_displ[mype_],sendbuf+remote_displ[mype_],nelements*sizeof(double));
    ...
    for(int p=0;p<npes_-1;p++){
        ...
        // receive
        if( my_sizes_[src] ){
            MPI_Irecv(recvbuf+my_displ[src],     my_sizes_[src],     MPI_DOUBLE, src, p, comm_, &req[0]);
            irecv=true;
        }else{
            irecv=false;
        }
        ...
        // send data
        if( remote_sizes_[dst] ){
            MPI_Isend(sendbuf+remote_displ[dst], remote_sizes_[dst], MPI_DOUBLE, dst, p, comm_, &req[1]);
        }
    ...
    for(int p=0;p<npes_-1;p++){
      ...
      if( mysize ){
          MPI_Irecv(recvbuf+my_displ[src],     mysize,     MPI_DOUBLE, src, p, comm_, &req);
          irecv=true;
      }else{
          irecv=false;
      }
      ...
      if( rsize ){
          //MPI_Isend(sendbuf+remote_displ[dst], remote_sizes_[dst], MPI_DOUBLE, dst, p, comm_, &sent_reqs[nsent]);
          MPI_Send(sendbuf+remote_displ[dst], rsize, MPI_DOUBLE, dst, p, comm_);
      }
      if( irecv )
    ...
    //MPI_Waitall(nrecv, &recv_reqs[0], &recv_status[0]);
#endif
  ...
  for(int im=0;im<nb_local_matrices_;im++){
    ...
    double* plocal_data=local_data_[im];
    ...
    for(int i=0;i<n_;i++){
      for(int j=0;j<n_;j++){
        ...
        //  cout<<"ii="<<ii<<", buf_my_val.size()="<<buf_my_val.size()<<endl;
        //}
        assert( ii<(int)buf_my_val.size() );
        if( ii>-1 )
          plocal_data[il]=buf_my_val[ii];
      }
    }
  }
...
TEMP_DECL
void SubMatrices<float>::gather(const DistMatrix<float>& mat)
{      
#ifdef DEBUG
   if( mype_==0 )
      cout<<"SubMatrices<float>::gather()"<<endl;
#endif
   gather_tm_.start();
   ...
   // build buffer for data array to send
   vector<float> buf_remote_val(tot_remote_size,0.);
   if(send_)
   for(int pe=0;pe<npes_;pe++){
      const int displ=remote_displ[pe];
      assert( displ>=0 );
   ...
   vector<float> buf_my_val(my_size,0.);

#ifdef USE_MPI

#if 0
  // send/recv data 
  MPI_Alltoallv(&buf_remote_val[0],&remote_sizes_[0],&remote_displ[0],MPI_FLOAT,
                &buf_my_val[0],    &my_sizes_[0],    &my_displ[0],    MPI_FLOAT,
                comm_);
#else
//The main interest of the MPI_Isend operation is the possible overlap 
//of computation with communications,
...
//program enter in the next MPI call. 
    ...
    float* recvbuf=&buf_my_val[0];
    float* sendbuf=&buf_remote_val[0];
  
    int nelements=submat_indexing_.getMySize(mype_);
    if( nelements>0 )memcpy(recvbuf+my_displ[mype_],sendbuf+remote_displ[mype_],nelements*sizeof(float));
    ...
    for(int p=0;p<npes_-1;p++){
        ...
        // receive
        if( my_sizes_[src] ){
            MPI_Irecv(recvbuf+my_displ[src],     my_sizes_[src],     MPI_FLOAT, src, p, comm_, &req[0]);
            irecv=true;
        }else{
            irecv=false;
        }
        ...
        // send data
        if( remote_sizes_[dst] ){
            MPI_Isend(sendbuf+remote_displ[dst], remote_sizes_[dst], MPI_FLOAT, dst, p, comm_, &req[1]);
        }
    ...
    for(int p=0;p<npes_-1;p++){
      ...
      int mysize=submat_indexing_.getMySize(src);
      if( mysize ){
          MPI_Irecv(recvbuf+my_displ[src],     mysize,     MPI_FLOAT, src, p, comm_, &req);
          irecv=true;
      }else{
          irecv=false;
      }
      ...
      int rsize=submat_indexing_.getRemoteSize(dst);
      if( rsize ){
          //MPI_Isend(sendbuf+remote_displ[dst], remote_sizes_[dst], MPI_FLOAT, dst, p, comm_, &sent_reqs[nsent]);
          MPI_Send(sendbuf+remote_displ[dst], rsize, MPI_FLOAT, dst, p, comm_);
      }
      if( irecv )
          MPI_Wait(&req, MPI_STATUS_IGNORE);
  ...
  for(int im=0;im<nb_local_matrices_;im++){
    assert( im<(int)local_data_.size() );
    float* plocal_data=local_data_[im];
    // loop over indexes (2 int)
    // index ii and ii+1
    for(int i=0;i<n_;i++){
      for(int j=0;j<n_;j++){
...
{
  for(int im=0;im<nb_local_matrices_;im++){
    os<< "SubMatrices "<<n_<<" x "<<n_<<", name="<<object_name_<<", matrix "<<im<<endl;
    os<<"Indexes: ";
    for(int i=0;i<n_;i++)
...
void SubMatrices<T>::get_array(const int im, vector<T>& val)
{
  ...
  for(int i=0;i<n_;i++){
    map<int,short>::const_iterator p0=mapind.find(submat_indexing_.getLocalIndex(im,i));
    assert(p0!=mapind.end());
    short il=p0->second;
    assert(il<n_);
    for(int j=0;j<n_;j++){
      map<int,short>::const_iterator p1=mapind.find(submat_indexing_.getLocalIndex(im,j));
...
template void SubMatrices<T>::SubMatrices<T>(const string&,
                   vector<vector<int> >&, MPI_Comm, DistMatrix<T>&);
template void SubMatrices<T>::print(ostream&);
#else
template class SubMatrices<DISTMATDTYPE>;
#endif
