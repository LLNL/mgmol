...
void Cdgsum2d(int icontxt, int m, int n, double *A, int lda)
{
    return;
}
...
void BlacsContext::buildCommunicator()
{
  ...
  if( status!=MPI_SUCCESS){
    cerr<<"BlacsContext::buildCommunicator() --- Error in MPI_Comm_group"<<endl;
  ...
  if( status!=MPI_SUCCESS){
    cerr<<"BlacsContext::buildCommunicator() --- Error in MPI_Group_incl"<<endl;
    exit(0);
  }
...
////////////////////////////////////////////////////////////////////////////////
BlacsContext::BlacsContext(MPI_Comm comm_global, const char type, const int max_cpus) :
   ictxt_(-1), myrow_(-1), mycol_(-1),
   comm_global_(comm_global)
{
  ...
  {
    cerr << " BlacsContext::BlacsContext: type = "<<type<<" is an incorrect parameter" << endl;
...
////////////////////////////////////////////////////////////////////////////////
BlacsContext::BlacsContext(MPI_Comm comm_global, const int nprow, const int npcol) :
...
////////////////////////////////////////////////////////////////////////////////
BlacsContext::BlacsContext(MPI_Comm comm_global,
                           const int ipe, const int nprow, const int npcol
                           ) :
   ictxt_(-1), myrow_(-1), mycol_(-1), nprow_(nprow), npcol_(npcol),
   comm_global_(comm_global)
{
   ...
   comm_global_(bc.comm_global_)
{
  ...
  if ( irow < 0 || icol < 0 || nr <= 0 || nc <= 0 ||
       irow+nr > bc.nprow() || icol+nc > bc.npcol() || nr*nc != nprow*npcol )
  {
   ...
   comm_global_(bc.comm_global_)
{
  ...
  {
    cerr << " BlacsContext::BlacsContext: row/col incorrect parameter: "<<type<< endl;
    MPI_Abort(comm_global_,0);
  }
