...
private:
    ...
    std::vector<std::map<int,short> > map_indexes_; // map function index <-> color
    std::vector<std::vector<int> >    vector_indexes_;
    
    int         nb_local_matrices_; // number of matrices on this PE
    const std::vector<std::vector<int> >    local_indexes_; // indexes of "local" data
...
public:
    ...
    int getDoubleIndex(const int i)const
    {
        return double_local_indexes_[i];
    }
    ...
    {
        if( pe_distmat<npes_distmat_ ) return my_sizes_[pe_distmat];
        else return 0;
    }
