                     ...
                     MPI_Comm comm )
{
   ...
   if( recv )memcpy(recvbuf+src*blocksize,
                    sendbuf+dst*blocksize,
                      ...
                      MPI_Comm comm )
{
   ...
   if( recv )memcpy(recvbuf+rdispls[src],
                    sendbuf+sdispls[dst],
                      ...
                      MPI_Comm comm )
{
   ...
   {      
      ...
      if( recv ){
         ...
         if( recvcnts[src]>0 )
            MPI_Irecv(precv, recvcnts[src], MPI_UNSIGNED_SHORT, src, i, comm, &rr);
      }
      if( dst<nfirst ){
         unsigned short* psend = sendbuf+sdispls[dst];
         if( sendcnts[dst]>0 )
            MPI_Isend(psend, sendcnts[dst], MPI_UNSIGNED_SHORT, dst, i, comm, &sr);
      }
      if( recv && recvcnts[src]>0 )MPI_Wait(&rr, MPI_STATUS_IGNORE);
   }
   MPI_Barrier(comm);
