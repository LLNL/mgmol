...
{
    if( onpe0 )
        (*MPIdata::sout)<<"getProcrustesTransform()"<<endl;
    ...
    if( onpe0 )
    {
        ...
        for(int i=0;i<nst;i++)
            if( eigenvalues[i]<tol )(*MPIdata::sout)<<"  "<<eigenvalues[i];
        (*MPIdata::sout)<<endl;
    }
    ...
    dist_matrix::DistMatrix<DISTMATDTYPE> gamma("Gamma",bc, &diag[0], nst, nst);
                                ...
                                dist_matrix::DistMatrix<DISTMATDTYPE>& rotation)
{
    ...
    dist_matrix::DistMatrix<DISTMATDTYPE> gamma("Gamma",bc, &eigenvalues[0], nst, nst);
