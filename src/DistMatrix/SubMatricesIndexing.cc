...
SubMatricesIndexing<T>::SubMatricesIndexing(
    const vector<vector<int> >& indexes, // functions ids for each subdomain on PE
    MPI_Comm comm,
    const DistMatrix<T>& empty_mat):comm_(comm),
                              local_indexes_(indexes)
{
#if USE_MPI
...
//npes_distmat_=npes_;
    ...
    for(int im=0;im<nb_local_matrices_;im++){
        ...
        for(short color=0;color<nlocal;color++)
            map_indexes_[im].insert(pair<int,short>(local_indexes_im[color],color));
    ...
    // for each active pe (for DistMatrix), store local associated functions indexes
            ...
            if(i>=0){
                ...
                while(p1!=local_indexes_[im].end()){
                    ...
                    if(j>=0){
                        ...
                        // index (i,j) value should be received from task pe_distmat
                        const int pe_distmat=pc+pr*npcol;
                        assert( pe_distmat>=0 );
                        assert( pe_distmat<npes_distmat_ );
                        //cout<<"(i,j)=("<<i<<","<<j<<"), pe_distmat="<<pe_distmat<<endl;
    ...
    for(int pe_distmat=0;pe_distmat<npes_distmat_;pe_distmat++){ 
        const vector<int>& pe_double_local_indexes_pe=pe_double_local_indexes[pe_distmat];
        my_sizes_[pe_distmat]=(int)pe_double_local_indexes_pe.size();
        //cout<<"mype="<<mype_<<", my_sizes_[pe="<<pe_distmat<<"]="<<my_sizes_[pe_distmat]<<endl;
        assert( my_sizes_[pe_distmat]%2 == 0 );
        assert( my_sizes_[pe_distmat]<=2*nb_local_matrices_*empty_mat.m()*empty_mat.n() );
...
#if USE_MPI
   // tell all the (DistMatrix)active PEs how many indexes I will need from them
  ...
  cout<<"mype="<<mype_<<", my_sizes_[0]="<<my_sizes_[0]<<endl;
   ...
   for(int p=0;p<npes_distmat_;p++){
      my_sizes_[p]/=2;
      assert( my_sizes_[p]<=nb_local_matrices_*empty_mat.m()*empty_mat.n() );
   }
   if( mype_<npes_distmat_ )
   for(int p=0;p<npes_;p++){
      remote_sizes_[p]/=2;
      assert( remote_sizes_[p]<=nb_local_matrices_*empty_mat.m()*empty_mat.n() );
   }
#ifdef DEBUG
  cout<<"mype="<<mype_<<", remote_sizes_[0]="<<remote_sizes_[0]<<endl;
#endif
  ...
  for(int im=0;im<nb_local_matrices_;im++){
    ...
    for(int ii=0;ii<maxii;ii++){
      map<int,short>::const_iterator p0=mapind.find(double_local_indexes_[2*ii]);
      if(p0!=mapind.end()){
        map<int,short>::const_iterator p1=mapind.find(double_local_indexes_[2*ii+1]);
        if(p1!=mapind.end()){
          short colori=p0->second;
          short colorj=p1->second;
          assert(colori>=0);
          assert(colorj>=0);
          //cout<<"PE:"<<mype_<<", index "<<double_local_indexes_[ii]<<","<<double_local_indexes_[ii+1]<<endl;
          //cout<<"PE:"<<mype_<<", (i,j)=("<<i<<","<<j<<")"<<endl;
          //cout<<"PE:"<<mype_<<", buf_my_val["<<ii/2<<"]="<<buf_my_val[ii/2]<<endl;
          assert( colori+nlocal*colorj<vector_indexes_[im].size() );
