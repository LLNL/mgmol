...
template <class T>
SparseDistMatrix<T>::SparseDistMatrix(MPI_Comm comm, 
                                      DistMatrix<T>& mat,
...
{ 
  ...
  if( target_nb_tasks<=0 )
       target_nb_tasks=npes_; // default
...
SparseDistMatrix<T>::SparseDistMatrix(const SparseDistMatrix<T>& spdistmat):
    comm_global_(spdistmat.comm_global_),
    mat_(spdistmat.mat_)
{
...
#if USE_MPI
void myMPI_AlltoallDouble(double* sendbuf, double* recvbuf,
                          const int count, MPI_Comm comm)
{
  int mype, npes;
  ...
  // copy local data
  memcpy(&recvbuf[mype*count], &sendbuf[mype*count], 
         count*sizeof(double));
  ...
  for(int p=1;p<npes;p++)
  {
    ...
    MPI_Irecv(&recvbuf[src*count], count, MPI_DOUBLE, src, p, comm, &req[0]);
    MPI_Isend(&sendbuf[dst*count], count, MPI_DOUBLE, dst, p, comm, &req[1]);    
...
template <class T>
void SparseDistMatrix<T>::push_back(const int index1, const int index2, 
                                    const T val)
{
...
void SparseDistMatrix<double>::consolidateArray()
{
...
#if USE_MPI
   //if( mype_==0 )
   //   cout<<"Consolidate, consolidation_number="<<consolidation_number_<<endl;
   ...
   
   {
      ...
      int rc=MPI_Alltoall(ndata2send, fraction_tasks, MPI_INT, 
                          ndata2recv, fraction_tasks, MPI_INT, sub_comm);
      ...
      for(int src=0;src<num_neighbors;src++){
         ...
         if( offset!=recvcounts[src] )
            cout<<"src="<<src<<", offset="<<offset<<", recvcounts="<<recvcounts[src]<<endl;
...
template <class T>
void SparseDistMatrix<T>::assign(const int size,
                                 const T* const val)
{
  assign_tm_.start();
  ...
  for(int k=0;k<size;k=k+2){
    ...
    if( vi>-1 ){
      ...
      if( pri!=mypr ){
        cerr<<"mype_="<<mype_<<", size="<<size<<", vi="<<vi<<", val["<<k<<"]="<<val[k]
            <<", i="<<i<<", j="<<j<<", pri="<<pri<<",mypr="<<mypr<<", vv="<<vv
            <<endl
            <<flush;
#if USE_MPI
        int errorcode;
      ...
      if( pcj!=mypc ){
        cerr<<"mype_="<<mype_<<", size="<<size<<", vi="<<vi<<", val["<<k<<"]="<<val[k]
            <<", i="<<i<<", j="<<j<<", pcj="<<pcj<<",mypc="<<mypc<<", vv="<<vv
            <<endl
            <<flush;
#if USE_MPI
        int errorcode;
...
template <class T>
void SparseDistMatrix<T>::setPartitioning(const int target_nb_tasks_per_partition)
{
...
void SparseDistMatrix<double>::parallelSumToDistMatrix()
{
...
#if USE_MPI
  int rc;
  if( npes_>1 )rc = MPI_Allreduce(&max_val_size, &maxsize, 1, MPI_INT, MPI_MAX, comm_global_);
  //for(int i=0;i<val_size;i++){
  ...
  MPI_Alltoall(&tmp_val[0],maxsize,MPI_DOUBLE,
                   buf_val,maxsize,MPI_DOUBLE,comm_global_);
  //if(my_task_index_==0)
...
void SparseDistMatrix<float>::parallelSumToDistMatrix()
{
  ...
  if(my_task_index_==0)
      cout<<"Call MPI_Alltoall, size="<<maxsize<<endl;
  MPI_Alltoall(&tmp_val[0],maxsize,MPI_FLOAT,
                   buf_val,maxsize,MPI_FLOAT,comm_global_);
  pSumSendRecv_tm_.stop();
...
void SparseDistMatrix<double>::parallelSumToDistMatrix()
{
  //if( my_task_index_==0 )cout<<"parallelSumToDistMatrix() using ZOLTAN"<<endl;
  ...
  for(int i=0;i<index_and_val_size;i++)
    max_index_and_val_size=max(max_index_and_val_size,(int)index_and_val_[i].size());
  ...
  if( npes_>1 )rc = MPI_Allreduce(&max_index_and_val_size, &newsize, 1, MPI_INT, MPI_MAX, comm_global_);
  assert( newsize>0 );
  for(int i=0;i<index_and_val_size;i++){
    index_and_val_[i].resize(newsize, -1.);
  }
    
  // Setup array of destination processor numbers for each of the objects to be sent.
  ...
  double* recvbuf;
  if( nreturn>0 )
    recvbuf=new double[newsize*nreturn];
  double* sendbuf;
  if( nsend>0 )
    sendbuf=new double[newsize*nsend];
  for(int i=0;i<nsend;i++){
    const int dst=proclist[i];
    const int dst_task_index=other_tasks_indexes_[dst];
    assert( dst_task_index<ntasks_mat_ );
    memcpy(&sendbuf[i*newsize],&index_and_val_[dst_task_index][0],newsize*sizeof(double));
  }
  ...
  mat_.clear();
  if( recv )assign(newsize*nreturn,&recvbuf[0]); // assign values received to DistMatrix
...
void SparseDistMatrix<float>::parallelSumToDistMatrix()
{
  ...
  float* recvbuf;
  if( nreturn>0 )
    recvbuf=new float[newsize*nreturn];
  float* sendbuf;
  if( nsend>0 )
    sendbuf=new float[newsize*nsend];
  for(int i=0;i<nsend;i++){
    const int dst=proclist[i];
    const int dst_task_index=other_tasks_indexes_[dst];
    assert( dst_task_index<ntasks_mat_ );
    memcpy(&sendbuf[i*newsize],&index_and_val_[dst_task_index][0],newsize*sizeof(float));
  }
...
void SparseDistMatrix<double>::sendRecvData()
{
  ...
  if( ntasks_per_partition_>1 )
  {
    ...
    if( rtasks_distmatrix_->isRemoteTaskActive(dest_task) ){
      ...
      if( size_index_and_val>0 )
         memcpy(sbuf_val, &index_and_val_[dst][0], size_index_and_val*sizeof(double) );
      if( size_index_and_val<newsize ){
         sbuf_val[size_index_and_val]=-1.;
      }
      MPI_Isend(sbuf_val, newsize, MPI_DOUBLE, 
                dest_key, 0, partition_comm_->comm(), psr);      
    }
  }
#endif
  ...
  if( recv ){
     ...
     if( index_and_val_[mytask_index].size()>0 )
        assign((int)index_and_val_[mytask_index].size(),&index_and_val_[mytask_index][0]);
  }
  ...
  for(int p=0;p<ntasks_per_partition_-1;p++){
    ...
    if( p<ntasks_per_partition_-2 ){
      ...
      if( rtasks_distmatrix_->isRemoteTaskActive(dest_task) ){
        ...
        int rc=MPI_Wait(psr, MPI_STATUS_IGNORE); // wait for previous send to complete before reusing buffer
        assert( rc==MPI_SUCCESS );

        if( size_index_and_val>0 )
           memcpy(&sbuf_val[0], &index_and_val_[dst][0], size_index_and_val*sizeof(double) );
        if( size_index_and_val<newsize ){
           sbuf_val[size_index_and_val]  =-1.;
        }
    ...
    if( recv )assign(newsize,tbuf_val); // assign values received to DistMatrix
  }
...
void SparseDistMatrix<float>::sendRecvData()
{
  ...
  if( ntasks_per_partition_>1 )
  {
    ...
    if( rtasks_distmatrix_->isRemoteTaskActive(dest_task) ){
      ...
      assert( size_index_and_val<=newsize );
      if( size_index_and_val>0 )
         memcpy(sbuf_val, &index_and_val_[dst][0], size_index_and_val*sizeof(float) );
      if( size_index_and_val<newsize ){
         sbuf_val[size_index_and_val]=-1.;
      }
      MPI_Isend(sbuf_val, newsize, MPI_FLOAT, 
                dest_key, 0, partition_comm_->comm(), psr);      
    }
  }
#endif
  ...
  for(int p=0;p<ntasks_per_partition_-1;p++){
    ...
    if( p<ntasks_per_partition_-2 ){
      ...
      if( rtasks_distmatrix_->isRemoteTaskActive(dest_task) ){
        ...
        if( size_index_and_val>0 )
           memcpy(&sbuf_val[0], &index_and_val_[dst][0], size_index_and_val*sizeof(float) );
        if( size_index_and_val<newsize ){
           sbuf_val[size_index_and_val]  =-1.;
        }
...
void SparseDistMatrix<double>::parallelSumToDistMatrix2()
{
  ...
  if( npartitions_>1 )
  {
    ...
    for(int i=0;i<ntasks_mat_;i++){
        memcpy(pmy_data, &index_and_val_[i][0], my_data_desc[i]*sizeof(double));
    ...
    for(int i=1;i<npartitions_;i++)
    {
      ...
      int tag=2*i;
      MPI_Irecv(&remote_data_desc[0], ntasks_mat_, MPI_INT, src,  tag, comm_global_, &rr1);
      MPI_Isend(&my_data_desc[0],     ntasks_mat_, MPI_INT, dest, tag, comm_global_, &sr1);
      ...
      if( remote_data_size>0 ){
          ...
          MPI_Irecv(&remote_data[0], remote_data_size, MPI_DOUBLE, src,  tag+1, comm_global_, &rr2);
      }    
      if( my_data_size>0 )
      MPI_Isend(&my_data[0],     my_data_size,     MPI_DOUBLE, dest, tag+1, comm_global_, &sr2);
      
      // append received data to index_and_val_
      if( remote_data_size>0 ){
          double* premote_data=&remote_data[0];
          MPI_Wait(&rr2, MPI_STATUS_IGNORE);
          //if( my_task_index_==0 )cout<<"append received data to index_and_val_ ..."<<endl;
          for(int pe=0;pe<ntasks_mat_;pe++){
#if 1
    ...
    MPI_Barrier( comm_global_ ); // to make sure all the communications are completed and memory can be
                          // released
    delete[] my_data;
    delete[] my_data_desc;
  }
...
void SparseDistMatrix<float>::parallelSumToDistMatrix2()
{
  ...
  if( npartitions_>1 )
  {
    ...
    for(int i=0;i<ntasks_mat_;i++){
        memcpy(pmy_data, &index_and_val_[i][0], my_data_desc[i]*sizeof(float));
        pmy_data += my_data_desc[i];
    }
    ...
    for(int i=1;i<npartitions_;i++)
    {
      ...
      if( remote_data_size>0 ){
          remote_data.resize(remote_data_size);    
          MPI_Irecv(&remote_data[0], remote_data_size, MPI_FLOAT, src,  tag+1, comm_global_, &rr2);
      }    
      if( my_data_size>0 )
      MPI_Isend(&my_data[0],     my_data_size,     MPI_FLOAT, dest, tag+1, comm_global_, &sr2);
...
#if MEMMON
   if( mype_==0 ){
       cout<<"parallelSumToDistMatrix, npartitions_="<<npartitions_<<endl;
       memmon_print_usage();
   }
#endif
   //if( mype_==0 )cout<<"parallelSumToDistMatrix, npartitions_="<<npartitions_<<endl;
   //cout<<"My task = "<<mype_<<", size(before)="<<size()<<endl;
   consolidateArray();
...
template void SparseDistMatrix<T>::assign(const int size,
                                      const T* const val);
