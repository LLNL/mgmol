...
public:
    ...
    virtual void setup(const double kbt, const int num_el, const std::vector<std::vector<int> >& global_indexes)
    {
        width_= kbt;
        nel_ = num_el;
    ...
    virtual void initializeGramMatrix(const SquareLocalMatrices<MATDTYPE>& ss, const int orbitals_index)=0;

    virtual void addMatrixElementSparseH(const int st1, const int st2, const double val)=0;
    virtual void addMatrixElementsSparseH(const SquareLocalMatrices<MATDTYPE>& slH)=0;
...
//    virtual SquareLocalmatrices& getLocalS()const=0;
    ...
    virtual double getTraceDiagProductWithInvS(std::vector<PROJMATDTYPE>& ddiag)=0;
    ...
    virtual void setDMuniform(const PROJMATDTYPE nel, const int orbitals_index)=0;
    
    virtual double dotProductWithInvS(const SquareLocalMatrices<MATDTYPE>& ss)=0;
    virtual double dotProductWithDM(const SquareLocalMatrices<MATDTYPE>& ss)=0;
    virtual double dotProductSimple(const SquareLocalMatrices<MATDTYPE>& ss)=0;
    ...
    virtual double getLinDependent2states(int& st1, int& st2, const bool)const=0;
    ...
    virtual void setHB2H()
    {
        exitWithErrorMessage("setHB2H");
    }    
    ...
    virtual double computeChemicalPotentialAndOccupations(const double width, 
                                                  const double nel,
                                                  const int max_numst)
    {
        (void) width;
    ...
    virtual void setDMto2InvS()
    {
        exitWithErrorMessage("setDMto2InvS");
    }  
