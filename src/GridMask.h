...
public:
    ...
    int loc_numpt(const unsigned short ln)const{
        return loc_numpt_[ln];
    }
    ...
    int init(const Vector3D&, const double rcut,
    ...
    void assign(const unsigned short iloc, const unsigned short level, 
                ...
                const std::vector<lmasktype>& val);
    ...
    void clear(){
        for(unsigned short l=0;l<nlevels_;l++)clear(l);
    }

    short mask_not_zero(const unsigned short level, const unsigned short iloc) const {
    ...
    virtual void apply(pb::GridFunc<float>&, const unsigned short, const unsigned short)=0;
    virtual void apply(pb::GridFunc<double>&, const unsigned short, const unsigned short)=0;

    virtual void apply(float*, const unsigned short, const unsigned short,
                       const bool first_application=false)=0;
    ...
    template<typename T>
    void setZero(T *u, const unsigned short level, const unsigned short iloc)const
    {
        memset(u+loc_numpt_[level]*iloc, 0, loc_numpt_[level]*sizeof(T));
    }
    
    template<typename T>
    void setZero(pb::GridFunc<T>& gu, const unsigned short level, const unsigned short iloc)const
    {
    ...
    template<typename T>
    void multiplyByMask(T *u,
                        const unsigned short level,
                        const unsigned short iloc)const;
    template<typename T>
    void cutWithMask(T *u,
                     const unsigned short level,
                     const unsigned short iloc)const;
};
